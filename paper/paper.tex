...
To treat the exceptional boundary point, where the nonexceptional theory
...
In both settings, we derive asymptotic expansions of the edge-density deficit
and the rate function that governs the exponential decay of the upper-tail probability.
Moreover, the conditioned random graph converges in cut distance
to the corresponding bipodal optimizer as $n\to\infty$.
As $(p,r)$ approaches the phase boundary from the symmetry-breaking side,
the optimizers converge to their constant limits through two distinct mechanisms.
For each fixed nonexceptional target density, one block shrinks to zero measure,
giving convergence in $L^1$ but not in $L^\infty$.
...
\input{sections/lz_boundary}
\input{sections/nonexceptional}
\input{sections/singular}
\input{sections/lean}
...
\input{sections/appendix_preliminaries}
\input{sections/appendix_lz_boundary}
\input{sections/appendix_nonexceptional_endpoint}
\input{sections/appendix_singular_endpoint}
