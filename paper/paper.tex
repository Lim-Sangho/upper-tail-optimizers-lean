...
bipodal (two-block) and unique up to relabelling.
Its block parameters depend analytically on $(p,r)$.
We derive first-order expansions for the edge-density deficit and block
parameters, together with a second-order expansion of the rate function
...
\input{sections/local_reduction}
\input{sections/nondegeneracy}
\input{sections/local_optimizer}
\input{sections/singular_endpoint}
\input{sections/lean}
...
They also verified that the project built without errors and checked
the versions of Lean (v4.33.1) and Mathlib (v4.33.1).
...
\input{sections/appendix_local_reduction}
\input{sections/appendix_local_optimizer}
