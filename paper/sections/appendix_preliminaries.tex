...
We first recall two results from~\cite{kenyon2014entropy} needed for the
proof of \Cref{thm:krrs-analytic-extension}.
Their Theorem~1.1 describes the optimizer parameters only for
\(\vartheta:=\tau-\eps^m>0\).  The rest of the appendix constructs their
two-sided analytic continuation and verifies that it agrees with those
optimizer parameters where they are defined.

\begin{theorem}[{\cite[Theorem~1.1]{kenyon2014entropy}}]
\label{thm:krrs-bipodality}
Let \(H\) be a \(d\)-starlike graph with \(m\ge2\) edges, and let
...
\[
  ...
  t(H,W)=\tau
\]
is unique up to relabelling and is bipodal.  Its parameters
\((q_{11},q_{12},q_{22},c)\) are jointly real-analytic in \((\eps,\tau)\) on
the region
\[
  \left\{(\eps,\tau):
    \eps\ne\frac{d-1}{d},\quad
...
\[
  ...
  c=O(\tau-\eps^m),
\]
where \(a_d(\eps)\in(0,1)\) satisfies
\[
  S_0'\bigl(a_d(\eps)\bigr)
  =2S_0'\bigl(\zeta_d(\eps)\bigr)-S_0'(\eps).
\]
\end{theorem}

Note that since \(S_0'\) is a strictly decreasing bijection from \((0,1)\) onto
\(\mathbb R\), the preceding equation uniquely determines \(a_d(\eps)\).

\begin{theorem}[{\cite[Theorem~3.3]{kenyon2014entropy}}]
...
\(\zeta_d(\eps)\).  The map \(\zeta_d:(0,1)\to(0,1)\) is strictly decreasing,
has nowhere-vanishing derivative, and is an involution.  Its unique fixed
point is \((d-1)/d\).
\end{theorem}

We also remark that \(\zeta_d(\eps)\) is the unique maximizer of
\(z\mapsto\psi_d(\eps,z)\) over \((0,1)\); see
\cite[Section~4]{kenyon2014entropy}.
...
every nonexceptional \(\eps_0\), there are a neighbourhood \(U\) of \(\eps_0\)
...
through \(\vartheta=0\).  The main purpose of the proof is to construct this
two-sided jointly analytic extension, which is needed for the boundary
analysis in
\Cref{thm:positive-second-variation,thm:local-optimizer-structure}.
...
\[
  ...
  z_0:=\zeta_d(\eps_0).
\]
By the involution and fixed-point assertions in
\Cref{thm:krrs-cross-density}, \(z_0\ne\eps_0\).  Set
\(a_0:=a_d(\eps_0)\), so that
\begin{equation}\label{eq:krrs-small-block-limit}
  S_0'(a_0)=2S_0'(z_0)-S_0'(\eps_0).
\end{equation}
By \Cref{thm:krrs-bipodality}, the parameters converge to
...
Strict convexity of \(u\mapsto u^d\) gives
...
\]
gives neighbourhoods of \((\eps_0,a_0,z_0,0)\) and a unique
...
neighbourhood, \(Q\) and \(C\) take values in \((0,1)\) whenever
...
\begin{equation}\label{eq:krrs-quotient-expansion}
  L(\eps,a,b,\vartheta)
  =\frac{\mathcal N_\eps(b)}{\mathcal A(\eps,b)}
    +\vartheta M(\eps,a,b)+O(\vartheta^2),
\end{equation}
where
...
parameters by \((a,b,q,c)(\vartheta)\), after relabelling so that the first
...
\((a_0,z_0,\eps_0,0)\), so they eventually lie in the neighbourhood
...
zero surplus.  Hence, by continuity of \(C\) and \(Q\), every sufficiently
...
a number \(\Delta_{\mathrm{IFT}}>0\), an open neighbourhood
...
depending on \(\eps\), so the fibre of
...
\(U\times(0,\Delta)\) the entropy maximizer is unique up to relabelling and
bipodal.  For each \((\eps,\vartheta)\) in this set, let
\(W^+_{\eps,\vartheta}\) be a representative of this maximizer, with its
podes labelled so that the first pode has size \(c^+\) and
...
\eqref{eq:krrs-optimizer-map} lie in the local solution neighbourhoods
...
because a bipodal graphon with a null pode is constant and has surplus
...
open neighbourhood \(\mathcal O\subset U\times(0,\Delta)\) of
