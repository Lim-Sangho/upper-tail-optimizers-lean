...
empty.  For $p,r\in(0,1)$, define the reduced objective
...
Let $B_{\eps,r}$ denote the maximizer. Then for every $p\in(0,1)$,
\begin{equation}\label{eq:reduced-objective-attainment}
  s(B_{\eps,r})=S_H(\eps,r^m),
  \qquad
  I_{p,r}(\eps)=I_p(B_{\eps,r}).
\end{equation}
Furthermore, 
...
\Cref{thm:krrs-analytic-extension} with $\tau=r^m$ supplies $B_{\eps,r}$
that satisfies $e(B_{\eps,r})=\eps$, $t(H,B_{\eps,r})=r^m$,
maximizes entropy among graphons with these densities,
and is unique up to relabeling.
Thus the definition \eqref{eq:fixed-density-entropy} gives $s(B_{\eps,r})=S_H(\eps,r^m)$,
and substituting $B_{\eps,r}$ into \eqref{eq:relative-entropy-identity} gives,
for every $p\in(0,1)$,
\[
  I_p(B_{\eps,r})=-2S_H(\eps,r^m)-\log(1-p)+\eps\log\frac{1-p}{p}=I_{p,r}(\eps).
\]
...
is $B_{\eps^*,r}$ up to relabeling, where $\eps^*=e(W^*)$ is a minimizer of
$I_{p,r}$ over $(r-\rho,r)$.  Conversely, if
$\eps\in(r-\rho,r)$ minimizes $I_{p,r}$ over $(r-\rho,r)$, then
$B_{\eps,r}$ is an optimizer of \eqref{eq:graphon-variational-problem}.  In
