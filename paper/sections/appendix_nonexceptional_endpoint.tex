...
\]
On the other hand, $B_{\eps^*,r}$ has edge density $\eps^*$ and
...
\[
  \Phi_H(p,r)\le I_p(B_{\eps^*,r})=I_{p,r}(\eps^*).
\]
...
\Cref{thm:krrs-analytic-extension}, $W^*$ agrees with $B_{\eps^*,r}$ up to relabeling.

For any $\eps\in(r-\rho,r)$, the graphon $B_{\eps,r}$ is feasible for the
variational problem defining $\Phi_H(p,r)$ in \eqref{eq:graphon-variational-problem}, so
\[
  I_{p,r}(\eps)=I_p(B_{\eps,r})\ge\Phi_H(p,r)
  =I_{p,r}(\eps^*).
\]
...
$I_p(B_{\eps,r})=I_{p,r}(\eps)=I_{p,r}(\eps^*)
=\Phi_H(p,r)$, and $B_{\eps,r}$ is feasible, so it is an optimizer
attaining $\Phi_H(p,r)$.
...
corresponding change in the block densities.  Fix a compact interval $K$
inside the $r$-neighborhood furnished by the theorem, take $r\in K$,
and let \(p\uparrow\pc(r)\) from the
symmetry-breaking side.  Relabel the podes as in \Cref{thm:nonexceptional-endpoint}, so
...
All limits below refer to this regime, and the $O_{H,K}$-bounds are uniform
for $r\in K$ and $p$ sufficiently close to $\pc(r)$.
...
Then
\[
  \delta_*=2D_d(r,z)c+O_{H,K}(\delta_*^2).
\]
...
\(\delta_*=\lambda(p,r)/A_H(r)+O_{H,K}(\lambda(p,r)^2)\) from
...
and
\[
  q_{12}(p,r)=\zeta_d(r)+O_{H,K}(\lambda(p,r)),
  \qquad
  q_{11}(p,r)=q_{11}^0(r)+O_{H,K}(\lambda(p,r)).
\]
...
\]
Since \(\vartheta=O_{H,K}(\delta_*)\), \Cref{thm:krrs-analytic-extension} and analyticity at
\((r,0)\) give
\[
  c=O_{H,K}(\delta_*),\qquad
  q_{22}-r=O_{H,K}(\delta_*),\qquad
  q_{12}-z=O_{H,K}(\delta_*),\qquad
  q_{11}-q_{11}^0(r)=O_{H,K}(\delta_*).
\]
...
\begin{equation}\label{eq:bipodal-edge-linearization}
  -\delta_*=(q_{22}-r)+2(z-r)c+O_{H,K}(\delta_*^2).
\end{equation}
...
\[
  \delta_*=2D_d(r,z)c+O_{H,K}(\delta_*^2),
  \qquad
  c=\frac{\delta_*}{2D_d(r,z)}+O_{H,K}(\delta_*^2),
\]
