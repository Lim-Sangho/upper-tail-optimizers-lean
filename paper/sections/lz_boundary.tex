...
that the constant graphon $W\equiv r$ is the unique minimizer
...
In this section, we analyse in detail the behaviour of the function $\varphi_{p,d}$ and of its convex minorant; this is the key to the proof of \Cref{thm:nonexceptional-optimizers}.
...
$x\in(0,1)$ and $u=x^{1/d}$,
\[
  \varphi_{p,d}''(x)=\frac{h_{p,d}(u)}{d^2u^{2d-1}},
\]
so $h_{p,d}(u)$ and $\varphi_{p,d}''(x)$ have the same sign.  If $p\ge p_*$,
...
$h_{p,d}(u)=0$, noting that
\[
  \partial_u h_{p,d}(u)=\frac{d(u-r_*)}{u(1-u)^2}
\]
...
\[
  ...
  \dist(x,\{r^d,\sm(r)^d\})^2
\]
inside the two contact neighbourhoods.

It remains to handle the complement of these neighbourhoods.  On the compact set
\[
...
Fix $r\in(0,1)\setminus\{r_*\}$ and put $p_0=\pc(r)$.
If $p\ge p_*$, then
...
Assume now that $p<p_*$.  If $r<r_*$, then $r=u_a(p_0)$.  If
$p_0\le p<p_*$, then $u_a(p)\ge u_a(p_0)=r$, so
...
$p<p_0$, then $u_a(p)<r<r_*<u_b(p)$, so $r^d$ lies strictly between the two
...
If $r>r_*$, then $r=u_b(p_0)$.  If $p_0\le p<p_*$, then
$u_b(p)\le u_b(p_0)=r$, so $r^d\ge x_b(p)$ and the convex minorant agrees with
$\varphi_{p,d}$ at $r^d$.  If $p<p_0$, then
