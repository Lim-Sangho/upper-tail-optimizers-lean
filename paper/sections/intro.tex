...
The result of Lubetzky and Zhao also yields that the probability of $\mathcal{E}_{H, r}$ is $\exp(-J_p(r)n^2/2 + o_{H,p,r}(n^2))$ when $(p, r)$ is in the replica-symmetric phase.
...
\begin{theorem}[Informal main results]\label{thm:main-results-informal}
  ...
  \begin{enumerate}[label=(\alph*)]
    ...
    conditioned on $\mathcal E_{H,r}$ is bipodal.  After labeling the
    ...
    The $O_{H,U}$-term is uniform for $(p,r)\in U$, while the $o_{H,p,r}$-term
    is taken as $n\to\infty$ with $H,p,r$ fixed.

    \item \label{thm:main-results-informal-b} There exists an analytic curve $h\mapsto(p_h,r_h)$ of
    ...
    $W_h$, up to relabeling.  As $h\downarrow0$, both block proportions
...
affirmative answer away from the exceptional density.  Combined with the
Chatterjee--Varadhan theorem above, it provides the upper-tail probability
asymptotics and the bipodal description of the conditioned random graph
stated in Theorem~\hyperref[thm:main-results-informal-a]{\ref*{thm:main-results-informal}~\ref*{thm:main-results-informal-a}}
...
measure-preserving relabeling, at $(p_h,r_h)$.  As $h\downarrow0$, the sizes
...
For fixed $r$, we first compute the extra cost of $W_\delta$ at $p=\pc(r)$,
...
the same densities and an extra cost of order $\delta^2$; $W_\delta$ has no
...
due to the KRR--S theorem.  Evaluating $I_p(W_\delta)$ at the same value
gives the expansion of the minimum relative entropy.
...
entropy-maximization results.  \Cref{sec:lz-boundary} analyzes the phase
boundary, and \Cref{sec:nonexceptional-endpoint,sec:singular-endpoint}
...
Appendix~\ref{app:notation} collects the notation and asymptotic conventions
used throughout the paper.  Technical arguments deferred from the main text
