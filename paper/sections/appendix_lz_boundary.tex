...
\[
  J_p'(u)=\log\frac{u(1-p)}{(1-u)p},
  \qquad
  J_p''(u)=\frac1{u(1-u)},
  \qquad
  J_p'''(u)=\frac{2u-1}{u^2(1-u)^2}
\]
gives
\[
  h_{p,d}'(u)=(2-d)J_p''(u)+uJ_p'''(u)
  =\frac{d(u-r_*)}{u(1-u)^2}.
\]
...
\[
  h_{p,d}(u)=\frac1{1-u}-(d-1)J_p'(u)
\]
shows that $h_{p,d}(u)\to+\infty$ as $u\downarrow0$ and as $u\uparrow1$.
At the left endpoint $J_p'(u)\to-\infty$, while at the right endpoint
$1/(1-u)$ dominates the logarithmic divergence of $J_p'(u)$.  The intermediate
...
$\varphi_{p,d}$.  For $u=x^{1/d}$, the chain rule gives
\[
  \varphi_{p,d}'(x)=\frac{J_p'(u)}{d u^{d-1}},
  \qquad
  \varphi_{p,d}''(x)
  =\frac{uJ_p''(u)-(d-1)J_p'(u)}{d^2u^{2d-1}}
  =\frac{h_{p,d}(u)}{d^2u^{2d-1}}.
\]
The denominator is positive, so $\varphi_{p,d}''(x)$ has the same sign as
$h_{p,d}(u)$.  When $p\ge p_*$, this proves convexity on $(0,1)$, and
continuity extends it to $[0,1]$.  When $p<p_*$, the change of variables
$x=u^d$ converts the three sign intervals for $h_{p,d}$ into precisely the
...
function $L$ lies below a differentiable function $\varphi$ in a neighbourhood
...
\]
because the only convex combination of points of $[0,1]$ with barycentre
...
If $u=x^{1/d}$, then
\[
  \partial_p\varphi_{p,d}(x)=\frac{p-u}{p(1-p)},
  \qquad
...
\]
The last inequality comes from the strict convexity of $u\mapsto u^d$.
...
\[
  ...
  \right).
\]
The secant slope of the strictly convex function $u\mapsto u^d$ from $u_a$ to
...
\[
  h_{p_*,d}(u)>0\qquad\text{for every }u\ne r_*.
\]
...
It remains to consider $p\downarrow0$.  For every fixed $u\in(0,1)$,
\[
  h_{p,d}(u)
  =
  \frac1{1-u}-(d-1)\log\frac{u(1-p)}{(1-u)p}
...
Fix $r\in(0,1)\setminus\{r_*\}$ and put $p_0=\pc(r)$.  The affine part of the
convex minorant of $\varphi_{p_0,d}$ joins
\[
