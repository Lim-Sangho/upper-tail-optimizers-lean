...
The notation $f=o(g)$ means $f/g\to0$.
...
$t(H,G)$ & Finite graphs $H,G$ &
$t(H,G)=\hom(H,G)/|V(G)|^{|V(H)|}$ is the homomorphism density. \\
$\mathcal E_{H,p,r}$ & $0<p<r<1$ &
$\{t(H,G(n,p))\ge r^{|E(H)|}\}$; the upper-tail event under $G(n,p)$,
with $n$ suppressed in the notation. \\
$W,\W_0$ & $W:[0,1]^2\to[0,1]$ &
...
$B_{\eps,\tau}$ & $\tau=r^m$, $r\in I$,
$\eps\in(r-\rho_0,r)$ &
The unique bipodal entropy maximizer with $e(B_{\eps,\tau})=\eps$ and
$t(H,B_{\eps,\tau})=\tau$, so $s(B_{\eps,\tau})=S_H(\eps,\tau)$.
In particular,
$I_p(B_{\eps,r^m})=I_{p,r}(\eps)$.
...
$W_{p,r}=B_{r-\delta_*(p,r),r^m}$ up to relabeling. \\
$A_{p,r},c(p,r)$;
