...
/-!
# The rank-one homomorphism-density identity (Section 7, `paper/singular_endpoint.tex`)

Section 7 works throughout with **rank-one** graphons `W = f ⊗ f`.  For a `d`-regular `H`
on `v = |V(H)|` vertices such a `W` satisfies the identity displayed, unlabelled, in the
`eq:block-stationarity` step of the proof of `lem:rank-one-kkt-family`
(`singular_endpoint_appendix.tex`), after the two-value reduction of
...
Section 7's candidates are the two-valued kernels `f = s·1_{[0,α]} + t·1_{(α,1]}`, whose
...
This is the combinatorial core of the rank-one density identity of Section 7 (the display
...
/-- **The rank-one identity for the bipodal `H`-density polynomial** (Section 7, the display
...
/-- **The rank-one homomorphism-density identity for the two-block candidates of Section 7**:
...
(`singular_endpoint_appendix.tex`), used in the `eq:block-stationarity` step to
