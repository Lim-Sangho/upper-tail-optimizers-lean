...
/-!
# Local uniqueness of the coalescing scalar family (Section 7, `paper/singular_endpoint.tex`)

`SingularEndpoint/FamilyExists.lean` builds the analytic family `h ↦ (u_h, ℓ_h, γ_h)` of
`lem:rank-one-kkt-family` and `SingularEndpoint/FamilySymm.lean` establishes its
`h ↦ -h` symmetry.  **This file proves the remaining clause of that lemma: its local
uniqueness.**

The paper's formulation is: *up to measure-preserving relabelling, every nearby
nonconstant rank-one KKT point satisfying `eq:block-stationarity` belongs to
this family* — made precise after the statement: if `(s, t, p, μ, α)` is sufficiently
close to `(u_*, u_*, p_*, 1/(m r_*^m), 1/2)`, then `h := (t - s)/2` lies in `(0, h₀)` and
`(s, t, p, r, μ, α) = (s_h, t_h, p_h, r_h, μ_h, α_h)`.  Its
scalar content — the part proved here — is that a nearby triple `(u', ℓ', γ')` solving the
...
`lem:rank-one-kkt-family` itself; `thm:endpoint-optimality` asserts instead
that `W_h` is the unique *minimizer* up to relabelling, and the prose before the family
lemma says the two stages "produce the analytic family and its local uniqueness".
...
in `S`.  The hypotheses fed to it are the same three as in `SingularEndpoint/FamilyExists.lean`:
...
The scalar form of the paper's "up to measure-preserving relabelling, every nearby
nonconstant rank-one KKT point satisfying `eq:block-stationarity` belongs to
this family": there
...
`lem:rank-one-kkt-family`; `thm:endpoint-optimality` states minimizer
