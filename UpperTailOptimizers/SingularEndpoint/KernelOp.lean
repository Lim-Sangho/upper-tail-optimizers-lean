...
/-!
# The kernel operator `T_K` and its `L⁴` bound (Section 7)

`paper/singular_endpoint.tex` introduces, just before `lem:localization-rank-one`, the kernel
operator

`(T_K g)(x) := ∫₀¹ K(x,y) g(y) dy`,

and the proof of that lemma uses the mapping property "`T_W : L^{2d/(2d-1)} → L^{2d}` has norm
at most `‖W‖_{2d}`".  This file formalises that mapping property at the exponent pair the
normalised contraction construction of the **existence** half actually needs,
...
Jensen/Bernoulli lower bound `∫ φ^{d-1} ≥ 1` for a nonnegative `φ` of unit mass (which is what
pins the normalisation `q = ∫ f^d` in the derivation, inside the proof of
`lem:localization-rank-one`, of the pointwise-bound half `0 ≤ f ≤ q^{-1/d}` of
`eq:rank-one-orthogonality`).

**What is not here.**  The contraction itself, and hence the existence half of
`lem:localization-rank-one`, is *not* proved here; neither is the uniqueness clause, the
closeness bound `eq:rank-one-reduction-bounds`, the coercivity bound `eq:moment-and-cost-gap-bounds`,
or `eq:rank-one-reduction-bounds`.  This file is the operator-theoretic foundation
those need.  `SingularEndpoint/Factor.lean` carries the algebraic consequences of the decomposition,
...
functions `ℝ → ℝ` and `MeasureTheory.integral`, matching `SingularEndpoint/Factor.lean` and the rest of
...
* `abs_pow_sub_pow_le_box`, `abs_pow_sub_pow_le_deg` — `|a^{d-1} - b^{d-1}| ≤ (d-1)M^{d-2}|a-b|`
  on `[0,M]`.  The name carries `_box` because `Nondegeneracy/PowBounds.lean` has an
...
/-- The kernel operator `(T_K g)(x) = ∫₀¹ K(x,y) g(y) dy` of `paper/singular_endpoint.tex`. -/
...
the normalised contraction construction of `lem:localization-rank-one` runs on. -/
...
(`Nondegeneracy/PowBounds.lean`), the `[0,1]` bound with the smaller constant `n`; the two are
