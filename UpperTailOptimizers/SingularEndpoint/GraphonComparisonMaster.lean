...
/-!
# Step 4 of `lem:graphon-lagrangian-bound` (Section 7, `paper/singular_endpoint.tex`)

`eq:graphon-lagrangian-bound` is the assembly of three estimates, at a **common** window
radius, exactly as in the paper's *Step 4: assembly*:
...
* **Step 1**, the rank-one law: the refined form of `eq:auxiliary-lagrangian-bound`
  ...
  the only source of a positive `ε_ρ(ν)` coefficient**; Steps 2 and 3 only spend it;
...
of `SingularEndpoint/GraphonComparisonMain.lean` respectively, the third with iterated set-restricted
...
* `exists_comparison_premaster` — Step 5's algebra: `eq:graphon-lagrangian-bound` with the
  bracket multiplying `ε` still carrying `ζ(h)` and `C_err·𝔈`;
...
* `Qh_eq_sq_sub`, `sq_Qh_le`, **`family_Ah_le`** — the fourth clause of
  `eq:graphon-comparison-estimates`, `A_ρ ≤ 16‖f - u_*‖₄⁴ + 16(u_h - u_*)⁴ + 2h⁴`, which
  `singular_endpoint_localization` does not supply;
...
/-- **Step 5 of `lem:graphon-lagrangian-bound`, the algebra.**
...
**The `ε` budget.**  Step 1 contributes `+b_ρ ε_ρ(ν)`.  Against it: `b_ρ/4` for the mixed
rectangles (`rowJensen_exists_mixed` at `β = b_ρ/4`), and `b_ρ/4` for the mixed *residual*
`2a‖E‖²_{2,Ω^c×Ω} ≤ 50a ε_ρ(ν)` (`mixed_resid_le`), which is why the coercivity constant is
capped at `a ≤ b_ρ/200` — the Lean's form of the paper's `C_{d,ρ} = max{1, 1600/b_ρ}`.  What
...
theorem exists_comparison_premaster (hd : 2 ≤ d) (B : KKTFamily d) :
  ...
  -- `SingularEndpoint/RowWindow.lean` names the same terms `centralSet` and `tailMass`.
...
Comparing with the paper's display: `M_ρ^{-1}[‖E‖₂² + ∫_{𝓝_ρ}Q_h²dν + ν(T_ρ)]` and `-M_ρΔ²`
are here, and the Lagrangian term is the layer's graph-free `η_hΔ_h(ν)` where the paper writes
`μ_h{t(H,W) - r_h^m}`; the paper reconciles the two in the display following
`eq:first-variation`, at the cost of an `O(Δ²)` that the retained `MΔ²` absorbs, and that reconciliation is not
formalised here (see `SingularEndpoint/DistributionFinal.lean`).
...
/-! ## The fourth clause of `eq:graphon-comparison-estimates`
...
`|Δ| ≤ Mh⁴` and `‖E‖₂ ≤ Mh²`.  The fourth, `A_ρ ≤ Ch⁴`, is the paper's remark that
`|Q_h(f)| ≤ (|f - u_*| + Ch)²` pointwise while `‖f - u_*‖₄ = O(h)`; it is proved here. -/
...
(`KKTFamily.tendsto_u_slope`), so the right-hand side is `O(h⁴)` — the fourth clause of
`eq:graphon-comparison-estimates`. -/
...
This is the form `terminal_optimality_of_master` (`SingularEndpoint/Terminal.lean`) consumes. -/
