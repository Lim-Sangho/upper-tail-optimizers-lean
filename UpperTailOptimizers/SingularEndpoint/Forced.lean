...
/-!
# The singular endpoint values are forced (Section 7, `paper/singular_endpoint.tex`)

`sec:singular-endpoint` opens by displaying the distinguished point

```
r_* = (d-1)/d,   p_* = (d-1)/((d-1) + exp(d/(d-1))),   γ_* = r_*^{-d},
```

and asserts, in the prose opening `sec:singular-endpoint`, that *these values are
forced whenever the two factor values in such an analytic family coalesce at an interior
point*.  This file proves that assertion.
...
`SingularEndpoint/Contact.lean`: a triple zero can only sit at `(w, p, γ) = (r_*, p_*, γ_*)`.
...
* `triple_contact_forced` — the assembled statement, the section's opening claim.
-/
...
/-- **The singular endpoint values are forced** (the opening claim of
...
This is the converse of the triple-contact identities of `SingularEndpoint/Contact.lean`: those
