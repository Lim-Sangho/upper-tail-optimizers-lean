...
/-!
# The singular endpoint supporting gap is positive (Section 7, `paper/singular_endpoint.tex`)
...
elementary facts about the scalar convexity defect `R_d`, which the paper now defines in
an unlabelled display just before the edge/moment identity.
...
which is positive on `(0,1)` away from `u_* = r_*` and vanishes there.  So the quotient is
