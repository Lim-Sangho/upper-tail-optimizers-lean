...
/-!
# The quantitative half of the rank-one law comparison (Section 7)

The rank-one distribution lemma splits in
two: its localisation half is inherited from `lem:localization-rank-one` — in the
Lean, `SingularEndpoint/DistributionLocal.lean` (`exists_gamAvg_localization`, `exists_tail_mass_le`,
`exists_mean_bootstrap`) — and its quantitative half is `lem:auxiliary-lagrangian-bound`
("Rank-one Lagrangian lower bound"), whose conclusion is `eq:auxiliary-lagrangian-bound`.
The exact expansion the quantitative half starts from is the unlabelled Lagrangian expansion
following `eq:first-variation`, in the Lean
`KKTFamily.distributionJ_sub_eq` (`SingularEndpoint/DistributionGap.lean`).  **This file is what comes
after that expansion**: the estimate of the quadratic form `∬K_h dσdσ` over the central
...
Notation: the paper abbreviates `A_{h,ρ}(ν) := ∫_{𝓝_ρ}Q_h²dν` and
`ε_ρ(ν) := ν(𝒯_ρ)`, `𝒯_ρ := [0,2] \ 𝓝_ρ`; the docstrings below keep the shorter `A_ρ` and
`ε_ρ`.  The kernel bounds of `lem:central-kernel-bound` are `ρ`-uniform (`C_d`, not `C_ρ`).
...
* the corner clause — the paper's `sup_{0<h<h_ρ}‖Θ_h - d³/4‖_{L^∞(𝓝_ρ²)} ≤ a/8`, with `a = d³/24` the constant of
  `lem:first-variation-bound`.  It appears as
  `hCorner : ∀ x ∈ centralWindow d ρ, ∀ y ∈ centralWindow d ρ,
   |B.kernelTheta h x y - (d:ℝ)^3/4| ≤ δ` together with `hδ : δ ≤ a_ρ/8`, `a_ρ = d³/24`;
* the boundedness of the one-variable functions `u_{0,h}` and `u_{d,h}` of the
  decomposition.  The four coefficients of
  `P_h` are bounded in `SingularEndpoint/PowFirstOrder.lean` (`exists_kp00_window`,
...
`SingularEndpoint/KernelError.lean` (`exists_kernelTheta_error`, `exists_u0_window`,
`exists_ud_window`, `exists_kernelTheta_bound`), and `SingularEndpoint/DistributionWindow.lean` — which
...
`SingularEndpoint/DistributionGap.lean` defines `KKTFamily.sigmaInt h ν A f = ∫_Afdν - ∫_Afdν_h`; no
...
`C_ρL_hε_ρA_ρ^{3/16} ≤ (a_ρ/8)A_ρ + C'_ρ(L_hε_ρ)^{16/13}`,
...
`ε_ρ ≤ Kh⁴`, `|Δ_h(ν)| ≤ Kh⁴` (in the paper consequences of
`lem:localization-rank-one`) and `exists_firstVariation_upperGap` conclude.  The paper reaches the same point without a Young step: its mixed-rectangle bound carries no
`A_ρ^{3/16}` term to absorb.  That step is `SingularEndpoint/DistributionAbsorption.lean`
...
`exists_abs_tailQuad_le` supplies `C_t = Cε_ρ²`.  The uniqueness statement the old law
lemma also carried is not here either: the paper proves it only at graphon level, in
the rigidity step of the proof of `thm:endpoint-optimality` (Section 7.6);
its one-dimensional form is `KKTFamily.distribution_unique` in `SingularEndpoint/DistributionUnique.lean`.
...
* `KKTFamily.kernel_window_decomposition` — `lem:central-kernel-bound` on `𝓝_ρ²`
...
* `KKTFamily.abs_tailSqSigma_le` and **`KKTFamily.centralQuad_lower`** — the
  central-square estimate `≥ -(δ + a_ρ/8)A_ρ - C_ρ{(1+2^d)ε_ρ + |Δ_h(ν)|}²` .

*The mixed and tail parts.*

* `KKTFamily.mixedQuad`, `.tailQuad`, `.tailSlice`;
* **`KKTFamily.exists_abs_mixedQuad_le`** — `lem:auxiliary-lagrangian-bound` integrated
...
* `KKTFamily.exists_firstVariation_lower_PsiT` — the first-variation bound `a_ρ = d³/24` transferred to
  the continued potential, the first half of `lem:auxiliary-lagrangian-bound`;
...
subset only**.  The variant `integrableOn_of_continuousOn` of `SingularEndpoint/DistributionMeasure.lean`
...
(`SingularEndpoint/DistributionMeasure.lean`) at exponent `1/2` applied to `Q_h²`, the concave Jensen step
...
(`SingularEndpoint/PowFirstOrder.lean`); they are what makes the product of two window points land in
...
the fourth term of `lem:central-kernel-bound` with the nonnegative square
`(d³/4)Q_h(x)Q_h(y)` split off.  `lem:central-kernel-bound` is exactly the
statement that this is at most `δ_ρ|Q_h(x)||Q_h(y)|`. -/
...
/-! ## `lem:central-kernel-bound` on the window, in residual form -/

/-- **`lem:central-kernel-bound` on the central square**, with the two one-variable
...
mechanism of `paper/singular_endpoint.tex`: `|f| ≤ C|Q_h|` on `𝓝_ρ` gives
...
/-- **The corner remainder is `δ|Q_h(x)|A_ρ^{1/2}` pointwise**, which is
`lem:central-kernel-bound` integrated in one variable.  The clause is a hypothesis; see the module
docstring. -/
...
reals.  `L = d³/192 = a_ρ/8` and `K = 192C_u²/d³` are the two Young constants of
`paper/singular_endpoint.tex`; the hypotheses `hL`, `hK` state them without division so
that the proof is a `linear_combination`. -/
...
`∬_{𝓝_ρ²}K_hdσdσ ≥ -(δ + a_ρ/8)A_ρ - C_ρ{(1+2^d)ε_ρ + |Δ_h(ν)|}²`,   `a_ρ = d³/24`,

with `C_ρ = C_p + 192C_u²/d³` explicit.  The three hypotheses that
`lem:central-kernel-bound` supplies in the paper — the four coefficient bounds
`C_p`, the bounds `C_u` on `u_{0,h}, u_{d,h}`, and the corner clause
`lem:central-kernel-bound` — are hypotheses here; all three are proved elsewhere
(`exists_kp00_window` and friends in `SingularEndpoint/PowFirstOrder.lean`, `exists_u0_window`,
`exists_ud_window` and `exists_kernelTheta_error` in `SingularEndpoint/KernelError.lean`) and
discharged in `SingularEndpoint/DistributionWindow.lean`; see the module docstring.

The nonnegative square `(d³/4)(∫_{𝓝_ρ}Q_hdσ)²` of the paper is discarded through
`dd/4·I_Q² ≥ 0`, exactly as the paper does. -/
...
`∫_{T_ρ}(∫_{𝓝_ρ}K_h(x,y)dσ(y))dν(x)` in which `lem:auxiliary-lagrangian-bound` applies
...
The inner σ-integral *is* the left-hand side of `lem:auxiliary-lagrangian-bound`
(`exists_tail_interpolation`) at the frozen tail point, so the only work is to integrate a
...
`𝒥_h(ν) - 2∫𝒦_hdν + 𝒥_h(ν_h)` (`SingularEndpoint/DistributionGap.lean`); this identifies it with the
...
part as `2∬_{𝓝_ρ×T_ρ}`, using the symmetry of the kernel; formally the two orders of
...
hypotheses hold on `𝓝_ρ` because two window points multiply into `[0,1]`.  This is the first
half of `lem:auxiliary-lagrangian-bound`, `∫Ψ_hdν ≥ a_ρA_ρ + ∫_{T_ρ}Ψ_hdν` with
`a_ρ = d³/24`. -/
...
Four inputs are combined: the exact expansion `eq:first-variation`
(`distributionJ_sub_eq`), the dual split `lem:auxiliary-lagrangian-bound` through the first-variation bound
`a_ρ = d³/24`, the three-way split `sigmaQuad_split`, and the central estimate
`centralQuad_lower`.  The coefficient is `a_ρ - δ - a_ρ/8 ≥ d³/32` once `δ ≤ a_ρ/8`.
...
inequality that the Young absorption of `paper/singular_endpoint.tex` acts on:
...
`C_m = C_ρL_hε_ρ(ε_ρ + |Δ| + A_ρ^{3/16})` and `C_t = Cε_ρ²` this is exactly the display the
paper absorbs. -/
