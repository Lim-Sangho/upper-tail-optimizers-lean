...
/-!
# The tail mass and the Factor clause of `lem:localization-rank-one` (Section 7)

`lem:localization-rank-one` of `paper/singular_endpoint.tex` has four displays.  The first,
`eq:graphon-quartic-localization`, is proved in `SingularEndpoint/L4Bound.lean`
(`integral_dist_pow_four_localized`) and `SingularEndpoint/FamilyLocalization.lean`
(`family_localization`, `family_localization_sq`).  The second,
the sharp `∫W^d` bound, is `family_ld_sharp` in `SingularEndpoint/LdSharp.lean` — an earlier
version of this docstring claimed it came with the first, which was wrong: the two files above
instantiate `gamInt_localized` only for the `∫|W - r_*|⁴` half.  This file proves the
remaining two displays:

* **`eq:factor-tail-mass`**, `|{x : f(x) ∉ 𝓝_ρ}| ≤ M_ρ h⁴`, which is Chebyshev at exponent
  four applied to the closeness clause `‖f - u_*‖₄ ≤ Cε` of
  `eq:rank-one-reduction-bounds` together with `ε⁴ = ‖W - r_*‖₄⁴ ≤ Mh⁴`
  (`family_localization`);
* the **Factor clause** `‖E‖₂ ≤ Mh²` and `|∫f^d - q_h| ≤ Mh⁴`, which sharpens the `O(h)`
  bound on `‖E‖₄` coming from closeness by one power of `h`.  The extra power comes from the
  combined stability inequality of `SingularEndpoint/FactorStability.lean` (`eq:moment-and-cost-gap-bounds`
  joined with `eq:rank-one-reduction-bounds`): under the competitiveness
  hypotheses the Hölder deficit `(∫W^d)^{m/d} - t(H,W)` is `O(h⁴)`, so
  ...
  `|q - q_h| ≤ Mh⁴` is then *derived*, not assumed: the exact expansion
  `eq:rank-one-reduction-bounds` gives `|∫W^d - q²| ≤ C‖E‖₂²`, and `∫W^d = q_h² + O(h⁴)` is
  the sharp `∫W^d` bound.

## What is a hypothesis here

The **existence** of the nonlinear Factor decomposition — chunk (I) of
`lem:localization-rank-one` — is not used.  Every statement below carries either a
`FactorDecomp d W` or the raw solution data (`f`, `q`, the pointwise Factor equation) as an
explicit hypothesis, exactly as `SingularEndpoint/FactorStability.lean` and
`SingularEndpoint/FactorSolution.lean` do, so that the join supplied elsewhere composes with them
without any change of statement.

Likewise the sharp `∫W^d` bound enters the moment clause as the hypothesis
`|∫W^d - q_h²| ≤ Kh⁴` (note `q_h² = r_h^d`, `KKTFamily.graphon_Wmoment`), and chunk (III)
`eq:rank-one-reduction-bounds` as `|t(H,W) - q^v| ≤ C₃‖E‖₂³`, which is how
`SingularEndpoint/FactorStability.lean` already phrases it.
...
* `exists_factorTail_factor_clause` — the two together in the paper's `∃M, ∃h₀` shape;
...
bounded measurable `f` obeying the `L⁴` half of `eq:rank-one-reduction-bounds`,
`‖f - u_*‖₄ ≤ Cε`,
...
`SingularEndpoint/LocalizationMain.lean`.  `family_factor_tail_mass` below is the specialisation in
...
absorption below needs, obtained from the `L⁴` half of `eq:rank-one-reduction-bounds` alone.
-/
...
Every term of `eq:rank-one-reduction-bounds` carries `E^{d-k}` with `d - k ≥ 2`, so bounding
...
This is the "after decreasing `h₀`, the cubic term is absorbed" sentence of the proof of
`lem:localization-rank-one`, and is the same idiom as `DistributionLocal.le_of_sq_bound`. -/
...
The hypotheses are those of `FactorDecomp.holder_stability` (chunk (II) of
`lem:localization-rank-one`, plus chunk (III) as `hT`) together with the two
competitiveness inputs of `lem:localization-rank-one`:

* `hloc : ‖W - r_*‖₄⁴ ≤ Kh⁴` — `eq:graphon-quartic-localization`;
* `hdef : (∫W^d)^{m/d} - t(H,W) ≤ Kh⁴` — the Hölder deficit, which is `O(h⁴)` because
  `t(H,W) ≥ r_h^m` and the sharp `∫W^d` bound;

and the smallness threshold `hthr`, which is what "after decreasing `h₀`" means: it says the
cubic term of the combined stability inequality is dominated by half the coercive term.  The
...
exact expansion `eq:rank-one-reduction-bounds`) and the sharp `∫W^d` bound in the form
...
/-- **The Factor clause of `lem:localization-rank-one`**, in the paper's shape.
...
The threshold `h₀ = c/(2(C₃(C₂K)^{1/4} + 1))` is exactly the "after decreasing `h₀`, the
cubic term is absorbed" of the proof; `c = stabCoer d v q₀` is the coercivity constant of
the combined stability inequality (`FactorDecomp.holder_stability`). -/
