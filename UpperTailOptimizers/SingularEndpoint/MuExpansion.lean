...
/-!
# The multiplier expansion `μ_h` (Section 7, `paper/singular_endpoint.tex`)

The last line of `eq:rank-one-parameter-expansions` (in `lem:rank-one-parameter-expansions`) is

  `μ_h = \frac{1}{m r_*^m}\left[1 + \frac{d²}{3}
  \left(2 + \frac{(v-2)(4d-1)}{d-1}\right)h² + O_{d,m}(h⁴)\right]`,  `μ_h := \frac{γ_h}{m q_h^{v-2}}`,

with `v = 2m/d`.  This file proves it, and completes `eq:rank-one-parameter-expansions`: the
six other lines are `Ucoeff_eq`, `Gcoeff_eq`, `Lcoeff_eq` (`SingularEndpoint/Expansions.lean`),
`tendsto_alph_slope` (`SingularEndpoint/Order5.lean`) and `tendsto_qVal_coeff`, `tendsto_rVal_coeff`
(`SingularEndpoint/EdgeGap.lean`).
...
* `tendsto_muVal_coeff` — the `μ` line of `eq:rank-one-parameter-expansions`;
...
/-- **`eq:rank-one-parameter-expansions`, the value of the `μ` line at `h = 0`**:
`μ_0 = 1/(m r_*^m)`.
...
/-- **`eq:rank-one-parameter-expansions`, the `μ` line**:
...
/-- **`eq:rank-one-parameter-expansions`, the `μ` line as the paper displays it**:
