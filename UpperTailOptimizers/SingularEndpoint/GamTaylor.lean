...
/-!
# Quintic singular endpoint Taylor data (Section 7, `paper/singular_endpoint.tex`)

`SingularEndpoint/Expansions.lean` reads the `h²` coefficients `U₂`, `Λ₂`, `γ₂` of
`eq:rank-one-parameter-expansions` off the three KKT equations divided by `h²`, `h³`, `h⁴`.  The
...
`h⁵` (`FORMALISATION.md`, *The coefficients*).  This file supplies the two pieces of Taylor data
...
* `GamTail`, `Gam_expand`, `GamTail_rStar`, `continuousAt_GamTail`, `tendsto_GamTail_comp` —
  ...
  block at the top of `SingularEndpoint/Expansions.lean`: `Γ_d' = 𝓛_*`, so the vanishing list runs
...
`eq:endpoint-gap-expansion` — so the expansion of `SingularEndpoint/TaylorTail.lean` starts
