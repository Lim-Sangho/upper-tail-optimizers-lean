...
/-!
# The singular endpoint cost comparison (Section 5, `paper/sections/singular.tex`)
...
* `RdInt_sq_le_gamInt` — Cauchy–Schwarz against `R_d² ≤ C_dΓ_d`, giving
  `𝒱_R² ≤ ∫R_d(W)² ≤ C_d 𝒢_Γ` in the Lean notation `𝒱_R = ∫R_d(W)`, `𝒢_Γ = ∫Γ_d(W)`; the
  paper's counterpart is the pointwise bound `z^d - r_*^d - d r_*^{d-1}(z - r_*) ≤ C_dΓ_d(z)^{1/2}`
  followed by `∫Γ_d(W)^{1/2} ≤ (∫Γ_d(W))^{1/2}` in the proof of `lem:localization-rank-one`;
...
What is *not* here is the `O(h⁴)` input: bounding `𝒢_{Γ,h} = ∫Γ_d(W_h)`,
`𝒱_{R,h} = ∫R_d(W_h)` and `Λ_h` along the
family needs the expansions `eq:rank-one-parameter-expansions`, and is done in
`SingularEndpoint/LocalizationRankOne/FamilyLocalization.lean` (`family_localization`) on top of the coercive step of
`SingularEndpoint/LocalizationRankOne/L4Bound.lean`.  The identities below are unconditional and hold for every graphon.
...
* `sq_integral_le_integral_sq` — Cauchy–Schwarz `(∫f)² ≤ ∫f²` on `gμ`, shared with
  `SingularEndpoint/LocalizationRankOne/FamilyLocalization.lean` and `SingularEndpoint/LocalizationRankOne/FactorTail.lean`.
-/
...
/-- The graphon integral `𝒱_R = ∫R_d(W)` (Lean notation; `sec:localization-rank-one` integrates
the corresponding pointwise bound without naming this integral). -/
...
/-- **`eq:graphon-cost-decomposition` with the edge-density term.**  For any two graphons and any
`p ∈ (0,1)`, with `Λ = ℓ(p) - ℓ(p_*)`,

`I_p(W) - I_p(W') = (∫Γ_d(W) - ∫Γ_d(W')) + β_d(∫W^d - ∫W'^d) + Λ(e(W) - e(W'))`. -/
theorem Ip_sub_Ip_eq_edgeDensity (hd : 2 ≤ d) {p : ℝ} (hp0 : 0 < p) (hp1 : p < 1)
    (W W' : Graphon) :
    W.Ip p - W'.Ip p
...
/-- **`R_d² ≤ C_dΓ_d` integrated**, `𝒱_R² ≤ ∫R_d(W)² ≤ C_d 𝒢_Γ`: the Lean form of the step in
the proof of `lem:localization-rank-one` that applies the pointwise bound
`z^d - r_*^d - d r_*^{d-1}(z - r_*) ≤ C_dΓ_d(z)^{1/2}` to `W` and then uses
`∫Γ_d(W)^{1/2} ≤ (∫Γ_d(W))^{1/2}`. -/
