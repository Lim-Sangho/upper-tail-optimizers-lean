...
/-!
# `lem:localization-rank-one`, assembled (Section 5)

This file closes the last gap of `lem:localization-rank-one` of `paper/sections/singular.tex`
and assembles the four chunks into one statement.

## The gap

`SingularEndpoint/LocalizationRankOne/FactorSolution.lean` proves the pointwise bound, the orthogonality relation and the
...
* the homomorphism-density bound of `eq:rank-one-reduction-bounds`, and a stability
  inequality combining it with the moment expansion (which the paper does not display) —
  quoted from
  `SingularEndpoint/LocalizationRankOne/FactorStability.lean` and `SingularEndpoint/LocalizationRankOne/FactorHDensity.lean` in `localization_rankOne`, whose
...
(`SingularEndpoint/LocalizationRankOne/LocalizationMain.lean`).  The paper asserts existence only; `factorMain_unique`
...
`SingularEndpoint/LocalizationRankOne/FactorFix.lean` *and* for `A(d)·ε ≤ 1`. -/
...
`SingularEndpoint/LocalizationRankOne/FactorSolution.lean` at the constant `b = 1`.  Only measurability and the bounds
...
exactly the hypothesis `hqlow` that seven statements of `SingularEndpoint/LocalizationRankOne/FactorSolution.lean` carry.
...
`SingularEndpoint/LocalizationRankOne/FactorSolution.lean` with `hqlow` discharged. -/
...
The stability bound — `eq:moment-and-cost-gap-bounds` in the combined shape — is stated in `SingularEndpoint/LocalizationRankOne/FactorStability.lean` with the
...
combined stability bound of `SingularEndpoint/LocalizationRankOne/FactorStability.lean` consumes it. -/
...
* `q > 0` and `q ≥ 2^{-d}`;
* `f ≤ q^{-1/d}` pointwise (the bound behind the pointwise clause `0 ≤ f ≤ 2` of
  `eq:rank-one-orthogonality`);
* `‖f - u_*‖₄ + ‖E‖₄ ≤ C(d)·ε` `eq:rank-one-reduction-bounds`;
* `|t(H,W) - q^v| ≤ 40^{|E(H)|}‖E‖₂³` `eq:rank-one-reduction-bounds`;
* `c‖E‖₂² - C‖W - r_*‖₄⁴ - 40^{|E(H)|}‖E‖₂³ ≤ (∫W^d)^{m/d} - t(H,W)` — a stability
  inequality combining the moment expansion with the homomorphism-density bound of
  `eq:rank-one-reduction-bounds`, which the paper does not display — with
...
`lem:localization-rank-one` asserts existence only, so this clause goes beyond the paper.  What `SingularEndpoint/LocalizationRankOne/FactorFix.lean` gives is
uniqueness of
...
The bounds `0 ≤ f ≤ M`, `q ≥ 2^{-d}` play the role of a fixed `L⁴`-neighbourhood of `u_*` —
by the Lean closeness bound (`eq:rank-one-reduction-bounds`) any such solution *is* within
