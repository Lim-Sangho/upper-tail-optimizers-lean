...
/-!
# `lem:localization-rank-one` along the family (Section 5, `paper/sections/singular.tex`)

`SingularEndpoint/LocalizationRankOne/L4Bound.lean` proves the coercive step of `lem:localization-rank-one` for an
...
with the Lean notation `𝒢_{Γ,h} = ∫Γ_d(W_h)` and `𝒱_{R,h} = ∫R_d(W_h)`.

What was missing there was the quantitative `O(h⁴)` input: the bounds `𝒢_{Γ,h} = O(h⁴)`,
`Λ_h = O(h²)` and `𝒱_{R,h} = O(h²)` hold *along the singular endpoint family*, and need the
expansions `eq:rank-one-parameter-expansions` (the paper records the first two, as
`∫Γ_d(W_h) = O_d(h⁴)` and `Λ_h = O_d(h²)`, in the proof of `lem:localization-rank-one`).  Those
are now available, so this file performs the instantiation and states
`eq:graphon-quartic-localization` in the form
...
The three rates fed in are the following family estimates:

* `tendsto_gamInt_graphon` (`SingularEndpoint/ConstantGraphonComparison/GamAverage.lean`): `∫Γ_d(W_h)/h⁴ → d³/3`, so
  `𝒢_{Γ,h} = O(h⁴)`;
* `tendsto_ell_coeff` (`SingularEndpoint/ConstantGraphonComparison/FamilyQuadratic.lean`): `Λ_h/h² → L₂` and `Λ_h → 0`, so
...
/-- **`eq:graphon-quartic-localization` along the singular endpoint family**, raised to the
fourth power.
...
/-- **An `L²` companion of `eq:graphon-quartic-localization` along the singular endpoint
family** (the paper states only the `L⁴` bound): `∫|W - r_*|² ≤ M h²` under the same
hypotheses as `family_localization`.
