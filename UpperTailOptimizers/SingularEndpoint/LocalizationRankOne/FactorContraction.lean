...
/-!
# The normalised contraction map of the nonlinear Factor construction (Section 5)
...
nonlinear Factor equation `T_W(f^{d-1}) = q f` of `eq:rank-one-orthogonality`, together with
the closeness bounds `eq:rank-one-reduction-bounds` (existence only; the Lean construction also
proves a uniqueness statement).
...
to a.e. equality) and the normalisation `q = ∫ f^d` are `SingularEndpoint/LocalizationRankOne/FactorFix.lean`; the
orthogonality half of `eq:rank-one-orthogonality` and the closeness bounds of
`eq:rank-one-reduction-bounds`
are `SingularEndpoint/LocalizationRankOne/FactorSolution.lean`; the combined stability inequality is
`SingularEndpoint/LocalizationRankOne/FactorStability.lean`; and the assembly with the Lean's uniqueness statement is
`SingularEndpoint/LocalizationRankOne/FactorMain.lean` (`exists_factorDecomp`, `localization_rankOne`, `factorMain_unique`).
...
Raw functions and `MeasureTheory.integral` throughout, as in `SingularEndpoint/LocalizationRankOne/KernelOp.lean`; the
...
Four estimates for `Lnorm` that `SingularEndpoint/LocalizationRankOne/KernelOp.lean` does not provide: a uniform bound, a
