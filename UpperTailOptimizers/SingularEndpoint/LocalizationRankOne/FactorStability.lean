...
/-!
# Local Hölder stability at the singular endpoint (Section 5)

This file formalises chunk **(II)** of the Lean proof of `lem:localization-rank-one`
(`paper/sections/singular_localization.tex`) — a coercive lower bound for `∫W^d - q²`
(`Wmoment_sub_qsq_ge`) and, combined with the clause `|t(H,W) - q(f)^v| ≤ C_{d,m}‖E‖₂³` of
`eq:rank-one-reduction-bounds`, the stability inequality

`(∫ W^d)^{m/d} - t(H,W) ≥ c‖E‖₂² - C‖W - r_*‖₄⁴ - C‖E‖₂³`,

on top of the algebraic layer of `SingularEndpoint/LocalizationRankOne/Factor.lean`.  Neither estimate is in the
paper: its proof of `lem:localization-rank-one` only bounds the binomial expansion of `∫W^d`
from above, `|∫W^d - q(f)²| ≤ C_d‖E‖₂²`, and obtains `‖E‖₂ ≤ C_dh` and `|q(f) - q_h| ≤ C_dh²`.
The Lean uses the stability inequality to reach the sharper `‖E‖₂ = O(h²)` and
`|q(f) - q_h| = O(h⁴)` (`SingularEndpoint/LocalizationRankOne/FactorTail.lean`, `SingularEndpoint/LocalizationRankOne/LdSharp.lean`).
The algebraic layer supplies the exact
binomial expansion of `∫W^d` (`FactorDecomp.Wmoment_eq`) together with the
...
* **Young absorption** of the cubic-and-higher tail of the expansion (`tail_absorption`,
  `abs_tail_le`), giving the coercive lower bound `Wmoment_sub_qsq_ge`;
* the **tangent-line** step at the base point `q²` (Bernoulli's inequality for the exponent
  `m/d = v/2`), which produces no `O(ε⁴)` remainder at all.

## What is assumed, and where it comes from

`Nonempty (FactorDecomp d W)` is vacuous (`f = 0` satisfies every field), so every closeness
statement that the argument needs (stated in terms of `ε = ‖W - r_*‖₄`, not of `h` as in
`lem:localization-rank-one`) is carried as an
*explicit hypothesis*, in the repo's integral vocabulary rather than as Mathlib `Lp` norms:

* `hA4 : P.rankOneQuart ≤ C₁ * defectQuart d W` — this is `‖A - r_*‖₄ ≤ Cε`, `ε = ‖W - r_*‖₄`,
  raised to the fourth power (the paper's proof of `lem:localization-rank-one` bounds
  `‖f ⊗ f - r_*‖₄` and `‖E‖₄` in terms of `h` instead);
* `hE4 : P.residQuart ≤ C₂ * defectQuart d W` — likewise `‖E‖₄ ≤ Cε`;
* `hq : q₀ ≤ P.qVal` with `0 < q₀` — a uniform lower bound on `q`, as in the paper's
  `q(f) ∈ [(r_*/2)^{d/2}, 1]` from the construction of the decomposition;
* `hT : |T - P.qVal ^ v| ≤ C₃ * P.residNorm ^ 3` — this is exactly chunk **(III)**, the clause
  `|t(H,W) - q(f)^v| ≤ C_{d,m}‖E‖₂³` of `eq:rank-one-reduction-bounds`, which is *not* proved here: `T` stands for
  `t(H,W)` and the hypothesis is `t(H,W) = q^v + 𝓡_H` with `|𝓡_H| ≤ C‖E‖₂³`.  It is proved in
  `SingularEndpoint/LocalizationRankOne/FactorHDensity.lean`.

The existence of the decomposition — chunk **(I)**, the `L^p` problem — is likewise not here;
it is the contraction construction of `SingularEndpoint/LocalizationRankOne/FactorContraction.lean`, `SingularEndpoint/LocalizationRankOne/FactorFix.lean` and
`SingularEndpoint/LocalizationRankOne/FactorSolution.lean`, delivered as `exists_factorDecomp`
(`SingularEndpoint/LocalizationRankOne/FactorMain.lean`), where all three chunks are joined into `localization_rankOne`.
...
* `FactorDecomp.Wmoment_sub_qsq_ge` — the coercive lower bound for `∫W^d - q²`;
...
defect `(A - 2θ)⁴` already exceeds `θ⁴` and pays for the whole of `θ^e E² ≤ 25`.  This is a
pointwise substitute for a measure-theoretic Chebyshev split into `{A ≥ θ}` and its
complement; the proof of `lem:localization-rank-one` has no such coercivity step (see
`coercive_quadratic`). -/
...
identity `4α(α u² + (K²/4α) u⁴ - K u³) = (2αu - K u²)²`.  It is the Young step that trades
the cubic tail for a fraction of the coercive term plus a quartic remainder
(`tail_absorption`), in a pointwise form that needs neither a square root nor Cauchy–Schwarz. -/
...
inequality for real exponents applied to `1 + s = X/q²`, used in place of a two-sided Taylor
expansion: because
...
the coercive bound `Wmoment_sub_qsq_ge`. -/
...
set `A ≥ r_*/2` of `coercive_quadratic`. -/
...
cubic-and-higher part of the binomial expansion of `∫W^d`. -/
...
/-- The full `ε⁴`-constant of `Wmoment_sub_qsq_ge`, in terms of the two closeness
constants `C₁` (for `‖A - r_*‖₄⁴`) and `C₂` (for `‖E‖₄⁴`) of the hypotheses `hA4`,
`hE4`. -/
...
/-- The `ε⁴`-constant of `Wmoment_sub_qsq_ge` is nonnegative. -/
...
/-- `‖E‖₂`, the quantity in which the stability inequality is stated. -/
...
/-- **Coercivity of the quadratic term** `∫∫A^{d-2}E²` (`A = f ⊗ f`) of the binomial expansion
of `∫W^d = ∫(f ⊗ f + E)^d`:

`∫∫ A^{d-2} E² ≥ (r_*/2)^{d-2} ‖E‖₂² - (400/r_*⁴) ‖A - r_*‖₄⁴`.

This lower bound has no counterpart in the paper: the proof of `lem:localization-rank-one`
only bounds the same expansion from above, `|∫W^d - q(f)²| ≤ C_d‖E‖₂²`.  The argument is a
Chebyshev-type split into the set `A ≥ r_*/2`, where the quadratic coefficient is at least
`(r_*/2)^{d-2}`, and its complement, carried out
...
/-- **The tail of the binomial expansion of `∫W^d` is `O(∫|E|³)`.**  Every term with `k < d-2` has
...
/-! ## The coercive lower bound for `∫W^d - q²` -/

/-- **The coercive lower bound for `∫W^d - q²`** (not in the paper; see the module docstring):

`∫ W^d - q² ≥ (3/4)(r_*/2)^{d-2}‖E‖₂² - binom(d,2)(400/r_*⁴)‖A - r_*‖₄⁴
              - tailQuartConst · ‖E‖₄⁴`.

The exact binomial expansion (`FactorDecomp.Wmoment_eq`) is peeled at `k = d-2`; the leading piece is
...
explicit constants: `Wmoment_sub_qsq_ge` fed through the tangent line at `q²` and joined
with the density-error hypothesis.

`T` stands for the homomorphism density `t(H,W)`; the hypothesis `hT` is precisely chunk
(III), the last clause of `eq:rank-one-reduction-bounds`, which is *not* proved here.  The graph `H`
enters only through `v = |V(H)|`, `m = |E(H)|` and the `d`-regular handshake `2m = vd`
(`hvm`).  The hypotheses `hA4`, `hE4` and `hq` are the closeness and lower-bound hypotheses
...
/-- **The combined stability inequality in `∃ c, C > 0` shape** (`lem:localization-rank-one`
states no such inequality; see the module docstring):
...
(III), the last clause of `eq:rank-one-reduction-bounds`, which is not proved here but in
`SingularEndpoint/LocalizationRankOne/FactorHDensity.lean`. -/
