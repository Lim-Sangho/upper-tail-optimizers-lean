...
/-!
# The rank-one splitting of the `H`-density (Section 5, `paper/sections/singular.tex`)
...
The input is the structure `FactorDecomp d W` of `SingularEndpoint/LocalizationRankOne/Factor.lean`, which transcribes
...
* `T = ∅` gives the rank-one term `∫ ∏_e A_e = (∫f^d)^{|V(H)|} = q^v`, because a rank-one
  ...
  (`prod_edges_eq_prod_pow_degree` of `SingularEndpoint/RankOneStationaryFamily/RankOne.lean`) and `H` is `d`-regular.
...
* `FactorDecomp.residL2`, `rowNorm` — `‖E‖₂` (as `√residSq`, `residSq` itself being
  `SingularEndpoint/LocalizationRankOne/FactorStability.lean`'s) and the row norms;
...
This is `prod_edges_eq_prod_pow_degree` of `SingularEndpoint/RankOneStationaryFamily/RankOne.lean` with `H.edgeFinset`
...
/-- `residL2` is `SingularEndpoint/LocalizationRankOne/FactorStability.lean`'s `residNorm` under another name; the two files
