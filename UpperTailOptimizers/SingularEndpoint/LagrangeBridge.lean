...
/-!
# The graph-carrying Lagrangian (Section 7, `paper/singular_endpoint.tex`)

The `SingularEndpoint` layer is deliberately graph-free: `lem:graphon-lagrangian-bound` and everything
under it subtract `η_hΔ_h(ν)`, where the paper writes `μ_h{t(H,W) - r_h^m}`.  The paper
...
/-- **The graph-carrying Lagrangian** (`paper/singular_endpoint.tex` together with
`eq:rank-one-reduction-bounds`).

`μ_h{t(H,W) - r_h^m} ≤ η_hΔ + |μ_h|{n²2^{2dn}Δ² + 40^m‖E‖₂³}`.

The `SingularEndpoint` layer subtracts the graph-free `η_hΔ_h(ν)`; the comparison route to
`thm:endpoint-optimality` wants `μ_h{t(H,W) - r_h^m}`.  (`μ_h` itself belongs to
`lem:rank-one-kkt-family`; the theorem's own statement no longer mentions it.)
