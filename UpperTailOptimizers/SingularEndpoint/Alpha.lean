...
/-!
# The block weight `α_h` of the coalescing family (Section 7, `paper/singular_endpoint.tex`)
...
`h = 0` — the expansions `eq:rank-one-parameter-expansions` give
...
`KKTFamily` (`SingularEndpoint/Family.lean`).

**The two difficulties, and how they are met.**

*The `0/0`.*  `exists_analytic_factor_of_tendsto` (`SingularEndpoint/OrderFactor.lean`) turns each of
the two one-sided asymptotics of `SingularEndpoint/Order4.lean` and `SingularEndpoint/Order4Right.lean` into
...
`SingularEndpoint/Order4.lean`, `SingularEndpoint/Order4Right.lean` and `SingularEndpoint/MfunAnalytic.lean`
transports verbatim. -/
...
analyticity lemmas of `SingularEndpoint/MfunAnalytic.lean` require. -/
...
desingularized system of `SingularEndpoint/FamilySystem.lean` divides by `h` and `h²`.  At `h = 0`
the three nodes coalesce at `u_*² = r_*` and the single equation that remains is the
base-point contact identity `Lstar_rStar` of `SingularEndpoint/Contact.lean`. -/
...
(`SingularEndpoint/Family.lean`): a family `h ↦ (u_h, γ_h, p_h, α_h)`, analytic on `|h| < h₀` and
...
* `0 < α_h < 1` comes from `rowBalance_alpha_mem` (`SingularEndpoint/RowSign.lean`) for `h > 0`,
