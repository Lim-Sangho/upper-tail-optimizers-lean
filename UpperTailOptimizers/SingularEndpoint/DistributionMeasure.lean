...
/-!
# The measure half of the tail-uniform two-point interpolation (Section 7)

`lem:auxiliary-lagrangian-bound` pairs
the two-node interpolation error of `SingularEndpoint/TailScalar.lean` against a probability measure
...
inequality `δ(u)² ≤ |Q_h(u)|` — is `SingularEndpoint/TailScalar.lean`.  **This file is the measure
half**, and it ends at `KKTFamily.exists_tail_interpolation`, which realises
...
sections set up vocabulary that `lem:auxiliary-lagrangian-bound` will reuse: the central window
...
The paper writes `σ := ν - ν_h` and integrates against `σ`.  Every occurrence is of the form
"integral of `f` against `σ`", so this file writes it throughout as "integral against `ν`
...
The three displays of the paper's proof are the three `have`s inside
`exists_tail_interpolation`.
...
* **The `g` term .**  `g` vanishes at both atoms (`gfun_sVal`, `gfun_tVal`), so
  ...
  the window.  The paper's Hölder chain
...
* **The mass term .**  `ν(𝓝_ρ) = 1 - ε_ρ`, from `ν([0,2]) = 1` and additivity across
  `𝓝_ρ`.
...
* `KKTFamily.exists_atoms_mem_centralWindow` — both atoms lie in the window once `h` is
  small;
* `KKTFamily.exists_tail_interpolation` — **`lem:auxiliary-lagrangian-bound`**;
* `KKTFamily.continuousOn_JpTildeH`, `.continuousOn_distributionIntegrand` — continuity of
  ...
  `SingularEndpoint/DistributionGap.lean` and `SingularEndpoint/DistributionQuant.lean`.
-/
...
/-- **The short circuit of the paper's Hölder chain.**  If `0 ≤ a` and `a² ≤ |b|`, then
`a^{3/4} ≤ (b²)^{3/16}`.

The paper descends `∫δ^{3/4} ≤ (∫δ²)^{3/8} ≤ (∫|Q_h|)^{3/8} ≤ A_ρ^{3/16}` in
...
/-- **`lem:auxiliary-lagrangian-bound`.**  For every window
...
The paper's `σ = ν - ν_h` is written out as a difference of two integrals; see the module
docstring.  The proof combines the three displays of `paper/singular_endpoint.tex` through
