...
/-!
# Derivatives of the rank-one KKT function (Section 7, `paper/singular_endpoint.tex`)

`SingularEndpoint/Contact.lean` differentiates `𝓛_*` — the rank-one KKT function
...
the unlabelled display at `paper/singular_endpoint.tex`, which is `Jp'_eq_ell_sub`), so
`F_{p,γ}' = J_p'' - γ(d-1)z^{d-2}` no longer mentions `p`.  Accordingly `Fkkt1`, `Fkkt2` and
`Fkkt3` carry only `γ`, and the `Lstar`-level statements of `SingularEndpoint/Contact.lean` are
...
* `Lstar3_eq` — agreement with `SingularEndpoint/Contact.lean`;
...
/-- The first derivative, in `Fkkt1` notation.  `SingularEndpoint/Bipodality.lean` already proves
...
/-! ## Agreement with the derivatives at the singular endpoint of `SingularEndpoint/Contact.lean` -/
