...
/-!
# Symmetry of the coalescing scalar family (Section 7, `paper/singular_endpoint.tex`)

`SingularEndpoint/FamilyExists.lean` produces the analytic family `h ↦ (u_h, ℓ_h, γ_h)` of
`lem:rank-one-kkt-family` from the analytic implicit function theorem, but says
nothing about how it behaves under `h ↦ -h`.  The paper's argument for the symmetries
`u_{-h} = u_h`, `ℓ_{-h} = ℓ_h`, `γ_{-h} = γ_h` is: the desingularized system is *even* in
`h`, so `h ↦ z_{-h}` is a second solution family through the same base point, and local
...
`SingularEndpoint/FamilySystem.lean`, which is legitimate for `h ≠ 0`; at `h = 0` there is nothing
...
`lem:rank-one-kkt-family` from `ℓ_h` through `pOf` of `SingularEndpoint/LogOdds.lean`; it
...
are `SingularEndpoint/Alpha.lean`, `SingularEndpoint/FamilyUnique.lean` and `SingularEndpoint/Expansions.lean`, and
`SingularEndpoint/FamilyBuild.lean` assembles them all into `exists_kktFamily`.
...
`SingularEndpoint/LogOdds.lean` adjoined.  It is analytic because `pOf` is analytic everywhere, it
