...
The point of proving it here is that it replaces an appeal to analyticity.  To reach
`eq:rank-one-parameter-expansions` the paper divides an analytic function by the monic cubic
with roots `s²`, `st`, `t²` (the analytic Weierstrass division behind
`eq:mh-derivative-factorization`) in order to see that a certain
combination vanishes to order four.  The lemma below gives the same factorisation from
...
* `exists_deriv_zero_of_two_roots`, `exists_deriv3_zero_of_four_roots` — the packaged Rolle
  steps the remainder is built from.  `SingularEndpoint/Interp2.lean` and `SingularEndpoint/Interp4.lean`
...
Section 7; it is stated for a bare triple of derivatives on a compact interval.
-/
