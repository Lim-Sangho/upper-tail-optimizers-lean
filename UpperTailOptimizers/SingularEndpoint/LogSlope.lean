...
/-!
# The analytic log slope (Section 7, `paper/singular_endpoint.tex`)
...
roots `s_h², s_h t_h, t_h²` merge, so they must be replaced by their divided differences —
the paper's "analytic Weierstrass division".  Every division that arises is by a power of
