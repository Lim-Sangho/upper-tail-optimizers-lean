...
The uniqueness clause of `thm:endpoint-optimality` asserts that a minimiser agrees with the
...
`thm:endpoint-optimality` wants its conclusion: a *single* measure-preserving `σ` of `[0,1]`
