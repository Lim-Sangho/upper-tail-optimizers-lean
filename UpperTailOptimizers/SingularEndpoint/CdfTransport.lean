...
`thm:endpoint-optimality`'s uniqueness clause needs a measure-preserving self-map of `[0,1]`
...
`volume|_{[0,|S|]}`.  Nothing in this file is specific to Section 7; it is general measure
...
This is the measure-theoretic content of `thm:endpoint-optimality`'s uniqueness clause: it
...
/-- **The relabelling map** of `thm:endpoint-optimality`'s uniqueness clause. -/
