...
/-!
# Terminal optimality at the singular endpoint (Section 7, `paper/singular_endpoint.tex`)

The closing subsection `sec:endpoint-optimality-proof` of `paper/singular_endpoint.tex` combines the
localization lemma with the full graphon calibration estimate `eq:graphon-lagrangian-bound`
to prove `thm:endpoint-optimality`.  Its last three steps are *pure arithmetic* once the
calibration estimate is available, and that is what this file proves:
...
* `singular_endpoint_endgame` — the scalar endgame.  If a competitor's cost gap `g` obeys the
  ...
  slack `c ≥ κΔ` linear in the moment displacement (the linear constraint control),
...
* `graphon_not_ae_const` — `W_h` is nonconstant for `h ≠ 0`, the nonconstancy clause of
  `thm:endpoint-optimality`.  Proved outright: the `d`-th moment `q_h²` exceeds
  `e(W_h)^d` by strict Jensen, whereas an a.e.-constant graphon has `∫W^d = e(W)^d`.

The hypothesis `hmaster` carried here is discharged in `SingularEndpoint/GraphonComparisonMaster.lean`
(`exists_comparison_master`, `exists_singular_endpoint_comparison_graph`). In
`SingularEndpoint/TerminalUnique.lean`, `exists_singular_endpoint_full` adds feasibility and
uniqueness for a given family. `singular_endpoint_full` constructs that family and collects
the other proved clauses of Theorem 7.1, with the documented qualifications.
...
/-- **`sec:endpoint-optimality-proof`, the closing display.**

`g` is the cost gap `I_{p_h}(W) - I_{p_h}(W_h)`, `c` the constraint slack
`t(H,W) - r_h^m`, `R` the residual `‖E‖₂² + ∫Q_h² dν + ν(T_ρ)` and `Δ` the moment
displacement `∫f^d - q_h`.  The master estimate `eq:graphon-lagrangian-bound` is `hmaster`,
the linearised constraint the linear constraint control is `hslack`, and `hsmall`
is the paper's "the negative quadratic term is absorbed by the positive constraint term".
...
/-- **`W_h` is nonconstant for `h ≠ 0`** — the nonconstancy clause of `thm:endpoint-optimality`.
...
/-- **`sec:endpoint-optimality-proof`, steps 5–7.**  Take the calibration estimate
`eq:graphon-lagrangian-bound` and the linearised constraint
the linear constraint control as given, for a fixed `h`, with a residual functional
`R` (the paper's `‖E‖₂² + ∫Q_h²dν + ν(T_ρ)`) and a moment displacement `Δ`.  Then `W_h` is
optimal among feasible graphons, and any competitor attaining its cost has vanishing
residual and moment displacement — which is what the paper turns into "`f` takes only the
values `s_h, t_h`, with weights `α_h`". -/
