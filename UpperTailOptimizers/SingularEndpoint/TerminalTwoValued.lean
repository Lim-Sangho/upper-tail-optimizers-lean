...
/-!
# The rigidity extracted from a vanishing residual (Section 7, `paper/singular_endpoint.tex`)

This is the rigidity step of `sec:endpoint-optimality-proof`, the one that feeds the uniqueness
clause of `thm:endpoint-optimality`: the paper's "`E = 0` and `Q_h = 0` `ν`-a.e., so
`ν = βδ_{s_h} + (1-β)δ_{t_h}`", read on `[0,1]` rather than on the law interval.
