...
/-!
# The singular endpoint scalar identities (Section 7, `paper/singular_endpoint.tex`)
...
* `measurable_Gam` — measurability of `Γ_d`, shared with `SingularEndpoint/GraphonComparison.lean`;
...
* `Ip_sub_Jp_eq` — the cost gap of an arbitrary graphon against `J_p(r)`, from which
  `eq:constant-comparison-identity` is specialised in `SingularEndpoint/CostGap.lean`.
-/
