...
/-!
# The sharp moment display of `lem:localization-rank-one` (Section 7)

`SingularEndpoint/FamilyLocalization.lean` instantiates the coercive step `gamInt_localized` along the
singular endpoint family for the `L⁴` half of `eq:graphon-quartic-localization` only: it drops the
...
`𝒢_Γ/2 ≥ 0`, which is the *second* display of the lemma,

`0 ≤ ∫W^d - r_h^d ≤ Mh⁴`   (the sharp `∫W^d` bound),

and then draws the two consequences the lemma needs downstream.
...
* `family_ld_sharp` — **the sharp `∫W^d` bound** along the family.  The lower bound is
  ...
  at `b₀ = β_d/2` with the same three rates of `eq:rank-one-parameter-expansions` that
  `family_localization` uses (`tendsto_gamInt_graphon`, `tendsto_ell_coeff`, and
  `𝒱_{R,h} = O(h²)` through `edge_eq_moment_sub_rdInt`).
* `family_holder_deficit` — the **Hölder deficit** `(∫W^d)^{m/d} - t(H,W) ≤ Kh⁴`, from
  feasibility `t(H,W) ≥ r_h^m` and the display above.
* `family_factor_clause` — the **Factor clause** of `lem:localization-rank-one` along
  the family: the decomposition produced by `localization_rankOne` satisfies `‖E‖₂ ≤ Mh²` and
...
this completes `lem:localization-rank-one` along the family.
...
for a convex increment.  The paper's proof controls the derivative there by noting that `r_h^d`
stays bounded away from `0`; that is not needed, because the base point `B = ∫W^d` of a graphon
is at most `1`, so `B^{a-1} ≤ 1` and the derivative factor is bounded by `a` outright.

## Paper results realised

* the sharp `∫W^d` bound — `family_ld_sharp`;
* `eq:generalized-holder` at the family, i.e. the deficit bound played against the coercivity
  `eq:moment-and-cost-gap-bounds` in the proof of `lem:localization-rank-one` —
  `family_holder_deficit`;
* the Factor clause of `lem:localization-rank-one` — `family_factor_clause`.
...
/-- **The `O(h⁴)` input of `eq:rank-one-parameter-expansions`, packaged once.**
...
* the whole right-hand side of `gamInt_localized` at the candidate `W_h`,
  `𝒢_{Γ,h} + |Λ_h||𝒱_{R,h}| + C_dΛ_h²/2`, is at most `Nh⁴`.

The three rates fed in are `tendsto_gamInt_graphon` (`𝒢_{Γ,h}/h⁴ → d³/3`),
`tendsto_ell_coeff` (`Λ_h → 0` and `Λ_h/h² → ℓ₂`) and, through `edge_eq_moment_sub_rdInt` at
...
/-- **The Factor clause of `lem:localization-rank-one` along the singular endpoint family.**
