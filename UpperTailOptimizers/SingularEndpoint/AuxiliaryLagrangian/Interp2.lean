...
/-!
# The two-node interpolation remainder (Section 5, `paper/sections/singular.tex`)

The arity-2 companion of `SingularEndpoint/RankOneStationaryFamily/Interp3.lean`: the same textbook Rolle argument with
...
everywhere-defined `dd2 f a b = dslope (dslope f a) b`, built the way `SingularEndpoint/RankOneStationaryFamily/Quotient.lean`
builds `quotient3` — is therefore trapped between `m/2` and `M/2` as soon as `m ≤ f'' ≤ M`.

## Why Section 5 wants it
...
an unlabelled display in that proof — to stay
...
divided-difference integral formula, exactly as `SingularEndpoint/RankOneStationaryFamily/Interp3.lean` and
`SingularEndpoint/AuxiliaryLagrangian/Interp4.lean` replace the two Weierstrass divisions.
...
Nothing in this file mentions the singular endpoint family, the graph `H`, or any object of Section 5;
...
This is `exists_deriv3_eq_of_three_roots` of `SingularEndpoint/RankOneStationaryFamily/Interp3.lean` with one node — and one
...
/-- **The second divided difference `f[a,b,·]`**, built the way `SingularEndpoint/RankOneStationaryFamily/Quotient.lean` builds
...
`analyticOnNhd_dslope` (`SingularEndpoint/RankOneStationaryFamily/Quotient.lean`). -/
