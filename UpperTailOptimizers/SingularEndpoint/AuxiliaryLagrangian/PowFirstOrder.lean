...
(Section 5, `paper/sections/singular_auxiliary_lagrangian.tex`)

The **first-order companion** of `SingularEndpoint/AuxiliaryLagrangian/PowInterp.lean`, and the first instalment of the
coefficient-bound clause in the proof of `lem:central-kernel-bound` reading

> The analogous lower-order divided-difference formulas and the uniform derivative bounds on
> `K̂_h` give a single `C_d ≥ 1` such that
> `max{|p_{ij,h}|, |u_{i,h}(x)|, |v_{i,h}(y)|} ≤ C_d`
> (`i,j ∈ {0,d}`, `0 < h < h_0`, `x,y ∈ 𝒩_{ρ_0}`).

The Lean bounds are stated on a closed window `𝓝_ρ` of prescribed radius, with constants
depending on that radius.

`SingularEndpoint/AuxiliaryLagrangian/PowInterp.lean` supplies the *second*-order theory of the interpolation
...
error estimate `sup_{0<h<h_ρ}‖Θ_h - d³/4‖_{L^∞(𝒩_ρ²)} ≤ a/4` in the proof of
...
* `exists_close_of_continuousAt` — the elementary window helper `|g h - L| ≤ 1` for small `h`,
  also used by `SingularEndpoint/AuxiliaryLagrangian/DistributionQuant.lean`.
-/
...
This is the first-order companion of `exists_powResid_bound` (`SingularEndpoint/AuxiliaryLagrangian/PowInterp.lean`); the
...
are `Jp''` and `Jp3` of `SingularEndpoint/RankOneStationaryFamily/Defs.lean` — so their bounds on a closed subinterval of
