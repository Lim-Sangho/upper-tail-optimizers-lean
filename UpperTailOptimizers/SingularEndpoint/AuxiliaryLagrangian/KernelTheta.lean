...
/-!
# The corner constant of `lem:central-kernel-bound` (Section 5, `paper/sections/singular_auxiliary_lagrangian.tex`)

The proof of `lem:central-kernel-bound` controls the sign of the last term of the interpolation
decomposition by evaluating the *coalescing*
...
* `powDopOf_powDopOf_kernel` — the specialisation at `F = Γ_d`, i.e. the displayed value
  ...
  `B.kernel 0` of `SingularEndpoint/AuxiliaryLagrangian/KernelInterp.lean` (`B.p 0 = p_*` by `p_zero`).
...
1. **`𝒟_d` annihilates the trivial part.**  In the paper's decomposition
   `K̂_0(w,z) = J_{p_*}(r_*) + β_d(wz - r_*^d) + Γ̃_d((wz)^{1/d})` (from
   `eq:entropy-continuation`) the first two terms are affine in each of `w`, `z`, so only the
   `Γ̃_d` term reaches `∂²_w∂²_z`; the paper leaves this implicit ("the chain rule leaves only
   the `Γ_d^{(4)}(r_*)` term").  `powDop_const` and `powDop_pow`: the
   constant and `β_d(xy)^d` of this decomposition of `K_0` contribute nothing, in each
...
The uniform corner-error bound `sup_{0<h<h_ρ}‖Θ_h - d³/4‖_{L^∞(𝒩_ρ²)} ≤ a/4` of the proof of
`lem:central-kernel-bound` is not here, and neither is the identification of the value computed here with the
coalescing limit of `KKTFamily.kernelTheta` (`SingularEndpoint/AuxiliaryLagrangian/KernelInterp.lean`), i.e. with the
paper's `Θ_0(u_*,u_*)`.  Both are `SingularEndpoint/AuxiliaryLagrangian/KernelError.lean`
(`KKTFamily.exists_kernelTheta_error`, with `thetaV_theta_value` transporting the
value proved here into the chart).  The paper obtains them from the jointly continuous
extension of `Θ_h` given by the integral representations of the divided differences ("By the
joint continuity of `Θ_h(x,y)` at `(h,x,y) = (0,u_*,u_*)`"); formalising that needs a derivative chain for
...
`powDop` (`SingularEndpoint/AuxiliaryLagrangian/PowInterp.lean`) takes the two derivatives of `F` as *data*, which is what
...
In the paper's coordinates the constant and `β_d(wz - r_*^d)` of
`K̂_0(w,z) = J_{p_*}(r_*) + β_d(wz - r_*^d) + Γ̃_d((wz)^{1/d})` are affine in each of `w`, `z`
and do not reach `∂²_w∂²_z`, which the paper leaves implicit.  In the original
variables, the constant and `β_d(xy)^d` of this decomposition of `K_0(x,y) = J_{p_*}(xy)`
...
`KKTFamily.kernelTheta`; that identification is `SingularEndpoint/AuxiliaryLagrangian/KernelError.lean`, see the
module docstring. -/
...
the object `SingularEndpoint/AuxiliaryLagrangian/KernelInterp.lean` decomposes:
