...
/-!
# The dual potential through the continued entropy (Section 5, `paper/sections/singular_auxiliary_lagrangian.tex`)

`lem:first-variation-bound` (`paper/sections/singular_auxiliary_lagrangian.tex`) states its two
lower bounds on `𝒩_ρ` and on the tail `𝒯_ρ = [0,2] ∖ 𝒩_ρ`, which together cover the law interval
`[0,2]`.  Its `Ψ_h` (`eq:first-variation`) is built from the kernel `K_h(x,y) = J̃_{p_h}(xy)` of
the **continued** entropy of `eq:entropy-continuation`, whereas `KKTFamily.Psi` (`SingularEndpoint/AuxiliaryLagrangian/FirstVariation.lean`)
...
of `2`.  This file introduces the tilde version and proves on all of `[0,2]` what
`SingularEndpoint/AuxiliaryLagrangian/FirstVariationBound.lean` had to leave out: the tail bound `Ψ_h ≥ b_ρ` off the
coalescence neighbourhood, and, as an auxiliary Lean fact not stated in the paper, the uniform
bound on `sup_{[0,2]}|Ψ_h|`.  The quartic
bound `Ψ_h ≥ (d³/24) Q_h²` near `u_*` is `KKTFamily.exists_firstVariation_lower`
(`SingularEndpoint/AuxiliaryLagrangian/FirstVariationBound.lean`), and `PsiT_eq_Psi` identifies the two potentials wherever both are
defined, so the two halves are bounds on one and the same function.

## The route

`J̃_{p_h}` is the displaced continuation of `eq:entropy-continuation`,
`J̃_{p_h}(z) = J̃_{p_*}(z) + Λ_h z + C_h`; the displacement is
...
(`PsiT_eq_Psi`).  At `h = 0` the wells coalesce and `PsiT_zero` is
`eq:endpoint-first-variation`, `Ψ_0(x) = 2Γ̃_d(u_* x)`, which is positive off `x = u_*` by
`GamTilde_pos_of_ne`.  A compactness argument on `𝒯_ρ = {x ∈ [0,2] : |x - u_*| ≥ ρ}` turns
that into a positive lower bound at `h = 0`, and the transfer to small `h > 0` is the
uniform convergence `Ψ_h → Ψ_0` on `[0,2]` that the paper's proof also invokes (there via
...
* `KKTFamily.exists_firstVariation_upperGap` — the tail bound `Ψ_h ≥ b_ρ` on
  `𝒯_ρ = [0,2] ∖ 𝒩_ρ` of `lem:first-variation-bound`;
* `KKTFamily.exists_PsiT_sup_bound` — the uniform bound on `sup_{[0,2]}|Ψ_h|` (auxiliary; not
  part of that lemma).
-/
...
`J̃_{p_h}(z) = J̃_{p_*}(z) + Λ_h z + C_h`,  `Λ_h = ℓ(p_h) - ℓ(p_*)`,
...
`J̃_{p_*}` of `SingularEndpoint/AuxiliaryLagrangian/JpTilde.lean` moved by the same affine function. -/
...
conditions of `SingularEndpoint/AuxiliaryLagrangian/FirstVariation.lean` and the quartic bound
...
`kappaVal_continuousAt`, live in `SingularEndpoint/RankOneStationaryFamily/FamilyContinuity.lean`. -/
...
/-- **The uniform sup bound** (auxiliary; not part of `lem:first-variation-bound`): `sup_{[0,2]}|Ψ_h|`
