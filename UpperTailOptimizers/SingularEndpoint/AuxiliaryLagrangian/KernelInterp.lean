...
/-!
# The exact two-variable kernel decomposition (Section 5, `paper/sections/singular_auxiliary_lagrangian.tex`)
...
* the exact four-term identity
  `K = I^xI^yK + R^xI^yK + I^xR^yK + R^xR^yK` (`interp_resid_decomposition`, the unlabelled
  exact identity in the proof of `lem:central-kernel-bound`), together with the commutation of
  the two operators;
* the exact kernel decomposition — an unlabelled display in the same proof —
  ...
  `p_{00} + p_{d0}x^d + p_{0d}y^d + p_{dd}x^dy^d` and `p_{d0} = p_{0d}`.  The paper writes the
  third term as `Q_h(y){v_{0,h}(y) + v_{d,h}(y)x^d}`; by symmetry `v_{i,h} = u_{i,h}`.

**Out of scope, deliberately.**  The value `Θ_0(u_*,u_*) = d^3/4` belongs to
`SingularEndpoint/AuxiliaryLagrangian/KernelTheta.lean` and the corner-error clause
`sup_{0<h<h_ρ}‖Θ_h - d³/4‖_{L^∞(𝒩_ρ²)} ≤ a/4` to `SingularEndpoint/AuxiliaryLagrangian/KernelError.lean`, as does the
uniform boundedness of `u_{0,h}`, `u_{d,h}` and `Θ_h`; the four coefficients `p_{··,h}` are bounded in
`SingularEndpoint/AuxiliaryLagrangian/PowFirstOrder.lean`.  See "What is not here" below.

## The route

Everything rests on `SingularEndpoint/AuxiliaryLagrangian/PowInterp.lean`, which is the one-variable theory of exactly
...
* **The two `Q_h` factors.**  `powResid_eq_powDD_mul` (`SingularEndpoint/AuxiliaryLagrangian/PowInterp.lean`) factors the
  residual as `(x^d - s^d)(x^d - t^d)·powDD`, and the geometric-sum factorisation
  `pow_sub_pow_eq` (`NonexceptionalEndpoint/QuadraticGrowth/PowBounds.lean`) turns `(x^d - s^d)(x^d - t^d)` into
...
* **Symmetry.**  `K_h(x,y) = J_{p_h}(xy)` is symmetric, so the two coefficient slices agree
  ...
  second term uses the *same* one-variable functions `u_{0,h}, u_{d,h}`.  The paper says
  "The third term is the symmetric counterpart" and names its functions `v_{0,h}, v_{d,h}`;
  they coincide with `u_{0,h}, u_{d,h}`, which halves the work.
...
it works on `𝒩_{ρ_0} ⊂ 𝒰 ⊂ (0,1)` throughout.  The hypothesis is genuinely needed and is not a
...
The uniform-boundedness clause of the proof of `lem:central-kernel-bound` — the bound
`max{|p_{ij,h}|, |u_{i,h}(x)|, |v_{i,h}(y)|} ≤ C_d` for `x,y ∈ 𝒩_{ρ_0}` —
is proved here **only for the
geometric cofactor**
(`mfac_mem_Icc`, `KKTFamily.exists_mfac_window`).  This corresponds to the paper's remark that
the factors `(x^d - s_h^d)/(x - s_h)` and `(x^d - t_h^d)/(x - t_h)` "are polynomial sums and
extend continuously to `x = s_h` and `x = t_h`", with the upper bound
`(x^d - s_h^d)/(x - s_h) = ∑_{j<d}x^{d-1-j}s_h^j ≤ d`; the Lean bound is two-sided.  The
remaining factor of each bound is a divided difference of
...
are supplied later instead: the four coefficients of `P_h` in `SingularEndpoint/AuxiliaryLagrangian/PowFirstOrder.lean`
(`exists_kp00_window`, `exists_kpd0_window`, `exists_kp0d_window`, `exists_kpdd_window`), and
the two one-variable functions and the corner quotient in `SingularEndpoint/AuxiliaryLagrangian/KernelError.lean`
...
so that they can be composed, which is what the four-term identity in the proof of
...
`SingularEndpoint/AuxiliaryLagrangian/PowInterp.lean`.  `interpFst_eq_lagrange` is the identification with the paper's
...
geometric-sum factorisation `pow_sub_pow_eq` (`NonexceptionalEndpoint/QuadraticGrowth/PowBounds.lean`) then splits off
...
is the polynomial sum `(x^d - s_h^d)/(x - s_h) = ∑_{j<d}x^{d-1-j}s_h^j` of the proof of
`lem:central-kernel-bound`, which extends continuously to `x = s_h`; together with the
divided-difference formulas it is why the interpolants and remainders have convergent limits
although the individual coefficients of `eq:moment-interpolation` contain `(t_h^d - s_h^d)^{-1}`. -/
...
This is `powResid_eq_powDD_mul` (`SingularEndpoint/AuxiliaryLagrangian/PowInterp.lean`) followed by
...
/-- **The law kernel** `K_h(x,y) = J_{p_h}(xy)`: the kernel `K_h(x,y) = J̃_{p_h}(xy)` of
`sec:auxiliary-lagrangian` on the central square, where `xy ∈ (0,1)` and `J̃_{p_h} = J_{p_h}`,
...
/-- The kernel is symmetric, since `xy = yx`.  This is why the third term of the four-term
identity — in the paper's words "the symmetric counterpart" of the second — uses the *same*
one-variable functions (the paper's `v_{0,h}, v_{d,h}` equal `u_{0,h}, u_{d,h}`). -/
...
/-- **The third term is the symmetric counterpart** of the second:
`Q_h(y){u_{0,h}(y) + u_{d,h}(y)x^d}`, with the *same* one-variable functions (the paper's
`v_{0,h}, v_{d,h}`).  The commutation
...
This is a two-sided, quantitative form of the paper's remark, in the proof of
`lem:central-kernel-bound`, that the factors `(x^d - s_h^d)/(x - s_h)` and
`(x^d - t_h^d)/(x - t_h)` are polynomial sums bounded by `d` — here for their product, with
explicit constants on `𝓝_ρ` and a lower bound as well; the threshold `δ` comes from
