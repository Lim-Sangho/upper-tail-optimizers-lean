...
/-!
# The product-variance identity and the variance bootstrap (Section 5)

The measure-algebra chain behind the law-level localization of
`SingularEndpoint/AuxiliaryLagrangian/DistributionLocal.lean`.  None of the three statements below is a display of the
paper, which never forms the law of an independent pair (`E`, `Var`, `XY` do not occur in it)
and controls the dispersion of the factor through `eq:rank-one-reduction-bounds` and the
Chebyshev step `eq:factor-tail-mass` instead:

* **The product–variance identity** — the exact
  identity `E(XY - r)² = (EX² - r)² + 2r·Var(X)` for independent `X, Y` with a common law;
* **the rank-one case of `R_d² ≤ C_dΓ_d`** (the Lean form of the paper's pointwise bound
  `z^d - r_*^d - d r_*^{d-1}(z - r_*) ≤ C_dΓ_d(z)^{1/2}`) — its consequence
  `2r_*·Var(X) ≤ E(XY - r_*)²`, together with the Cauchy–Schwarz step
  `E(XY - r_*)² ≤ (E(XY - r_*)⁴)^{1/2}` through which `𝒢^{1/2}` enters it;
* the Taylor moment bound `0 ≤ EX^d - (EX)^d ≤ C·Var(X)`.
...
`ν ([0,2])ᶜ = 0` exactly as in `SingularEndpoint/AuxiliaryLagrangian/DistributionMeasure.lean`.  That is why this half of the
capstone is independent of `lem:central-kernel-bound`.

## Independence without a product measure

The identity is naturally phrased through independent copies `X, Y`.  Here it is the iterated
...
factorisation `pow_convex_gap_eq` of `NonexceptionalEndpoint/QuadraticGrowth/PowBounds.lean` — no derivative and no
...
`SingularEndpoint/AuxiliaryLagrangian/DistributionMeasure.lean`'s `integrableOn_of_continuousOn` at `s = [0,2]`, followed by
...
which is how `𝒢^{1/2}` reaches the variance bootstrap in the Lean's chain
...
/-- **The variance bootstrap**, the rank-one case of `R_d² ≤ C_dΓ_d`, whose counterpart in the
paper is the pointwise bound `z^d - r_*^d - d r_*^{d-1}(z - r_*) ≤ C_dΓ_d(z)^{1/2}` applied to an
arbitrary competitor `W` rather than to the factor law:

`2r_*·Var(X) ≤ E(XY - r_*)²`.

Immediate from `integral_prod_sub_sq` at `r = r_*`, the discarded term `(m₂ - r_*)²` being a
square.  Combined with `integral_sq_le_sqrt_integral_pow_four` and the bound
`E(XY - r_*)² ≤ C𝒢^{1/2}` this gives `Var(X) ≤ C𝒢^{1/2}` (neither appears in the paper).  The degree hypothesis is
...
`pow_convex_gap_eq` (`NonexceptionalEndpoint/QuadraticGrowth/PowBounds.lean`) writes the left-hand side exactly as
