...
/-!
# The Young absorption of `lem:auxiliary-lagrangian-bound` (Section 5)

`SingularEndpoint/AuxiliaryLagrangian/DistributionQuant.lean` stops one step short of the refined form of `eq:auxiliary-lagrangian-bound`: it
...
(Lean notation, as in that file: `A_ρ` and `ε_ρ` are the closed-window counterparts of the
paper's `A_{h,ρ}(ν)` and `ν(𝒯_ρ)`, and `C_ρ` is a constant produced at the radius `ρ`),
and leaves the Young absorption that turns those two into `ζ(h)ε_ρ` undone.  **This file is that
absorption**, and the two conclusions it yields: the refined form and a closed-window form of
`eq:auxiliary-lagrangian-bound`.

The absorption has no counterpart in the paper.  The proof of `lem:auxiliary-lagrangian-bound`
bounds the mixed rectangles through the `1/2`-Hölder bound of `lem:continuation-kernel-bounds`
by `C_dh^{1/2}ν(𝒯_ρ) + C_dν(𝒯_ρ)²`, so no `A_{h,ρ}^{3/16}` term and no `ζ` arise, and it closes
by decreasing `h_ρ` so that `C_dh^{1/2} ≤ b_ρ/4` and `C_dν(𝒯_ρ) ≤ b_ρ/4`.  The paper-form
statement, proved along that route, is `auxiliary_lagrangian_bound`
(`SingularEndpoint/GraphonComparison/GraphonLagrangianBound.lean`).

## The three analytic steps

The exponents `16/3` and `16/13` are conjugate, `3/16 + 13/16 = 1`.  Written with the
splitting constant left to be *derived* rather than guessed, the steps are
...
* `zeta`, `tendsto_zeta`, `exists_zeta_le` — the loss function
  ...
  `ζ(h) → 0` as `h ↓ 0` and the `ε`-form actually consumed: after decreasing the threshold
  once more, `ζ(h) ≤ b_ρ/2`.  Both summands vanish because any positive power of `h` beats
...
* `KKTFamily.exists_distributionGap_refined` — the refined form of `eq:auxiliary-lagrangian-bound`, obtained by
  ...
  of `A_ρ` is `a/2 = d³/48`, as in `eq:auxiliary-lagrangian-bound`: `distributionGap_lower_of_bounds`
  leaves `d³/32`, and the Young step spends `a/8 = d³/192` of it.
* `KKTFamily.exists_distributionGap` — a closed-window form of `eq:auxiliary-lagrangian-bound`
  with a single constant `M`, from the refined bound by
  `exists_firstVariation_upperGap` (`Ψ_h ≥ b_ρ` on `T_ρ`) and `exists_zeta_le` at `b_ρ/2`.

## What is assumed, and what is not done

The hypotheses `hCorner`, `hu0`, `hud` of `SingularEndpoint/AuxiliaryLagrangian/DistributionQuant.lean` — the two clauses of
the proof of `lem:central-kernel-bound` proved in `SingularEndpoint/AuxiliaryLagrangian/KernelError.lean`
...
them.  `SingularEndpoint/AuxiliaryLagrangian/DistributionFinal.lean` discharges them, through the package of
`SingularEndpoint/AuxiliaryLagrangian/DistributionWindow.lean`.  The first-variation bound `hTW` is carried in the same way
...
The distribution-level uniqueness statement,
which reads `ν = ν_h` off this bound for an exact minimiser (the analogue of the last step of
`sec:singular-proof`), is not here; it is
...
* `tendsto_logInv_mul_rpow` — `(1 + log(1/h))·h^r → 0` as `h ↓ 0` for every `r > 0`, shared
  with `SingularEndpoint/GraphonComparison/GraphonComparisonMaster.lean`.
-/
...
/-- **The Young step** (not in the paper; see the module docstring).  With the conjugate exponents
`16/3` and `16/13`,
...
/-- **The loss function `ζ(h) = C_{ρ,K}(L_h^{16/13}h^{12/13} + L_hh⁴)`** (not in the paper),
with `L_h = 1 + log(1/h)`, written with `h^{12/13}` in
...
/-- `(h⁴)^{3/13} = h^{12/13}`: the identification of the factor `h^{12/13}` of `ζ` with the shape
...
/-- **The `ε`-form of `tendsto_zeta` actually consumed**: after decreasing the threshold once
more, `ζ(h) ≤ b_ρ/2`.  The paper has no `ζ`; its corresponding final step decreases `h_ρ` so
that `C_dh^{1/2} ≤ b_ρ/4` and `C_dν(𝒯_ρ) ≤ b_ρ/4`. -/
...
/-- **The absorption, stripped of all measure theory.**
...
`Δ_h(ν)` and `G` for the Lagrangian gap — and the two localisation hypotheses `ε_ρ ≤ Kh⁴`,
`|Δ_h(ν)| ≤ Kh⁴`,
...
The four absorptions are: `ε² ≤ Kh⁴ε` and `ε|Δ| ≤ Kh⁴ε` for the two elementary
...
spent by the Young step is `dd/192 = a/8`, which the drop from `dd/32` to `dd/48` pays for
with `dd/96` to spare. -/
...
The paper displays the gap bound once, as `eq:auxiliary-lagrangian-bound`; a
constant-explicit refinement appears only inside the proof, where the tail term is the
linear `b_ρν(𝒯_ρ)` coming from the tail bound of `lem:first-variation-bound`, together with the
losses `-C_dν(𝒯_ρ)² - C_dh^{1/2}ν(𝒯_ρ)`.  The
...
localisation hypotheses `ε_ρ ≤ Kh⁴` and `|Δ| ≤ Kh⁴`,

`𝒥_h(ν) - 𝒥_h(ν_h) - η_hΔ ≥ (d³/48)A_ρ + ∫_{T_ρ}Ψ̃_hdν - ζ(h)ε_ρ - MΔ²`,

with `d³/48 = a/2` and `ζ` the vanishing loss function of `zeta`/`tendsto_zeta`.  The tail
integral `∫_{T_ρ}Ψ̃_hdν` is kept intact: it is consumed only in
`exists_distributionGap`, and in the closed-window assembly of `lem:graphon-lagrangian-bound`
it is passed on to the rare rows.
...
hypotheses `hu0`, `hud`, `hCorner` are the clauses of the proof of
`lem:central-kernel-bound` that `SingularEndpoint/AuxiliaryLagrangian/DistributionQuant.lean` carries as hypotheses
(proved in `SingularEndpoint/AuxiliaryLagrangian/KernelError.lean`), and
...
/-- **A closed-window form of `eq:auxiliary-lagrangian-bound`**, with a single constant `M`:
...
taken in the linearised form `η_hΔ_h(ν)` of the expansion following `eq:first-variation`
