...
/-!
# The two-node Hermite interpolation remainder (Section 5, `paper/sections/singular.tex`)

The confluent companion of `SingularEndpoint/RankOneStationaryFamily/Interp3.lean`.  There the three interpolation nodes are
...
potential `eq:first-variation` in the factored form `Ψ_h(x) = Q_h(x)² D_h(x)`, with
`Q_h(x) = (x - s_h)(x - t_h)` the node quadratic and a lower bound on the quotient `D_h` that is
**uniform** as `h ↓ 0`.  The paper obtains the factorisation by analytic division of `Ψ_h` by
`Q_h²`, with a quotient `D_h` jointly analytic in `(h, x)`.  As in `SingularEndpoint/RankOneStationaryFamily/Order4.lean` we replace that by
...
already proved in `SingularEndpoint/AuxiliaryLagrangian/FirstVariation.lean` — `KKTFamily.Psi_sVal`, `.Psi_tVal`,
...
Section 5; it is stated for a bare quadruple of derivatives on a compact interval.
-/
...
replaced by two double nodes, and it is what replaces the paper's analytic division of `Ψ_h`
by `Q_h²` in the proof of `lem:first-variation-bound`. -/
