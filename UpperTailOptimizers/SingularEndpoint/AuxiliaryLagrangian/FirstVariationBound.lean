...
/-!
# The quartic first-variation lower bound (Section 5, `paper/sections/singular_auxiliary_lagrangian.tex`)
...
`Ψ_h(x) ≥ a Q_h(x)²` with `a = d³/24`, for `x` in a closed window `[u_* - w, u_* + w]`, which
contains the paper's `𝒩_ρ` for every `ρ ≤ w`.

`SingularEndpoint/AuxiliaryLagrangian/FirstVariation.lean` proves the four contact conditions
`Ψ_h(s_h) = Ψ_h(t_h) = Ψ_h'(s_h) = Ψ_h'(t_h) = 0`, and
`SingularEndpoint/ConstantGraphonComparison/CostGap.lean` proves the collapsed form `eq:endpoint-first-variation`,
`Ψ_0(x) = 2Γ_d(u_* x)` (the paper writes `Γ̃_d`, which equals `Γ_d` on `[0,1]`).  What is added
here is the *quantitative* half.

## The argument

The paper applies analytic division by the monic polynomial `Q_h²` on a neighbourhood of
`u_*`, obtaining `Ψ_h = Q_h² D_h` with `D_h` jointly analytic in `(h, x)`, and reads
`D_0(u_*) = d³/12` off the quartic expansion of `eq:endpoint-first-variation`.  As
elsewhere in `SingularEndpoint/` (see `SingularEndpoint/RankOneStationaryFamily/Order4.lean`, `SingularEndpoint/AuxiliaryLagrangian/Interp4.lean`) that
...
continuity of `(h, ξ) ↦ Ψ_h⁗(ξ)` at `(0, u_*)`: on a small enough box `Ψ_h⁗ ≥ d³`, i.e.
`Ψ_h⁗/24 ≥ d³/24`, which is the paper's `D_h ≥ d³/24`, while `Ψ_0⁗(u_*)/24 = d³/12` is its
`D_0(u_*)`.
...
**Scope.**  The companion bound `Ψ_h ≥ b_ρ` on the tail `𝒯_ρ = [0,2] ∖ 𝒩_ρ` is not proved here; it is
`KKTFamily.exists_firstVariation_upperGap` in `SingularEndpoint/AuxiliaryLagrangian/PsiTilde.lean`.  The reason for the
split is that the paper's `Ψ_h` (`eq:first-variation`) is built from the kernel
`K_h(x,y) = J̃_{p_h}(xy)` of the continued entropy `eq:entropy-continuation`
(`paper/sections/singular_auxiliary_lagrangian.tex`),
...
where they agree; the paper likewise differentiates `Ψ_h` only near `u_*`, where all products
lie in `(0,1)`.
...
* `KKTFamily.Psi1_sVal`, `.Psi1_tVal` — the two slope contacts of
  `SingularEndpoint/AuxiliaryLagrangian/FirstVariation.lean` restated as `Ψ_h'(s_h) = Ψ_h'(t_h) = 0`;
...
`SingularEndpoint/RankOneStationaryFamily/Contact.lean`); the exponent `d - 3` does **not**, and is kept as a Nat cast
throughout (see `Psi4`). -/
...
(`SingularEndpoint/RankOneStationaryFamily/FkktDeriv.lean`), which this file does not import. -/
...
`Psi1` is the derivative expression of `hasDerivAt_Psi` (`SingularEndpoint/AuxiliaryLagrangian/FirstVariation.lean`)
...
the derivative displayed just before `lem:first-variation-bound` (`dη_h = 2γ_hq_h`). -/
...
`Lstar3` (`SingularEndpoint/RankOneStationaryFamily/Contact.lean`) and `Fkkt4` (`SingularEndpoint/ConstantGraphonComparison/Fkkt4.lean`): differentiating
...
/-- `hasDerivAt_Psi` (`SingularEndpoint/AuxiliaryLagrangian/FirstVariation.lean`) with its derivative expression named:
`Ψ_h' = Psi1`. -/
...
`Psi_deriv_sVal` and `Psi_deriv_tVal` of `SingularEndpoint/AuxiliaryLagrangian/FirstVariation.lean` are stated as the
...
This is the paper's `D_0(u_*) = d³/12` multiplied by the `24` of the Hermite remainder. -/
...
the statement stays inside a window (the paper's `Ψ_h` is built from the continued entropy
`J̃` of `eq:entropy-continuation`, which agrees with `J_{p_h}` only where the products lie in
`[0,1]`, while `KKTFamily.Psi` uses `J_{p_h}` itself). -/
...
constant `a = d³/24` of `lem:first-variation-bound`.  A weakening of
`exists_Psi4_window`. -/
...
the paper's `Ψ_h ≥ a Q_h²` on `𝒩_ρ` with the value `a = d³/24` chosen there.
...
(`SingularEndpoint/AuxiliaryLagrangian/PsiTilde.lean`), which works with the continued entropy; see the module
docstring. -/
