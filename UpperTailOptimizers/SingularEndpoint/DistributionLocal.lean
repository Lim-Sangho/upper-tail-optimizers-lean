...
/-!
# Automatic localization of the one-dimensional law (Section 7)
...
* the variance bootstrap, with `𝒢` on the right — `exists_variance_le_sqrt_gamAvg`; the
  paper carries no such display: the
  convexity defect `R_d` and `R_d² ≤ C_dΓ_d` play this role in the proof of
  `lem:localization-rank-one`;
* the mean bootstrap — `KKTFamily.exists_mean_bootstrap`; no display in the
  paper, the differenced edge identity
  the edge/moment identity playing its role;
* the localization `𝒢 = O(h⁴)` — `KKTFamily.exists_gamAvg_localization`, the target;
  the paper's counterpart is the unlabelled bound
  `(∫W^d - r_h^d) + ∫Γ_d(W) ≤ C_d h⁴` in the proof of `lem:localization-rank-one`;
* the tail-mass estimate — `KKTFamily.exists_tail_mass_le`; the paper's counterpart is
  ...
  (a standing hypothesis, now a consequence of
  localization rather than an assumption).
...
`gamAvg d ν = ∫∫Γ̃_d(xy) dν dν` is the paper's `𝒢 = EΓ̃_d(XY)` and
`KKTFamily.distributionJ B h ν = ∫∫J̃_{p_h}(xy) dν dν` its `𝒥_h`.  As in
`SingularEndpoint/DistributionVariance.lean`, independence is expressed by an iterated integral rather
...
* `KKTFamily.Lam`, `KKTFamily.Cconst`, `KKTFamily.distributionJ` — the law
  functional and the two displacement coefficients `Λ_h`, `C_h` (defined in the
  paper just above `eq:endpoint-supporting-gap`);
...
`SingularEndpoint/DistributionGap.lean`, `SingularEndpoint/DistributionQuant.lean` and `SingularEndpoint/DistributionAbsorption.lean`,
assembled in `SingularEndpoint/DistributionFinal.lean`.
-/
...
/-- **The averaged continued gap `𝒢 = E Γ̃_d(XY)`** of the localization proof
(the paper's localization
`lem:localization-rank-one`, works with `∫Γ_d(W)` at the graphon level and does not
form this average), written as an iterated integral rather than through a product
...
`SingularEndpoint/DistributionVariance.lean`, carrying a leading coefficient. -/
...
`h^n` of the identically named private helper of `SingularEndpoint/FamilyLocalization.lean` — is what
...
`eventually_nhdsWithin_iff` + `Metric.eventually_nhds_iff` idiom of `SingularEndpoint/Gap.lean`, used
...
localization runs at the graphon level, where `R_d² ≤ C_dΓ_d` and Cauchy–Schwarz
play this role in the proof of `lem:localization-rank-one`.  The chain here is the
older one.  `two_mul_rStar_mul_variance_le` of `SingularEndpoint/DistributionVariance.lean` gives
...
Inverting `z ↦ z^d` only near `u_*` is what would make
it first needed the crude bound `𝒢 = O(h²)` — through `Var(X) = o(1)` — to know that the mean
is near `u_*` (the paper drops this inversion argument altogether).  On the
...
abbreviation the paper introduces just above `eq:endpoint-supporting-gap`,
with the displacement identity `eq:entropy-parameter-shift` and the expansion
`Λ_h = O_d(h²)` from `eq:rank-one-parameter-expansions`.  A name for the coefficient that
...
(`C_h` is defined just above `eq:endpoint-supporting-gap`) collected,
...
again, exactly as in `SingularEndpoint/DistributionVariance.lean`.
...
which every lemma of `SingularEndpoint/DistributionVariance.lean` applies to `ν_h`. -/
...
(whose expansion `d³h⁴/3 + O_d(h⁶)` is an
unlabelled display in the proof of `lem:constant-graphon-comparison`).  Integrating twice against the
...
(`SingularEndpoint/GamAverage.lean`), whose limit `d³/3` is turned into a two-sided bound by
...
(`tendsto_qVal_coeff`, `eq:rank-one-parameter-expansions`), and the inversion
...
(the Γ-localisation display, the goal of the kernel-free half of the old
two-part law lemma; the paper's counterpart of this bound is the unlabelled
`(∫W^d - r_h^d) + ∫Γ_d(W) ≤ C_d h⁴` in the proof of `lem:localization-rank-one`,
feeding the sharp `∫W^d` bound and `eq:graphon-quartic-localization`).
...
The five inputs are the five steps of the paper's proof: `gamAvg_le_of_distributionJ_le`
(now `eq:graphon-cost-decomposition`), `exists_gamAvg_distribution_le` (`𝒢_h = O(h⁴)`),
`exists_variance_le_sqrt_gamAvg` (now the rank-one case of `R_d² ≤ C_dΓ_d`),
`exists_mean_bootstrap`
together with `exists_distributionMean_bound` (now the differenced the edge/moment identity), and
`tendsto_ell_coeff` (`Λ_h = O(h²)`); `localization_arith` chains them and applies Young's
inequality. -/
