...
/-!
# Running Banach: a solution of the nonlinear Factor equation (Section 7)

`lem:localization-rank-one` of `paper/singular_endpoint.tex` asserts, for a graphon `W` with
`‖W - r_*‖₄` small, the existence of a nonnegative factor `f` solving the **nonlinear Factor
equation**

`T_W(f^{d-1}) = q f`,  `q = ∫ f^d`

— the first half of `eq:rank-one-orthogonality`.  This file proves exactly that, by running
Banach's fixed-point theorem on the normalised contraction map of `SingularEndpoint/FactorContraction.lean`
across the `L⁴` bridge of `SingularEndpoint/FactorLpBridge.lean`, and then rescaling.

## The three steps

1. **A smaller threshold.**  `SingularEndpoint/FactorContraction.lean` fixes all its constants before its
...
`eq:rank-one-reduction-bounds`.  The first three are `SingularEndpoint/FactorSolution.lean`,
the fourth `SingularEndpoint/FactorStability.lean` and the fifth `SingularEndpoint/FactorHDensity.lean`, joined in
`SingularEndpoint/FactorMain.lean`.  The uniqueness proved here is uniqueness of *a.e. fixed points of
the normalised map inside the invariant set* `S` (`factorFix_fixedPoint_unique`), which is what
Banach gives; the paper's uniqueness of the solution of the Factor equation in a fixed
`L⁴`-neighbourhood of `u_*` is a different statement, and its normalised form is
`factorMain_unique` (`SingularEndpoint/FactorMain.lean`).
...
Every constant of `SingularEndpoint/FactorContraction.lean` is fixed before its threshold `ε₀`, so the
...
`SingularEndpoint/FactorContraction.lean`. -/
...
functions.  `SingularEndpoint/FactorLpBridge.lean` supplies the complete set `lpBridgeSet M`; what is
...
vanish).  It is **not** the paper's uniqueness clause, which asserts uniqueness of the solution
of `eq:rank-one-orthogonality` in a fixed `L⁴`-neighbourhood of `u_*`; transferring between
the two is left to the assembly. -/
