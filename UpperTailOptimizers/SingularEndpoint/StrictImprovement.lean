...
/-!
# Strict improvement over the constant graphon (Section 7, `paper/singular_endpoint.tex`)
...
and in particular that the difference is **negative** for all sufficiently small `h > 0`, so
that `(p_h, r_h)` lies in the symmetry-breaking phase.
...
* `tendsto_gamInt_sub_gamRVal` (`SingularEndpoint/GamRVal.lean`): the first is `\frac{d³}{3}h⁴ + o(h⁴)`.
  It needed only `α_h → 1/2`;
* `tendsto_ell_coeff` with `Lcoeff_eq` gives `Λ_h = \frac{2d³}{3(d-1)}h² + o(h²)`
  (the third line of `eq:rank-one-parameter-expansions`), and `tendsto_edge_gap` gives
  `e(W_h) - r_h = -(d-1)h² + o(h²)` (unlabelled in the proof of `lem:constant-graphon-comparison`),
  so the second bracket is `-\frac{2d³}{3}h⁴ + o(h⁴)`.

The total is `-\frac{d³}{3}h⁴ + o(h⁴)`.  Only the **sign** is consumed downstream, so the
paper's sharper `O_d(h⁶)` remainder — which comes from evenness in `h` — is not needed.
...
paper's closing sentence that `W ≡ r_h` "is feasible but has strictly larger cost".

The constant graphon `W ≡ r_h` is feasible because its `H`-density is `r_h^m`
(`graphon_tDensity_eq_rVal_pow`), so this places `(p_h, r_h)` in the symmetry-breaking
phase. -/
...
Kept apart from `singular_endpoint_full` on purpose: this is the one singular endpoint statement that costs the
`lubetzkyZhao` axiom. -/
