...
/-!
# One-variable interpolation in the basis `{1, x^d}` (Section 7, `paper/singular_endpoint.tex`)
...
`s_h^d, t_h^d`, so the arity-2 theory of `SingularEndpoint/Interp2.lean` applies verbatim.  The whole
...
`KKTFamily.rVal` (`SingularEndpoint/Family.lean`).

## The paper's operator

Line 1751 of the proof introduces
`𝒟_d F = ½ (F''(u_*) - (d-1)/u_* · F'(u_*))`, the value at the coalescing limit of
`R_0^xF(x)/(x - u_*)^2`.  It is `powDop` below, and `powDop_eq_powDeriv2` is the correspondence
...
## Relation to `SingularEndpoint/TailScalar.lean`

`TailScalar.lean` already carries these definitions for the single function
`φ(u) = J̃_{p_h}(x̃u)` of `lem:auxiliary-lagrangian-bound`; that file is the special case
...
* `powDop`, `powDop_eq_powDeriv2` — the paper's `𝒟_d` and the normalisation constant;
...
Nothing here mentions the singular endpoint family, the graph `H`, or any object of Section 7; the
...
`I_h^xF = A + Bx^d`; `KKTFamily.gCoefB` (`SingularEndpoint/TailScalar.lean`) is the special
...
On `w > 0` this is the substitution of `paper/singular_endpoint.tex`, under which the
...
`φ' = φ₁` then `dφ/dw = φ₁(x) / (d x^{d-1})`.  This is the operator appearing in the paper's
`𝒟_d`. -/
...
Equivalently `(φ₂(x) - (d-1)/x · φ₁(x)) / (d² x^{2d-2})`, which is the paper's `𝒟_d` up to the
...
/-! ## The paper's coalescing operator -/

/-- **The operator `𝒟_d` of `paper/singular_endpoint.tex`**:
...
/-- **The correspondence with the paper's operator, and the normalising constant.**  For
`x > 0`,

  `𝒟_d F = (d² x^{2d-2} / 2) · (d²F/dw²)`.

The residual proved below is `powDeriv2 ξ / 2` times `(x^d - s^d)(x^d - t^d)`, while the paper
...
This is `dd2_factor` (`SingularEndpoint/Interp2.lean`) read through the chart: `dd2_factor` is the
...
This is `exists_dd2_eq_deriv2_half` (`SingularEndpoint/Interp2.lean`) applied to `powChart d φ` on
...
this is what replaces the "standard integral formula for first and second divided differences"
quoted in the proof of `lem:central-kernel-bound`. -/
...
cofactor.  The two-sided bound the paper actually uses is `powDD_mem_of_powDeriv2_mem`. -/
