...
/-!
# `lem:localization-rank-one` as a single theorem (Section 7)

`lem:localization-rank-one` asserts **one** pair of
constants `(M, h₀)` for which four displays hold simultaneously, of every graphon `W` that is
feasible at `(p_h, r_h)` and costs no more than the family candidate `W_h`:

* `eq:graphon-quartic-localization` — `∫|W - r_*|² ≤ Mh²` and `∫|W - r_*|⁴ ≤ Mh⁴`;
* the sharp `∫W^d` bound — `0 ≤ ∫W^d - r_h^d ≤ Mh⁴`;
* the nonlinear Factor decomposition `W = f ⊗ f + E` with `‖E‖₂ ≤ Mh²` and
  `|∫f^d - q_h| ≤ Mh⁴`;
* `eq:factor-tail-mass` — `|{x : f(x) ∉ 𝓝_ρ}| ≤ M_ρ h⁴`.

The four were proved separately, each with its own `(M, δ)`:  `family_localization_sq` and
`family_localization` (`SingularEndpoint/FamilyLocalization.lean`), `family_ld_sharp` and
`family_factor_clause` (`SingularEndpoint/LdSharp.lean`), and
`family_factor_tail_mass_of_closeness` (`SingularEndpoint/FactorTail.lean`).  This file is the merge:
...
The paper's last display is about *the* `f` of the decomposition, so the merge is only
...
`SingularEndpoint/FactorSolution.lean` chain below it), nor threading the pointwise equation back out
of `localization_rankOne` (which would mean rebuilding, in `SingularEndpoint/LdSharp.lean`, the
...
* **`lem:localization-rank-one`**, all four displays with one `(M, δ)` —
...
directly.  The decomposition's remaining paper hypotheses — `f ≥ 0` and `T_E(f^{d-1}) = 0` —
...
all four displays of the lemma hold at that single `M`:
