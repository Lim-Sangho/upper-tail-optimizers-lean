...
/-!
# Joint analyticity of the desingularized family system (Section 7, `paper/singular_endpoint.tex`)

`lem:rank-one-kkt-family` constructs the singular endpoint family by applying the
**analytic** implicit function theorem to the desingularized system

  `F(u, ℓ, γ, h) = (E₁, E₂, E₃)`

of `SingularEndpoint/FamilySystem.lean`, at the base point
`(u, ℓ, γ, h) = (u_*, ℓ_*, γ_*, 0)`.  Two hypotheses feed that theorem: invertibility of the
...
/-- The log-odds `ℓ(z) = log((1-z)/z)` of `SingularEndpoint/Defs.lean` is analytic at every point of
...
form of `SingularEndpoint/FamilySystem.lean`: the denominators `1 - s²` and `t` take the nonzero
...
the closed form of `SingularEndpoint/FamilySystem.lean`: the single denominator `(1 - st)²` takes the
