...
/-!
# The coalescing rank-one stationary family (Section 7, `paper/singular_endpoint.tex`)

`lem:rank-one-kkt-family` of `paper/singular_endpoint.tex` produces, near the singular
...
as a structure, `KKTFamily d`. It does not formalise the first-variation definitions
`eq:graphon-stationarity` and `eq:block-stationarity`, or prove their equivalence to
these scalar equations; see `FORMALISATION.md`, deviation [D11]. It then derives the
...
silently.  That relativisation is no longer a limitation: `SingularEndpoint/FamilyBuild.lean` proves
...
where it is wanted, is the fourth-order expansion `eq:rank-one-parameter-expansions`, the
ordering `p_h < r_h`, and local exhaustiveness (the last being available as
...
* `KKTFamily.sVal_neg`, `.tVal_neg`, `.qVal_even`, `.rVal_even` — reflection in `h`, the
  symmetries `p_h, r_h, u_h` even and `α_{-h} = 1 - α_h` of `thm:endpoint-optimality`;
...
* `KKTFamily.rVal_pow_card_edges` and `.graphon_tDensity_eq_rVal_pow` — `r_h^m = q_h^v`,
  hence `t(H, W_h) = r_h^m` (`thm:endpoint-optimality`);
...
Three clauses of the lemma are deliberately **not** fields.

* The expansions `eq:rank-one-parameter-expansions`: nothing below consumes them, and carrying
  them would make the structure harder to construct.
* The ordering `p_h < r_h` of the admissibility clause: again unused downstream.
* Local exhaustiveness ("any nonconstant two-valued rank-one graphon whose data are
  sufficiently close to the base point … agrees with the member of this family with
  half-gap `h = (t-s)/2`").  Its *base-point* instance is proved outright, and separately,
  as `triple_contact_forced` (`SingularEndpoint/Forced.lean`).

Four clauses that the paper states here are **derived** rather than assumed, and so are
theorems below rather than fields: the ordering `0 < p_h < r_h < 1`, the constraint
`t(H, W_h) = r_h^m` (`graphon_tDensity_eq_rVal_pow`), the nonconstancy of `W_h`
(`graphon_not_ae_const`, `SingularEndpoint/Terminal.lean`), and the local uniqueness clause
(`exists_scalar_family_locally_unique`, `SingularEndpoint/FamilyUnique.lean`). -/
...
`lem:rank-one-kkt-family` states that `u_h, p_h, γ_h, q_h, r_h, μ_h, ℓ_h` are
functions of `h²` while `α_{-h} = 1 - α_h`, and `thm:endpoint-optimality` repeats it for
`p_h, r_h, u_h`.  The structure carries the four primitive symmetries as fields; the derived
scalars inherit them here.  Reflection exchanges the two factor values, and `q_h` is symmetric
...
/-- `r_{-h} = r_h`, the evenness clause of `thm:endpoint-optimality`. -/
...
/-- **The candidate graphon `W_h`** of `thm:endpoint-optimality`: the rank-one two-block
...
/-- `t(H, W_h) = q_h^{|V(H)|}`: the rank-one identity of `paper/singular_endpoint.tex`. -/
...
/-- **`t(H, W_h) = r_h^m`**, the feasibility clause of `thm:endpoint-optimality`. -/
