...
/-!
# Distribution-level uniqueness, after `sec:singular-proof` (Section 5)

`SingularEndpoint/AuxiliaryLagrangian/DistributionFinal.lean` states a closed-window, measure-level form of
`eq:auxiliary-lagrangian-bound` (and its refined form) with every deferred hypothesis
discharged.  This file turns that gap bound into a *uniqueness* statement for the distribution
problem: a probability measure `ν` on `[0,2]` with `m_d(ν) = q_h` and `𝒥_h(ν) ≤ 𝒥_h(ν_h)` is
`ν_h`.  The paper states no such lemma.  Its counterpart is the last paragraph of the proof of
`thm:singular-endpoint` (`sec:singular-proof`, `paper/sections/singular_proof.tex`), where
`Δ_h(ν) = 0`, `A_{h,ρ}(ν) = 0` and `ν(𝒯_ρ) = 0` for the distribution `ν` of the values of `f`,
so that "`ν` is supported on `{s_h,t_h}`, and `Δ_h(ν) = 0` implies
`ν = α_hδ_{s_h} + (1-α_h)δ_{t_h}`".  The argument below is the same, with
`eq:auxiliary-lagrangian-bound` in place of `eq:graphon-lagrangian-bound`.

## The four steps

1. `Δ_h(ν) = 0` makes the moment hypothesis `|Δ_h(ν)| ≤ Kh⁴` of `exists_distributionGap`
   trivial, and its tail hypothesis `ν(T_ρ) ≤ Kh⁴` (the Lean form of `eq:factor-tail-mass`) is
   `KKTFamily.exists_tail_mass_le` (`SingularEndpoint/AuxiliaryLagrangian/DistributionLocal.lean`), which holds
   for exactly the competitors considered here.  `exists_distributionGap` then reads
   `M⁻¹(A_ρ + ε_ρ) ≤ 𝒥_h(ν) - 𝒥_h(ν_h) ≤ 0`, and both `A_ρ = ∫_{𝓝_ρ}Q_h²dν` and
   `ε_ρ = ν(T_ρ)` (the closed-window counterparts of `A_{h,ρ}(ν)` and `ν(𝒯_ρ)`) are
   nonnegative, so both vanish.
...
`exists_distributionGap` fixes the constant `K` of its two localisation hypotheses *before* the window
radius `ρ`, while `exists_tail_mass_le` produces its `K` *from* `ρ`.  The composition in
`SingularEndpoint/AuxiliaryLagrangian/DistributionFinal.lean` therefore cannot be used as a black box here; instead the same
...
* **`KKTFamily.distribution_unique`** — distribution-level uniqueness, the analogue of the
  last step of `sec:singular-proof`;
* **`KKTFamily.distributionMeasure_isMinimizer`** — `ν_h`
...
`distributionMeasure_Icc_compl` and `integral_pow_distributionMeasure` (`SingularEndpoint/AuxiliaryLagrangian/DistributionMeasure.lean`,
`SingularEndpoint/AuxiliaryLagrangian/DistributionLocal.lean`) — so the statement is not vacuous.
-/
...
/-- **A measure carried by `{s,t}` is a combination of two Dirac masses.**  This turns
"`ν` is supported on `{s_h,t_h}`" (last paragraph of `sec:singular-proof`,
`paper/sections/singular_proof.tex`) into `ν = ν{s_h}δ_{s_h} + ν{t_h}δ_{t_h}`, a form that also
identifies the two weights.
...
The moment hypothesis of `exists_distributionGap` is trivial because `Δ_h(ν) = 0`, and its tail
hypothesis is `exists_tail_mass_le`; `exists_distributionGap` then bounds
...
/-! ## Step 4: the two-point measure with the right moment is `ν_h` -/

/-- **Distribution-level uniqueness** (the analogue of the last step of `sec:singular-proof`).
...
/-- **`ν_h` is the unique minimiser at the pinned moment**:
for every small `h > 0` the candidate law `ν_h` is *the* minimiser of `𝒥_h` among
probability measures on `[0,2]` with `m_d(ν) = q_h`.  The paper states no such lemma; see the
module docstring.
...
contradict the strict inequality.  No attainment or compactness argument is needed. -/
