...
/-!
# `thm:singular-endpoint`, the optimality clause (Section 5)

`sec:singular-proof` closes by combining `eq:graphon-lagrangian-bound` with a linear lower
bound on the constraint slack; here both are fed into the scalar endgame
`singular_endpoint_endgame`.  Both inputs are now available:

* the calibration estimate in its graph-carrying form is
  `exists_singular_endpoint_comparison_graph` (`SingularEndpoint/GraphonComparison/GraphonComparisonMaster.lean`);
* the linearised constraint is derived here, from the *same* three ingredients as the
  ...
  `thm:singular-endpoint`, with constant `C_{d,m,ρ}`), which the master estimate itself
  supplies once the competitor is feasible and no more costly than `W_h`.

What this file stops short of is the uniqueness half of `thm:singular-endpoint`.
`singular_endpoint_endgame` returns `Δ = 0` and `R = 0`, i.e. `E = 0`, `ν(T_ρ) = 0` and `Q_h = 0`
`ν`-a.e.; turning that into a measure-preserving relabelling of `W_h` is the last step of the
paper's proof: "`Δ_h(ν) = 0` implies `ν = α_hδ_{s_h} + (1-α_h)δ_{t_h}`.  Moreover, `E ≡ 0`
...
/-- **The linear lower bound on the constraint slack.**  If `sl ≥ aΔ - C₁Δ²`, `2κ ≤ a`, `Δ ≥ 0` and
`C₁Δ ≤ κ`, then `κΔ ≤ sl`. -/
...
/-- **`thm:singular-endpoint`, optimality clause.**
...
The proof is `sec:singular-proof`.  If the competitor costs more there is nothing to
...
then `t(H,W) - r_h^m ≥ κΔ` with `κ = n c^{n-1}/2`; the paper states this linearised
constraint with the quadratic defect already absorbed, as
`μ_h{t(H,W) - r_h^m} - C_{d,m,ρ}Δ_h(ν)² ≥ (1/2)μ_h v q_h^{v-1}Δ_h(ν)`.  The
...
`exists_singular_endpoint_optimality` and the uniqueness clause of `thm:singular-endpoint` follows
in `SingularEndpoint/Proof/TerminalUnique.lean`. -/
theorem exists_singular_endpoint_rigidity (hd : 2 ≤ d) {V : Type*} [Fintype V] [DecidableEq V]
  ...
  -- the residual bound in the proof of `thm:singular-endpoint`: `‖E‖₂ ≤ M|Δ|`
...
/-- **`thm:singular-endpoint`, optimality clause.**  The immediate corollary of
