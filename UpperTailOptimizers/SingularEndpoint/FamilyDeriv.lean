...
/-!
# The base-point Jacobian of the family system (Section 7, `paper/singular_endpoint.tex`)

`SingularEndpoint/FamilyBase.lean` and `SingularEndpoint/FamilyAnalytic.lean` supply two of the three
...
with `jac3` the `3 × 3` continuous linear equivalence of `SingularEndpoint/Jacobian3.lean`.

The computation is *elementary*, not analytic.  At `h = 0` every `logSlope` argument of
`SingularEndpoint/FamilySystem.lean` carries a factor `h`, so it is identically `logSlope 0 = 1` as
...
(`SingularEndpoint/FamilySystem.lean`) — the hypothesis-free forms, which is what a statement about
...
`cast_sub_two` of `SingularEndpoint/Contact.lean` record. -/
