...
/-!
# The `L⁴` bridge: raw-function estimates ↦ a fixed point (Section 7)

The existence-and-uniqueness half of `lem:localization-rank-one` is obtained here by a **normalised contraction** rather than
...
`SingularEndpoint/KernelOp.lean` deliberately works with raw functions `ℝ → ℝ` and the *functional*
...
* a **pointwise** (not a.e.) representative in `[0, M]` for a class that satisfies
  `0 ≤ F ≤ M` a.e. — which is what `SingularEndpoint/Factor.lean`'s `FactorDecomp` needs, since its
...
* the invariant set `lpBridgeSet M = {0 ≤ F ≤ M, ∫ F = 1}`: nonempty, convex, closed, and
  hence complete;
* a one-line packaging of Banach's theorem on that set, with its uniqueness companion —
  the uniqueness clause of `lem:localization-rank-one` *is* Banach uniqueness.
...
chunk (I) of `lem:localization-rank-one`.
...
`Lnorm` (`SingularEndpoint/KernelOp.lean`) is a Bochner integral of a real power; `eLpNorm` is an
...
contraction estimate proved with the raw-function vocabulary of `SingularEndpoint/KernelOp.lean` be
...
This is what `SingularEndpoint/Factor.lean` needs: the fields `f_nonneg` and `f_bdd` of
...
(`abs_pow_sub_pow_le_deg`) and what `SingularEndpoint/Factor.lean` needs pointwise. -/
...
output of the contraction iteration is handed to `SingularEndpoint/Factor.lean`. -/
...
/-- **Uniqueness**, the clause `lem:localization-rank-one` states as "there is a unique
nonnegative `f`": two fixed points in `S` of a map that contracts on `S` coincide.  No
completeness is needed for this half. -/
