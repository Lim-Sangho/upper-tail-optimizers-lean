...
/-!
# The fourth derivative of the rank-one KKT function (Section 7, `paper/singular_endpoint.tex`)

`SingularEndpoint/Contact.lean` differentiates the KKT function
`𝓛_*(z) = J_{p_*}'(z) - γ_* z^{d-1}` three times and records the non-degeneracy
`𝓛_*'''(r_*) = d⁵/(d-1)²`; `SingularEndpoint/FkktDeriv.lean` does the same for a general multiplier
`γ`.  Those three derivatives pin the `h⁴` order of the increments at the contacts `A_h` and `B_h`
of `sec:singular-endpoint`, because a three-node Lagrange remainder evaluated at an
unknown intermediate point is only good to leading order.

The `h⁵` order needs one derivative more: the quotient `R` in the paper's factorisation
`𝓛_h = P · R` — the unlabelled display `𝓜_h'(z) = D_h(z)·R(h,z)` at
`paper/singular_endpoint.tex` — has `R(r_*) = 𝓛_*'''(r_*)/6` and
`R'(r_*) = 𝓛_*^{(4)}(r_*)/24`, and it is the ratio of these two that produces the
coefficient `a_1 = α'(0) = 2d²u_*/(3(d-1))` of the block weight.  This file supplies the
missing derivative, in the style of `SingularEndpoint/FkktDeriv.lean`, and evaluates it at `r_*`:
...
* `Fkkt4` — the fourth `z`-derivative of `F_{p,γ}`, with `hasDerivAt_Fkkt3` closing the
  chain begun in `SingularEndpoint/FkktDeriv.lean`;
* `Lstar4`, `hasDerivAt_Lstar3` — the same at the singular endpoint parameters, continuing the chain
  of `SingularEndpoint/Contact.lean`.  `Lstar4 d = Fkkt4 d (gammaStar d)` holds definitionally, so no
...
`hasDerivAt_Fkkt'`, `hasDerivAt_Fkkt1`, `hasDerivAt_Fkkt2` of `SingularEndpoint/FkktDeriv.lean`.
...
`SingularEndpoint/Contact.lean`.  The two trailing factors are Nat casts for the reason explained
at `Fkkt4`. -/
noncomputable def Lstar4 (d : ℕ) (z : ℝ) : ℝ := Fkkt4 d (gammaStar d) z

/-- The fourth derivative of `𝓛_*`, closing the chain `hasDerivAt_Lstar`,
`hasDerivAt_Lstar1`, `hasDerivAt_Lstar2` of `SingularEndpoint/Contact.lean`. -/
