...
/-!
# The window package for the quantitative half of `lem:auxiliary-lagrangian-bound` (Section 7)

`KKTFamily.distributionGap_lower` (`SingularEndpoint/DistributionQuant.lean`) assembles the gap bound of
...
* the four coefficient bounds `|p_{00,h}|, |p_{d0,h}|, |p_{0d,h}|, |p_{dd,h}| ≤ C_ρ` and the
  two one-variable bounds `|u_{0,h}|, |u_{d,h}| ≤ C_ρ` on `𝓝_ρ`, i.e. the "all coefficients
  and one-variable functions are bounded by `C_ρ`" clause of
  `lem:central-kernel-bound`;
* the corner error `|Θ_h(x,y) - d³/4| ≤ δ` of `lem:central-kernel-bound` with
  `δ ≤ a_ρ/8 = d³/192`;
* the quartic first-variation bound `(d³/24)Q_h² ≤ Ψ̃_h` of `lem:first-variation-bound`;
* the crude entropy bound `sup_{[0,4]}|J̃_{p_h}| ≤ C` of `paper/singular_endpoint.tex`.
...
the mixed and tail terms are still exact, so it is not yet the paper's final display.  The
Young absorption of `paper/singular_endpoint.tex` that turns them into `ζ(h)ε_ρ` is
`SingularEndpoint/DistributionAbsorption.lean` (`exists_distributionGap_refined` =
the refined form, `exists_distributionGap` = `eq:auxiliary-lagrangian-bound`), and
`SingularEndpoint/DistributionFinal.lean` combines that with this package into the hypothesis-free
...
* `KKTFamily.DistributionWindow` — the bundled hypothesis list of `distributionGap_lower`, at a
  fixed window radius `ρ`, fixed constants `C_ρ` (split into `Cu` for the one-variable
  functions and `Cp` for the coefficients) and `C`, and a fixed `h`;
...
* **`KKTFamily.exists_distribution_window_forall`** — the same at *every* radius below a
  threshold, which is what the Step-5 join of `lem:graphon-lagrangian-bound` needs;
...
accuracy is fixed at the largest value `distributionGap_lower` accepts, `δ = a_ρ/8 = d³/192`.
...
structure DistributionWindow (B : KKTFamily d) (ρ Cu Cp C h : ℝ) : Prop where
  ...
  /-- `lem:central-kernel-bound` at accuracy `a_ρ/8 = d³/192`. -/
  tailSq_le : ∀ x ∈ centralWindow d ρ, ∀ y ∈ centralWindow d ρ,
    |B.kernelTheta h x y - (d : ℝ) ^ 3 / 4| ≤ (d : ℝ) ^ 3 / 192
  /-- The quartic first-variation bound for the continued potential, with `a_ρ = d³/24`. -/
...
itself.  That is not enough for the Step-5 join of `lem:graphon-lagrangian-bound`: the law half
...
Unlike the multiplier bound of `SingularEndpoint/GraphonComparisonMain.lean`, this one is not free by
...
`M = C_ρ + 192C_ρ²/d³` is the constant of `distributionGap_lower`, built from the window package.
This is the refined form of `eq:auxiliary-lagrangian-bound` with the mixed and tail terms still exact, so it is
not yet the paper's final display; the Young absorption that turns them into `ζ(h)ε_ρ` is
`KKTFamily.exists_distributionGap_refined`
(`SingularEndpoint/DistributionAbsorption.lean`), and the two are joined in `SingularEndpoint/DistributionFinal.lean`. -/
