...
/-!
# The sum of increments at the contacts at order `h⁴` (Section 5, `paper/sections/singular.tex`)

`SingularEndpoint/RankOneStationaryFamily/Order4.lean` and `SingularEndpoint/RankOneStationaryFamily/Order4Right.lean` prove the `h⁴` order of the two
...
separate `-2d³/3`, and is what `SingularEndpoint/ConstantGraphonComparison/Order5.lean` divides the `h⁵` numerator by.
...
two scalars whose `h²` rates `SingularEndpoint/ConstantGraphonComparison/Expansions.lean` has pinned.
...
already-known rates `Λ_h/h² → L₂` and `Δ_h/h² → G₂`.  The closed form uses
`family_eq_cross` (`L₂ = G₂ r_*^{d-1}`) and `Lcoeff_eq`. -/
