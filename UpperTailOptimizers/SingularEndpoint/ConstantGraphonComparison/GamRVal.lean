...
/-!
# The subtracted gap `Γ_d(r_h)` is negligible (Section 5, `paper/sections/singular.tex`)
...
`SingularEndpoint/ConstantGraphonComparison/GamAverage.lean` evaluated the first summand of the first bracket,
...
Exactly the squeeze used for the middle node in `SingularEndpoint/ConstantGraphonComparison/GamAverage.lean`: `r_h` may equal
