...
/-!
# The `h⁵` increment at the contacts and the block-weight slope (Section 5)
...
the `α_h` entry of `eq:rank-one-parameter-expansions`.

## How the `h⁵` order is reached without a Weierstrass division

The `h⁵` coefficient cannot come from the Lagrange remainder used at `h⁴`: there the third derivative is evaluated at an unlocated `ξ`, and at `h⁵` the answer
...
1. **The displacement identity** `Mfun_sub_eq` (`SingularEndpoint/ConstantGraphonComparison/IncrementSum.lean`) writes the
...
Everything else is the `Γ_d` expansion of `SingularEndpoint/ConstantGraphonComparison/GamTaylor.lean`.  The fifth-order contact
constant `k₄ = Γ_d^{(5)}(r_*)` (the paper's `k_4` is `k₄/24`, and its `k_3` is `k₃/6`) enters
through `contact_const_rel`, `k₄ r_* = 5(d-2)k₃`, and
...
/-- **The trapezoid term in closed form.**  The `G₂`-weighted trapezoid error contributes

  `G₂ · \frac{16(d-1)(d-2)u_*³ r_*^{d-3}}{3} = \frac{32(d-2)u_* r_* k₃}{9}`,
...
/-- **`eq:rank-one-parameter-expansions`, the `α_h` entry**:
