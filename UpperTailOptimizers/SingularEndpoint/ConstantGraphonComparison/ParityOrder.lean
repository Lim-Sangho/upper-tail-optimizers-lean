...
`paper/paper.tex` state their remainders two orders past the leading term:
...
The argument uses only `taylorTail` (`SingularEndpoint/ConstantGraphonComparison/TaylorTail.lean`):
