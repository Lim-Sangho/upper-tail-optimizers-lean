...
/-!
# The averaged gap along the coalescing family (Section 5, `paper/sections/singular.tex`)

The proof of `lem:constant-graphon-comparison` in `paper/sections/singular.tex` evaluates the averaged
...
the first half of `eq:family-gap-expansion`, one of the two ingredients of that comparison.
This file proves the limit form
...
* `tendsto_Gam_div_pow_four` (`SingularEndpoint/ConstantGraphonComparison/GamQuartic.lean`) — `Γ_d` vanishes to order exactly
...
* `tendsto_sVal_sq_rate`, `tendsto_tVal_sq_rate`, `tendsto_sVal_mul_tVal_rate`,
  `tendsto_alph` (`SingularEndpoint/ConstantGraphonComparison/FamilyRates.lean`) — the three node rates `-2u_*`, `+2u_*`, `0`
...
* `eventually_abs_lt_h₀` — the parameter is eventually inside the family window.  The last
  two are shared with `SingularEndpoint/ConstantGraphonComparison/GamRVal.lean`.
-/
...
leading term of the paper's `eq:family-gap-expansion`, without its `O_d(h⁶)` remainder, which
would need the family expansions `eq:rank-one-parameter-expansions`. -/
