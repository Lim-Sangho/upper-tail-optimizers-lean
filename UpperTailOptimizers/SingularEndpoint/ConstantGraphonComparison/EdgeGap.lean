...
/-!
# The singular endpoint edge gap (Section 5, `paper/sections/singular.tex`)
...
In the paper this is the first display of `lem:constant-graphon-comparison`, whose proof uses
neither the abbreviations `U₂`, `a₁` nor the explicit coefficient computation reproduced below.

This file proves its leading term, `(e(W_h) - r_h)/h² → -(d-1)`, together with the two
expansions it passes through: that of `q_h`, from `app:rank-one-parameter-expansions`, and the `r_h` entry of
`eq:rank-one-parameter-expansions`.  The `O_d(h⁴)` remainder, from the parity of the family, is
part of `constant_graphon_comparison` (`SingularEndpoint/ConstantGraphonComparison/CostRemainder.lean`):
...
`SingularEndpoint/ConstantGraphonComparison/Expansions.lean` and the block-weight slope of `SingularEndpoint/ConstantGraphonComparison/Order5.lean`; no new
...
* `tendsto_qVal_coeff`, `tendsto_rVal_coeff` — the `q_h` expansion of
  `app:rank-one-parameter-expansions` and the `r_h` entry of `eq:rank-one-parameter-expansions`;
...
/-- **The expansion of `q_h`** in the proof of `lem:rank-one-parameter-expansions`
...
/-- **`eq:rank-one-parameter-expansions`, the `r_h` entry**: `r_h = r_* - \frac{2(4d-1)}{3}h² + o(h²)`.
