...
/-!
# Analyticity and the singular endpoint Taylor data of `𝓛_*` and `Γ_d` (Section 5, `paper/sections/singular.tex`)
...
`SingularEndpoint/RankOneStationaryFamily/Quotient.lean` and its consumers take as input. -/
...
/-- `𝓛_*^{(4)}` near `r_*`, the top of the chain supplied by `SingularEndpoint/ConstantGraphonComparison/Fkkt4.lean`. -/
