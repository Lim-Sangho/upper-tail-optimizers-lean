...
/-!
# The first-variation dual potential (Section 7, `paper/singular_endpoint.tex`)
...
(; the paper defines `κ_h` only
implicitly, by `Ψ_h(s_h) = 0`, and no longer displays this evaluation).
...
wells (the constant `a` is `ρ`-uniform) and `Ψ_h ≥ b_ρ` away from them,
which the paper gets from the Weierstrass factorisation
`Ψ_h = Q_h² R_h` — is not here: it is `exists_firstVariation_lower` (`SingularEndpoint/FirstVariationBound.lean`) and
`exists_firstVariation_upperGap` (`SingularEndpoint/PsiTilde.lean`), both by a two-node Hermite remainder in
...
`eq:first-variation`, and no longer states this evaluation; it follows in one line
