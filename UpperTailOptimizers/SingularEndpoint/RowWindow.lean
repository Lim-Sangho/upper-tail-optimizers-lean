...
/-!
# The central set `Ω_ρ(f)` and the tail mass `ε_ρ(ν)` (Section 7, `paper/singular_endpoint.tex`)

Two objects of the proof of `lem:graphon-lagrangian-bound` (Section 7.5), together
with the elementary facts every later step uses.

* `centralSet d ρ f = {y : f(y) ∈ 𝓝_ρ}` is the paper's `Ω_ρ(f)`, the set of coordinates at
  which the Factor factor lies in the coalescence window `𝓝_ρ = [u_* - ρ, u_* + ρ]`;
* `tailMass d ρ f = |Ω_ρ(f)^c|` is the paper's `ε_ρ(ν)`, the mass the law bound
  `lem:auxiliary-lagrangian-bound` charges at rate `b_ρ` and the calibration display
  `eq:graphon-comparison-estimates` bounds by `C_{d,m,ρ}h⁴`.
...
`SingularEndpoint/RowJensen.lean` both need, and the two uniform family-scalar bounds
`centralSet_exists_qVal_lower` and `centralSet_exists_gam_bound` that the assembly in
`SingularEndpoint/GraphonComparisonMaster.lean` needs.

## Contents

* `centralSet`, `measurableSet_centralSet`;
* `tailMass`, `tailMass_nonneg`, `measure_centralSet_add_tailMass`,
  `half_le_measure_centralSet` — `|Ω_ρ(f)| ≥ 1/2` once `ε_ρ ≤ 1/2`, the sixth clause of
...
`SingularEndpoint/GraphonComparisonMain.lean` has the same statement as a `private` declaration; this copy
is public, being wanted by `SingularEndpoint/GraphonComparisonMaster.lean` as well. -/
