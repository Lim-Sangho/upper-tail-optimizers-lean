...
/-!
# The desingularized family system (Section 7, `paper/singular_endpoint.tex`)
...
`lem:rank-one-kkt-family` read, with `s = u - h` and `t = u + h`,
  ...
  `𝓛(s²) = 0`,  `𝓛(s t) = 0`,  `𝓛(t²) = 0`,   where `𝓛(z) = ℓ_h - ℓ(z) - γ_h z^{d-1}`
                                              (`Lell` of `SingularEndpoint/FamilyEqs.lean`).

At `h = 0` the three roots merge and the system degenerates.  The proof of
`lem:rank-one-kkt-family` replaces it by

  `G₁ = 𝓛(s t)`,  `G₂ = 𝓛(t²) - 𝓛(s²)`,  `G₃ = 𝓛(t²) + 𝓛(s²) - 2𝓛(s t)`,

whose last two members vanish identically at `h = 0`, and then divides `G₂` by `h` and `G₃`
by `h²` — the paper's "analytic Weierstrass division".
...
`SingularEndpoint/FamilyEqs.lean`, through `logSlope` on the logarithmic halves and through the
...
* `Esys2_zero`, `Esys3_zero` — the values of `E₂` and `E₃` at `h = 0`, with the
  hypothesis-free forms `Esys2_zero'`, `Esys3_zero'` that `SingularEndpoint/FamilyDeriv.lean` uses.
-/
...
Both are pure algebra on top of the four factorisations of `SingularEndpoint/FamilyEqs.lean`: the
...
/-- The value of `E₂` at `h = 0` for *every* `u`.  `SingularEndpoint/FamilyDeriv.lean` consumes exactly
...
`SingularEndpoint/FamilyBase.lean`; the content is `Esys2_zero'`. -/
...
`SingularEndpoint/FamilyDeriv.lean` consumes.
...
The three hypotheses are kept for documentation and for the call sites of
`SingularEndpoint/FamilyBase.lean`; the content is `Esys3_zero'`. -/
