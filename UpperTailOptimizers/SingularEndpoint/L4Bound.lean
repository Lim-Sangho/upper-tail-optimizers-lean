...
/-!
# The coercive step of `lem:localization-rank-one` (Section 7)

`Localization.lean` proves the exact cost comparison `eq:graphon-cost-decomposition`,

`(β_d + Λ/(d r_*^{d-1}))(S - S_h) + 𝒢_Γ ≤ 𝒢_{Γ,h} + Λ(𝒱_R - 𝒱_{R,h})`,

together with the Cauchy–Schwarz bound `𝒱_R² ≤ C_d 𝒢_Γ` and the quartic lower bound
`c∫|W - r_*|⁴ ≤ 𝒢_Γ`.  This file performs the step the paper describes as "Young's
inequality gives `S - S_h + 𝒢_Γ ≤ Ch⁴`": the cross term `Λ𝒱_R` is absorbed into half of
`𝒢_Γ` at the cost of a `C_d Λ²/2` remainder, which leaves the coercive inequality
`eq:graphon-quartic-localization` for `∫|W - r_*|⁴`.
...
performed in `SingularEndpoint/FamilyLocalization.lean` (`family_localization`).  The bounds below need
...
* `gamInt_nonneg`, `rdInt_nonneg` — positivity of the two integrals of
  `sec:localization-rank-one`, from the pointwise `eq:endpoint-gap-positivity` and the
  nonnegativity of the scalar convexity defect `R_d` (defined, unlabelled, just before
  the edge/moment identity);
...
/-- `𝒱_R = ∫R_d(W) ≥ 0`: the scalar convexity defect `R_d` (the unlabelled definition
before the edge/moment identity) is pointwise nonnegative on `[0,∞)`, hence at the values of
`W`. -/
...
Along the singular endpoint family the right-hand side is `O(h⁴)`, which is the paper's
`∫|W - r_*|⁴ ≤ Mh⁴`; the `L²` half follows from it by Cauchy–Schwarz. -/
