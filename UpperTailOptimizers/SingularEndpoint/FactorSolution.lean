...
/-!
# What follows from the nonlinear Factor equation (Section 7)

`lem:localization-rank-one` of `paper/singular_endpoint.tex` produces a nonnegative factor `f`
solving the nonlinear Factor equation `T_W(f^{d-1}) = qf` with `q = ∫f^d`.  The *production* of
`f` is a fixed-point argument (`SingularEndpoint/FactorContraction.lean` + `SingularEndpoint/FactorLpBridge.lean`).
...
* `exists_factorDecomp_of_solution` — the join point: the hypothesis list above yields a term of
  `SingularEndpoint/Factor.lean`'s `FactorDecomp d W` whose factor is `f` and whose `qVal` is `q`.
...
* `factorSolutionLnorm_add_le`, `factorSolutionLnorm_neg` — Minkowski at `p = 4` for a general
  finite measure, and the sign-invariance of `Lnorm`, shared with `SingularEndpoint/FactorMain.lean`.
-/
...
The same `integrable_of_abs_le` idiom as in `SingularEndpoint/KernelOp.lean` and
`SingularEndpoint/FactorContraction.lean`; it is `private` in both, hence repeated here under a
file-specific name.
-/
...
/-- **The bridge to `SingularEndpoint/Factor.lean`.**  A pointwise solution of the nonlinear Factor
...
`factorSolution_ortho`.  Everything `SingularEndpoint/Factor.lean` proves — `factor_eq`,
...
`SingularEndpoint/FactorContraction.lean` proves Minkowski for `Lnorm` only at the measure `unitμ`; the
...
(`SingularEndpoint/FactorContraction.lean`) is stated only at `unitμ`; the residual `E` of
...
This is `contractionNum_split` of `SingularEndpoint/FactorContraction.lean`, but for a factor `f` that is *not*
