...
/-!
# `lem:auxiliary-lagrangian-bound`, assembled (Section 7, `paper/singular_endpoint.tex`)

`SingularEndpoint/DistributionAbsorption.lean` proves the refined form of `eq:auxiliary-lagrangian-bound` and
`eq:auxiliary-lagrangian-bound` while still carrying the hypotheses that
`SingularEndpoint/DistributionQuant.lean` deferred — the `C_ρ` bounds on the coefficients of `P_h` and on
`u_{0,h}`, `u_{d,h}`, the corner accuracy `|Θ_h - d³/4| ≤ δ`, the first-variation quartic bound, and
the membership of the two atoms in the window.  `SingularEndpoint/DistributionWindow.lean` proves all of
...
* **`exists_distributionGap_refined_window_forall`** — the same at every radius below a threshold,
  the form the Step-5 join of `lem:graphon-lagrangian-bound` needs;
* `exists_distributionGap_window` — `eq:auxiliary-lagrangian-bound`, hypothesis-free.

## Where the uniqueness clause lives

The **uniqueness clause** of `sec:endpoint-optimality-proof`
is `KKTFamily.distribution_unique` in `SingularEndpoint/DistributionUnique.lean`, which imports this file:
...
`δ = d³/192`, which is the paper's `δ_ρ ≤ a_ρ/8`.  The coefficient `d³/48` is `a_ρ/2`. -/
...
(`SingularEndpoint/GraphonComparisonMain.lean`, `comparisonMain_exists_central_bound`) is in the same form, so a
...
The `A_ρ` coefficient `d³/48 = a_ρ/2` is **independent of `ρ`**, which is what lets the
...
dominates `M⁻¹[A_ρ + ε_ρ] - MΔ²`, with no side conditions beyond `2 ≤ d`, smallness of `h`,
admissibility of `ν`, and `eq:graphon-comparison-estimates`. -/
