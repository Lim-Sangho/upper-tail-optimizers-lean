...
/-!
# Exact division by `h` in the family equations (Section 7, `paper/singular_endpoint.tex`)
...
At `h = 0` the three roots `s², s t, t²` merge, so the system degenerates and is replaced by
the equivalent system

  `G₁ := 𝓛(s t)`,  `G₂ := 𝓛(t²) - 𝓛(s²)`,  `G₃ := 𝓛(t²) + 𝓛(s²) - 2𝓛(s t)`,

whose last two members vanish identically at `h = 0`; one then divides `G₂` by `h` and `G₃`
...
* the logarithmic halves factor through two elementary identities for `ℓ(z) = log((1-z)/z)`,
  ...
  `SingularEndpoint/LogSlope.lean` then pulls out the factor `h` resp. `h²` syntactically
...
analytic through `h = 0`, which is the analytic extension of the divided slopes ("each
slope has an apparent `0/0` singularity at `h = 0`, but both extend analytically") in the
proof of `lem:rank-one-kkt-family`. -/
...
Division of `G₂ = 𝓛(t²) - 𝓛(s²)` by `h` in `lem:rank-one-kkt-family`, on the
logarithmic half. -/
...
Division of `G₃ = 𝓛(t²) + 𝓛(s²) - 2𝓛(st)` by `h²` in
