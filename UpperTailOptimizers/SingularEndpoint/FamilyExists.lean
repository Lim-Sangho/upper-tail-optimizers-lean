...
/-!
# Existence of the coalescing scalar family (Section 7, `paper/singular_endpoint.tex`)
...
* `analyticAt_Fsys` (`SingularEndpoint/FamilyAnalytic.lean`) — joint analyticity of the
  desingularized system at the base point;
* `Fsys_base` (`SingularEndpoint/FamilyBase.lean`) — its vanishing there, which reduces to the
  contact identities `Lstar_rStar`, `Lstar1_rStar`, `Lstar2_rStar`;
* `exists_hasStrictFDerivAt_Fsys` (`SingularEndpoint/FamilyDeriv.lean`) — the base-point derivative,
  identified with the invertible `jac3` of `SingularEndpoint/Jacobian3.lean`.
...
`SingularEndpoint/FamilySymm.lean` (the symmetries and the density `p_h`), `SingularEndpoint/Alpha.lean`
(`exists_family_with_block_weight`), `SingularEndpoint/FamilyUnique.lean` (local uniqueness) and
`SingularEndpoint/Expansions.lean`, and `SingularEndpoint/FamilyBuild.lean` assembles them into
