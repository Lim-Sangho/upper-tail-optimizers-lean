...
`SingularEndpoint/FamilyQuadratic.lean`, `SingularEndpoint/Order5.lean`,
`SingularEndpoint/EdgeGap.lean` and `SingularEndpoint/MuExpansion.lean` prove the *rates* along
...
The paper's reason for the extra order is the parity recorded in `lem:rank-one-kkt-family`:
"`u_h, γ_h` and `Λ_h` have expansions in even powers of `h`, while `α_h - 1/2` has an expansion
in odd powers".  `SingularEndpoint/ParityOrder.lean` turns exactly that into the two-order gain,
...
family (`SingularEndpoint/ParityOrder.lean`). -/
