...
/-!
# The corner error of `lem:central-kernel-bound` (Section 7, `paper/singular_endpoint.tex`)

The corner quotient `Θ_h` of `lem:central-kernel-bound` converges to the
constant `d³/4` of `SingularEndpoint/KernelTheta.lean`, uniformly on the central square and uniformly
...
`lem:auxiliary-lagrangian-bound` consumes; the paper's `δ_ρ := sup … → 0` is the same statement
read through a supremum, and stating it unwound avoids having to produce the supremum before
the boundedness that this file proves.
...
`SingularEndpoint/PowInterp.lean` already sandwiches a single `powDD` between `m/2` and `M/2`
...
`powDopOf_powDopOf_kernel` (`SingularEndpoint/KernelTheta.lean`) already evaluates the iterated
...
`SingularEndpoint/PowFirstOrder.lean`); `thetaV1_sub_pStar` records that `thetaV1` inherits exactly
...
* `KKTFamily.exists_kernelTheta_error` — **`lem:central-kernel-bound`**;
* `abs_powDeriv2_le`, `KKTFamily.hasDerivAt_kernelCoefA_fst`, `.hasDerivAt_kernelCoefB_fst`,
  `.exists_u0_window`, `.exists_ud_window`, `.exists_kernelTheta_bound` — the `C_ρ` clause of
  `lem:central-kernel-bound` for the one-variable functions `u_{0,h}`, `u_{d,h}`
  (`= v_{0,h}, v_{d,h}` by symmetry of the kernel) and for `Θ_h`, which were the last of that
  clause left open by `SingularEndpoint/PowFirstOrder.lean`.
-/
...
`powDop_eq_powDeriv2` converts the paper's `𝒟_d` into the chart second derivative. -/
...
(`SingularEndpoint/PowFirstOrder.lean`), and it is what the `C_ρ` bounds on `u_{0,h}`, `u_{d,h}`
consume. -/
...
/-- **`lem:central-kernel-bound`**, stated
...
This is the paper's `δ_ρ → 0` with the supremum unwound, and it is exactly what the
quantitative half of `lem:auxiliary-lagrangian-bound` consumes.

The proof is the two mean-value steps of `SingularEndpoint/PowInterp.lean` applied in the two
...
/-! ## The remaining `C_ρ` bounds: the one-variable functions `u_{0,h}`, `u_{d,h}`

`lem:central-kernel-bound` asserts that all coefficients and one-variable functions are
bounded by `C_ρ`.  `SingularEndpoint/PowFirstOrder.lean` proved that for the four coefficients of
`P_h`; what was left were `u_{0,h}`, `u_{d,h}` — equal to `v_{0,h}`, `v_{d,h}` by symmetry of
the kernel — and `Θ_h`.  Each is `mfac`, bounded by `mfac_mem_Icc`, times a **second** divided
difference of a coefficient slice, so all that is needed beyond `SingularEndpoint/PowFirstOrder.lean`
...
(`SingularEndpoint/PowFirstOrder.lean`) fed with the entropy bounds `exists_abs_Jp'_le`,
...
/-- **`u_{0,h}` is bounded on the central window, uniformly in small `h`** — the `C_ρ` clause
of `lem:central-kernel-bound` for the first one-variable function.  `u_{0,h}` is `mfac`
...
/-- **`Θ_h` is bounded on the central square**, the last of the `C_ρ` clauses: an immediate
