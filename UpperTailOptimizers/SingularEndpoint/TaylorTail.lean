...
/-!
# The analytic Taylor tail (Section 7, `paper/singular_endpoint.tex`)

`SingularEndpoint/Quotient.lean` divides an analytic function by a *cubic* whose roots are the three
nodes of the coalescing family.  Section 7 also needs the confluent case at a **single**
...
factor, which is stronger than the Lagrange form of `SingularEndpoint/Interp3.lean`: the remainder is
a named function of `z`, not an unlocated `ξ`, so it can be evaluated in a limit.

That is exactly what the `h⁵` order needs.  The `h⁴` order of `SingularEndpoint/Order4.lean` only
...
evaluated, so the Lagrange form is not enough (`FORMALISATION.md`, *The coefficients*).
...
which is the function `SingularEndpoint/Quotient.lean` already knows preserves analyticity
...
* `taylorTail_expand_of_vanishing`, `taylorTail_expand_of_iteratedDeriv_eq_zero` — the
  two-term form used in Section 7, where the first `n` coefficients vanish.
-/
...
is `iterate_dslope_self` of `SingularEndpoint/Quotient.lean` under the new name. -/
...
/-- **The form used in Section 7.**  For `f` analytic at `a` whose first `n` derivatives at
`a` vanish,
