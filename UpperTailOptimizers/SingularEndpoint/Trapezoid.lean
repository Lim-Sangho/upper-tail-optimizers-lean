...
/-!
# The trapezoid remainder (Section 7, `paper/singular_endpoint.tex`)

Section 7 computes the `h⁵` coefficient of the increment at the contacts
...
`SingularEndpoint/Order4.lean` (compare the discussion in `SingularEndpoint/TaylorTail.lean`).
...
`SingularEndpoint/FamilyQuadratic.lean` applied to each of the two inner sums.
...
Section 7 uses this at `x = t_h`, `y = s_h`, where `x - y = 2h` exactly, so the cubic factor
...
Each inner sum is handled by `tendsto_geom_sum₂` (`SingularEndpoint/FamilyQuadratic.lean`), giving the
...
`trapezoid_error_eq` into the `h³`-rate statement Section 7's `h⁵` bookkeeping consumes. -/
