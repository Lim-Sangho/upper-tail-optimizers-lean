...
/-!
# The continued entropy `J̃_{p_*}` and its modulus (Section 7, `paper/singular_endpoint.tex`)
...
```
J̃_{p_*}(u) := J_{p_*}(r_*) + β_d (u^d - r_*^d) + Γ̃_d(u),   0 ≤ u ≤ 4,
```

built from the continued gap `Γ̃_d` of `SingularEndpoint/Continuation.lean`.  Two properties are
...
which supplies the only regularity that the frozen-tail pairing step in the proof of
`lem:auxiliary-lagrangian-bound` needs.  The paper no longer displays this
quantitative modulus: it records only that the
continued entropies are uniformly bounded and equicontinuous on `[0,4]`, fixing an
abstract common modulus `ω_d` in the prose after `eq:entropy-continuation`.
The explicit `τ(1 + log⁺(1/τ))` bound proved here is stronger than the equicontinuity the paper now asserts.
...
`J̃_{p_*}(u) = J_{p_*}(r_*) + β_d (u^d - r_*^d) + Γ̃_d(u)` for `0 ≤ u ≤ 4`.
...
`Nondegeneracy/PowBounds.lean` has the `M = 1` case (`abs_pow_sub_pow_le`); here the
