...
/-!
# The family cost gap and the collapsed dual potential (Section 7)

Two exact consequences of the family data, both proved outright.

**The cost gap.**  `lem:constant-graphon-comparison` of `paper/singular_endpoint.tex` runs through
...
`tendsto_cost_gap` (`SingularEndpoint/StrictImprovement.lean`), which is all that
`singular_endpoint_strict_improvement` consumes, and the sharper `O_d(h⁶)` remainder is not needed
and not proved.
...
(`SingularEndpoint/FirstVariationBound.lean`) and `exists_firstVariation_upperGap` (`SingularEndpoint/PsiTilde.lean`), which
replace the paper's Weierstrass factorisation by a two-node Hermite remainder.
...
`thm:endpoint-optimality`.  (`p_0 = p_*` is the field `p_zero`.) -/
