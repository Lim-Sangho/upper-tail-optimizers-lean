...
/-!
# The cubic sandwich at `𝓜_{p,γ}` (Section 5, `paper/sections/singular.tex`)

`SingularEndpoint/RankOneStationaryFamily/Sandwich.lean` bounds the two increments of an antiderivative of a `C³` function
with three roots; this file instantiates it at the pair `(𝓜_{p,γ}, F_{p,γ})` of
`SingularEndpoint/RankOneStationaryFamily/Family.lean`, whose three roots are exactly the three edge values of a rank-one
KKT point `eq:three-value-kkt`.

The result is the quantitative form of `Mfun_lt_of_root_left` and `Mfun_lt_of_root_right`
(`SingularEndpoint/RankOneStationaryFamily/RowSign.lean`): those give the *signs* of
