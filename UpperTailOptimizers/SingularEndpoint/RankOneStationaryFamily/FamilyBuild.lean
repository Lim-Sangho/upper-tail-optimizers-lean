...
/-!
# The coalescing family exists (Section 5, `paper/sections/singular.tex`)

`SingularEndpoint/RankOneStationaryFamily/Family.lean` transcribes `lem:rank-one-kkt-family` as a structure,
...
`exists_family_with_block_weight` (`SingularEndpoint/RankOneStationaryFamily/Alpha.lean`) produces the family
...
`eq:three-value-kkt` at `s_h², s_h t_h, t_h²`, and the block-weight
