...
/-!
# The `h⁴` order along the coalescing family, right increment (Section 5)

This is the companion of `SingularEndpoint/RankOneStationaryFamily/Order4.lean`: the same statement for the *other* of the
two increments of `eq:block-proportion-formula`, the leading term of the paper's expansion
(in `app:rank-one-kkt-family`, with remainder `O_d(h⁵)`),
...
The proof is a transcription of `SingularEndpoint/RankOneStationaryFamily/Order4.lean`, and reuses its uniform bound
...
**Why `h > 0`.**  As in `SingularEndpoint/RankOneStationaryFamily/Order4.lean`: the nodes are ordered `s² < st < t²` exactly
...
i.e. `B_h = 𝓜_h(t_h²) - 𝓜_h(s_h t_h) = -2d³h⁴/3 + o(h⁴)`.  `SingularEndpoint/RankOneStationaryFamily/RowSign.lean` already
