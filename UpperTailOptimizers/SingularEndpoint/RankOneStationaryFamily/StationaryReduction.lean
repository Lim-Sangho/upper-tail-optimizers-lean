...
/-- **`lem:stationary-rank-one-bipodality`, the two-value step.**  Let `p ∈ (0,1)`, let `μ ≥ 0`,
and let `f` be measurable with `η ≤ f ≤ 1-η` pointwise for some `η > 0`.  If the rank-one graphon
`f ⊗ f` satisfies the KKT conditions at `(p,r)` with multiplier `μ`, then `f` takes at most two
values up to null sets.

`kkt_scalar_of_stationary` turns `eq:graphon-stationarity` into the scalar equation
`eq:rank-one-kkt`, and `exists_two_values` runs the root count of `F_{p,γ}` on it.  The lemma as
the paper states it — bounds almost everywhere, and the normal form `f = f_{α,s,t}` after a
relabelling when `f` is not a.e. constant — is `stationary_rank_one_bipodality`
(`SingularEndpoint/RankOneStationaryFamily/StationaryBipodality.lean`). -/
...
and `eq:block-stationarity` are equivalent to the scalar conditions the rest of Section 5
uses. -/
