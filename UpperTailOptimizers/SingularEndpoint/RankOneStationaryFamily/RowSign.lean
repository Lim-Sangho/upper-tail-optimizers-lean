...
/-!
# Signs between the three roots, and the block weight (Section 5)

`lem:rank-one-kkt-family` of `paper/sections/singular.tex` recovers the block weight
...
`S(z) = z^{d-1}(1-z)` of `SingularEndpoint/RankOneStationaryFamily/Bipodality.lean`, which increases strictly on `[0,r_*]`
...
derivative to the shape function of `SingularEndpoint/RankOneStationaryFamily/Bipodality.lean`. -/
