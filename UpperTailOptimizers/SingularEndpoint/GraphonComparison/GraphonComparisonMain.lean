...
/-!
# The residual half of `lem:graphon-lagrangian-bound` (Section 5, `paper/sections/singular_graphon_comparison.tex`)

`SingularEndpoint/GraphonComparison/GraphonComparison.lean` carries `lem:graphon-lagrangian-bound` as far as the *law* half:
...
law term.  This file estimates the residual entropy integral over the central square, as in
the paper's proof, together with a crude form of the remaining tail contribution.

## Paper results realised

* the division step in the proof of **`lem:graphon-lagrangian-bound`** —
  `comparisonMain_cubic_ideal`, with *explicit* quotients
  `U_h, V_h` (the paper divides `𝓜_h'(xy)` itself by `Q_h` in each variable; here its cubic
  factor is divided, so the quotients are polynomials, computed in closed form) and
  `comparisonMainU_diag`, `comparisonMainV_diag` for `U_0(u_*,u_*) = V_0(u_*,u_*) = 0`;
* the multiplier bound behind **`lem:graphon-lagrangian-bound`** —
  ...
  `|F_{p_h,γ_h}(xy)| ≤ θ(|Q_h(x)| + |Q_h(y)|)` with `θ` as small as one likes.  The paper
  takes `θ = √(a/4)`, in the pointwise form its Young step consumes;
* the strong-convexity step on the central square in the proof of
...
* the orthogonality step of that proof, which moves the linear term to the tail–tail
  square — `comparisonMain_weighted_ortho_fst`,
...
which has the `‖E‖₂²` term (the paper's `C_{d,ρ}^{-1}‖E‖₂²`) together with the `A_ρ` loss it is
allowed, but with `ν(T_ρ)` appearing as a **loss** rather than as part of the gain
`C_{d,ρ}^{-1}ν(𝒯_ρ)` of `eq:graphon-lagrangian-bound`.

In the proof that gain is not manufactured here at all: it is the rank-one tail
`∫_{T_ρ}Ψ_h dν ≥ b_ρ ν(T_ρ)` coming from `lem:first-variation-bound` (in the paper through
`eq:auxiliary-lagrangian-bound`), and the central square and the mixed rectangles only have
to stay below it.  So this file also exposes the **central-only** bound, over the closed-window
set `G_ρ = {x : f(x) ∈ 𝓝_ρ}` (the Lean counterpart of the paper's `Ω_ρ = {x : f(x) ∈ 𝒩_ρ}`),
...
displayed above.  The three estimates are put
together in `SingularEndpoint/GraphonComparison/GraphonComparisonMaster.lean` (`exists_comparison_master`), where the
...
* `comparisonMain_exists_multiplier_bound` — the multiplier bound, with `θ` arbitrarily small;
...
* `comparisonMain_abs_kkt_error_le` — the `𝓜_h'` term on the central square, a step of the
  proof of `lem:graphon-lagrangian-bound`;
* `comparisonMain_exists_central_bound` — the central-only residual bound, the form the assembly
  in `SingularEndpoint/GraphonComparison/GraphonComparisonMaster.lean` consumes;
...
/-! ## The division step of `lem:graphon-lagrangian-bound`: the cubic lies in the ideal `(Q_h(x), Q_h(y))`

The paper divides `𝓜_h'(xy)` by `Q_h(x) = (x - s_h)(x - t_h)` in `x` and by `Q_h(y)` in `y`,
`𝓜_h'(xy) = Q_h(x)U_h(x,y) + Q_h(y)V_h(x,y) + C_h(x,y)` with `C_h` of degree at most one in each
variable, and argues that `C_h` vanishes because `𝓜_h'(xy) = 0` on the grid `{s_h,t_h}²`
(`eq:mh-derivative-factorization`).  Here the division is applied to the cubic factor
`D_h(xy) = (xy - s_h²)(xy - s_ht_h)(xy - t_h²)` of `eq:mh-derivative-factorization`, so both
divisions are *polynomial* and the quotients can be written down; `comparisonMain_cubic_ideal` is then a
`ring` identity, valid for every `(s,t,x,y)` with no side condition at all — in particular
through the coalescing point `s = t`, where the grid argument degenerates. -/

/-- The first quotient `U_h(x,y)` of the division step (Lean version, dividing `D_h(xy)`), in
closed form.
...
/-- The second quotient `V_h(x,y)` of the division step (Lean version), in closed form: the
`y`-division of the remaining remainder by `Q_h(y)`. -/
def comparisonMainV (s t x y : ℝ) : ℝ :=
  ((s + t) ^ 2 - s * t) * (x * y) - (s + t) * (s * t) * y - (s * t) ^ 2

/-- **The division step of the proof of `lem:graphon-lagrangian-bound`, for the cubic factor.**
...
this is read off from `𝓜_0'(xy)` being an analytic multiple of
`(xy - r_*)³ = (u_*X + u_*Y + XY)³` (`X = x - u_*`, `Y = y - u_*`), every monomial of which
has degree at least two in `X` or in `Y`, while `Q_0(z) = (z - u_*)²`. -/
...
the two polynomials, together with `comparisonMainU_diag` and `comparisonMainV_diag`, is the
counterpart of the paper's step "By joint continuity, after decreasing `ρ_0` and `h_ρ`, we may
ensure that `|U_h|` and `|V_h|` are at most `√(a/4)` on `𝒩_ρ²`" — there for the quotients of
`𝓜_h'(xy)`, here for those of its cubic factor `D_h(xy)`, and with an arbitrary `ε` in place
of `√(a/4)`. -/
...
substitute for the paper's factorisation `eq:mh-derivative-factorization`,
`𝓜_h'(z) = J_{p_h}'(z) - γ_hz^{d-1} = D_h(z)R(h,z)` with `R` jointly analytic;
the bound on `R` becomes a bound on `F'''_{p_h,γ_h}` on a compact interval, uniform in
...
there.  This is the paper's `eq:mh-derivative-factorization`, `𝓜_h'(z) = D_h(z)R(h,z)`, in the
form `|R| ≤ C/6`. -/
...
This is the paper's bound `|𝓜_h'(xy)| ≤ √(a/4){|Q_h(x)| + |Q_h(y)|}` on `𝒩_ρ²`, with an
arbitrary `θ` in place of `√(a/4)`, in the pointwise form
that the Young step of `lem:graphon-lagrangian-bound` consumes.  Its two
ingredients are the ideal division (`comparisonMain_cubic_ideal`, with the smallness of
...
/-! ## The strong-convexity step of `lem:graphon-lagrangian-bound`, pointwise
...
paper's (`J_{p_h}''(z) = 1/(z(1-z)) ≥ 4`). -/

/-- **The strong-convexity step, pointwise.**  Wherever the rank-one value
...
produces *one* admissible `ρ`; the assembly in `SingularEndpoint/GraphonComparison/GraphonComparisonMaster.lean`
needs the law half and the residual half at a **common** radius, and each manufactures its own.  Capping one by the other yields
...
have to *lie in* the window — and is restated separately in `SingularEndpoint/AuxiliaryLagrangian/DistributionWindow.lean`. -/
...
The paper splits the residual entropy integral over the central square `Ω_ρ²`, the mixed
rectangles and the tail–tail square, with `Ω_ρ = {x : f(x) ∈ 𝒩_ρ}`.  Here the sets are the
closed-window `G_ρ = {x : f(x) ∈ 𝓝_ρ}` and its complement, and the
...
/-! ## The three integral estimates of the central square -/
...
/-- **The orthogonality identity of the proof of `lem:graphon-lagrangian-bound`.**  Inclusion–exclusion turns the
...
/-- **The orthogonality corner, crude form** (a Lean variant; the paper's corner bound is
`C_dν(𝒯_ρ)²`).  The corner is bounded by a constant times `|G_ρᶜ|`, using `|E| ≤ 5` pointwise. -/
...
that turns the crude `Cε` of `comparisonMain_abs_tailSq_le` into the sharp `Cε‖E‖₂` of
`comparisonMain_abs_tailSq_le_L2`. -/
...
/-- **The orthogonality corner, sharp form**.
...
but **not** for `eq:graphon-lagrangian-bound`: there the loss must be beaten by the tail
gain `b_ρε`, and `b_ρ` has no reason to exceed `2^{2(d-1)}·5`.  The paper keeps both tail
factors and uses boundedness, obtaining `C_dν(𝒯_ρ)²`; the route here is Cauchy–Schwarz on the
corner, whose product measure is `ε²`, giving `Cε‖E‖_{2,(G_ρ^c)²} ≤ Cε‖E‖₂ ≤ CMεh²` along the
family.  Either way the loss vanishes relative to `ε`.
...
/-- **The `𝓜_h'` term on the central square**, a step of the proof of
`lem:graphon-lagrangian-bound`.  The KKT-linear term over the central set is bounded
by `θ(A_ρ + ‖E‖²_{2,G_ρ²})` — the **central** residual norm, which is what the assembly in
`SingularEndpoint/GraphonComparison/GraphonComparisonMaster.lean` needs: a
bound against the full `‖E‖₂²` would cost a fixed multiple of `ε` on the mixed rectangles,
and the tail gain `b_ρε` has no reason to beat it.  The multiplier bound `comparisonMain_exists_multiplier_bound` supplies the
factor `θ(|Q_h(f(x))| + |Q_h(f(y))|)`, and Young's inequality is used in the form
`ab ≤ (a² + b²)/2` (the paper uses `uv ≤ u²/4 + v²` and `(u+v)² ≤ 2(u² + v²)`, reaching
`(a/4)A_{h,ρ}(ν) + ‖E‖²_{2,Ω_ρ²}`). -/
...
rare rows, counted twice: the Lean form of the paper's `‖E‖²_{2,[0,1]²∖Ω_ρ²} ≤ 50ν(𝒯_ρ)`, from
`‖E‖_∞ ≤ 5` and `λ([0,1]²∖Ω_ρ²) ≤ 2ν(𝒯_ρ)`. -/
...
/-- **The central part `(C)` of the residual estimate**, the central-square part of the proof of
...
needs the two apart, because the tail carries a *gain* (`∫_{T_ρ}Ψ_h dν ≥ b_ρν(T_ρ)`) where the
crude treatment only pays a loss.  The three ingredients are the same as there — the
strong-convexity step (`comparisonMain_central_convexGap`, now applied under the
`χχ` weight so that the tail simply drops out), the `𝓜_h'` term
(`comparisonMain_abs_kkt_error_le`) and the orthogonality corner — except that the corner
...
the mixed-rectangle and tail–tail estimates of `SingularEndpoint/GraphonComparison/RowJensen.lean`.  The first is
stated with the `χ` weights of this file, the second with iterated set-restricted integrals
matching the paper's `∬_{Ω_ρ^c×Ω_ρ}` notation, so the two have to be put in one language.
These lemmas do that, and record the
...
/-- **The rare–rare corner of `‖E‖₂²` is `O(ε²)`**: `‖E‖²_{2,(G_ρ^c)²} ≤ 25ε²`, from
`‖E‖_∞ ≤ 5` (the paper bounds the whole complement of the central square at once,
`‖E‖²_{2,[0,1]²∖Ω_ρ²} ≤ 50ν(𝒯_ρ)`). -/
...
/-- **The residual entropy integral split three ways**, over the central square, the two mixed
rectangles and the rare–rare corner, as in the proof of `lem:graphon-lagrangian-bound`; in this
file's labels, the central part `(C)` plus the tail part `(T)` resolved into its two mixed
rectangles and the rare–rare corner. -/
...
This is the central-square part of the proof of `lem:graphon-lagrangian-bound` — the
strong-convexity bound, the `𝓜_h'` term and the orthogonality corner — together with a
*crude* treatment of the
...
from this statement alone; the sharp form of the loss is the row-collapse estimate of
`SingularEndpoint/GraphonComparison/RowJensen.lean`, and the assembly is `exists_comparison_master`
(`SingularEndpoint/GraphonComparison/GraphonComparisonMaster.lean`).

The coefficient of `‖E‖₂²` is `1`, as in the paper's central-square estimate
`≥ ‖E‖²_{2,Ω_ρ²} - (a/4)A_{h,ρ}(ν) - C_dν(𝒯_ρ)²`, because the pointwise modulus `2` of the
strong-convexity step is more than enough to absorb the `θ`-small KKT error.  The passage from
the central to the full `‖E‖₂²` is paid here by `Cν(T_ρ)`; the paper instead passes to
`C_{d,ρ}^{-1}‖E‖₂²`.  `θ` is at the caller's disposal, so the `A_ρ` loss can be made smaller
than the `a/2 = d³/48` gain of the refined form of `eq:auxiliary-lagrangian-bound`. -/
...
`comparison_splitting` (`SingularEndpoint/GraphonComparison/GraphonComparison.lean`) is the identity
...
sharp form of the loss being the row-collapse estimate of `SingularEndpoint/GraphonComparison/RowJensen.lean`.  A second, purely bookkeeping
...
`KKTFamily.exists_distributionGap_refined_window_forall` (`SingularEndpoint/AuxiliaryLagrangian/DistributionFinal.lean`, from
`exists_distribution_window_forall`) is what removes it.  Both are carried out in the assembly
`exists_comparison_master` (`SingularEndpoint/GraphonComparison/GraphonComparisonMaster.lean`). -/
