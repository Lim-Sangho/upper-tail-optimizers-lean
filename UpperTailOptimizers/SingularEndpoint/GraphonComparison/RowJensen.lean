...
/-!
# The mixed rectangles of `lem:graphon-lagrangian-bound`: the row collapse (Section 5, `paper/sections/singular_graphon_comparison.tex`)

This file treats the part of the proof of `lem:graphon-lagrangian-bound`
(`sec:graphon-comparison`) that lives *outside* the central square `Ω_ρ(f)²`,
`Ω_ρ(f) = {x : f(x) ∈ 𝓝_ρ}`: the two mixed rectangles `Ω_ρ(f)^c × Ω_ρ(f)` and the tail–tail
square `(Ω_ρ(f)^c)²`.  It is called Step 3 below; the paper's proof has no numbered steps.
Neither part is asked to produce a gain; both are only kept from costing more than a fixed
fraction of the positive tail term, `b_ρ ε_ρ(ν)` here and `(b_ρ/2)ν(𝒯_ρ)` in
`eq:auxiliary-lagrangian-bound`.  Here `ε_ρ(ν) := |Ω_ρ(f)^c| = ν(T_ρ)` is this file's notation
for the tail mass, the counterpart of the paper's `ν(𝒯_ρ) = λ(Ω_ρ^c)`.

The mixed rectangles are handled, as in the paper, by **collapsing the row to a single
average**.  For almost every `x`, the orthogonality `T_E(f^{d-1}) = 0` of
`eq:rank-one-orthogonality` bounds the row integral `u_*^{d-1}|∫_{Ω_ρ(f)}E(x,y)dy|` by `O(h)`,
so the row average `W̄_x = |Ω_ρ(f)|^{-1}∫_{Ω_ρ(f)}W(x,y)dy` is pinned to the rank-one value,
`|W̄_x - u_*f(x)| = O(h)`.  Jensen's inequality then replaces the row by that single value.
The paper compares the two sides through the `1/2`-Hölder bound of
`eq:continuation-kernel-bounds`, reaching the row bound `≥ -C_{d,ρ}h^{1/2} ≥ -b_ρ/16`; here a
log-Lipschitz modulus `ω(τ) = τ(1 + log⁺(1/τ))` of `J̃_{p_h}` (`exists_JpTildeH_modulus`)
absorbs both the pinning error and the spread of `f` across `Ω_ρ(f)` (`rowJensen_per_row`),
with a bound that is again uniform in `x` and vanishes with `h`.  Integrating over
`x ∈ Ω_ρ(f)^c` gives the mixed-rectangle bound, whose right side is `ε_ρ(ν)` times a quantity
that vanishes with `h`; that is `rowJensen_exists_mixed`, stated for every `β > 0` (the loss
is at most `β ε_ρ(ν)` once `h` is small), where the paper fixes `β = b_ρ/8`.
...
`W̄_x = |Ω_ρ(f)|^{-1}∫_{Ω_ρ(f)}W(x,y)dy`, of the row collapse (Step 3 in this file's
terminology) in the proof of `lem:graphon-lagrangian-bound`. -/
...
`‖f - u_*‖₄ ≤ C_dh` of `eq:graphon-comparison-estimates`. -/
...
collapses the row to a single value in the proof of `lem:graphon-lagrangian-bound`; `J_p` is convex on
...
This crude square-root form is all Step 3 needs; it plays the role of the `1/2`-Hölder bound
of `eq:continuation-kernel-bounds` that the paper uses at this point. -/
...
The paper's row bound is
`≥ -C_d|W̄_x - u_*f(x)|^{1/2} - C_d∫_{Ω_ρ}|f(y) - u_*|^{1/2}dy ≥ -C_{d,ρ}h^{1/2}`, from the
`1/2`-Hölder bound of `eq:continuation-kernel-bounds`.  The route here is the crude
`ω(t) ≤ 3√t` of `logMod_le_sqrt` followed by `√(2t) ≤ 2t/s + s/2`, which reaches the same
conclusion — the bound vanishes as `h ↓ 0` under `eq:graphon-comparison-estimates`, and that
is all that is used afterwards. -/
...
The paper takes `β = b_ρ/8`, so that the `(b_ρ/2)ν(𝒯_ρ)` of `eq:auxiliary-lagrangian-bound`
covers this loss with room to spare; stating it for an arbitrary `β` corresponds to the paper's
choice of `h` so small that its row bound satisfies `C_{d,ρ}h^{1/2} ≤ b_ρ/16`.
...
absorbed, like the mixed rectangles, into the positive tail term `b_ρ ε_ρ(ν)` (in the paper, the
`(b_ρ/2)ν(𝒯_ρ)` of `eq:auxiliary-lagrangian-bound`). -/
...
the mixed-rectangle half of the paper's `‖E‖²_{2,[0,1]²∖Ω_ρ²} ≤ 50ν(𝒯_ρ)`
...
the tail.  This is where the proof gets the **positive** tail-mass term of
`eq:graphon-lagrangian-bound` (the paper's `C_{d,ρ}^{-1}ν(𝒯_ρ)`); the central square and Step 3
only spend it. -/
...
`singular_endpoint_localization` exports `‖f - u_*‖₄ ≤ Mh` (one constant `M` for all its
clauses; the paper's clause is `‖f - u_*‖₄ ≤ C_dh`) and the raw fourth moment that
Step 3 consumes. -/
