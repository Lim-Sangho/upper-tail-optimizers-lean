...
In these statements `ρ₀`, `a = d³/24`, `ρ ↦ b_ρ` and `ρ ↦ C_{d,ρ}` come before the graph;
`C_{d,m}` comes after the graph but before `ρ`; `h_ρ` comes last.  For
`lem:graphon-lagrangian-bound` the proof is carried out once, in
`graphon_lagrangian_bound_of_close_uniform`, where `C_{d,m}` and `h_ρ` are chosen from
`m = |E(H)|` alone, before the graph; `graphon_lagrangian_bound_of_close` is its specialisation.
The paper forms with `C_{d,m}` chosen before the graph are `auxiliary_lagrangian_bound_uniform`
and `graphon_lagrangian_bound_uniform` in `SingularEndpoint/GraphonComparison/PaperForms.lean`.
...
does not depend on it, and only through `m` and `v = 2m/d`, which is why `C_{d,m}` does not
depend on it beyond `m`.
-/
...
theorem graphonLagrangian_two_le_card {V : Type*} [Fintype V] [DecidableEq V]
...
/-- **`lem:graphon-lagrangian-bound` for every decomposition close to the constant, with
graph-independent constants.**  There are `ρ₀ > 0` and `ρ ↦ C_{d,ρ} ≥ 1` on `(0, ρ₀)` such that
for every `m ≥ 1` there is `C_{d,m} ≥ 1` with: for every `0 < ρ < ρ₀` and `K₀ ≥ 0` there is
`0 < h_ρ ≤ h₀` such that for every `d`-regular graph `H` with `m` edges, all `0 < h < h_ρ`, every
graphon `W` and every decomposition `W = f ⊗ f + E` with `‖f - u_*‖₄ ≤ K₀h` and `‖E‖₂ ≤ K₀h`,
and `ν` the law of `f`,
...
`τ_ρ = min(b_ρ/4, α/(2(4/ρ)⁴))` and `c_ρ = min(1, τ_ρ/150)`; `C_{d,m} = C + C_μ n²(2^d)^{2n}`
with `n = 2m/d` (the number of vertices of any such `H`), `C` the graph-free constant of
`auxiliary_gap_bound_law` and `C_μ` a bound for `|μ_h|` (which depends on `m` only).  The graph
enters the proof only through `m`, `n`, `μ_h = B.muVal m h` and the factor `40^m` of the
homomorphism-density splitting, so `C_{d,m}` and `h_ρ` are chosen **before** `H`. -/
theorem graphon_lagrangian_bound_of_close_uniform (hd : 2 ≤ d) (B : KKTFamily d) :
  ...
  intro m hm
  -- the graph enters only through `m`, `n = 2m/d`, `μ_h` and the rank-one splitting of `t(H,W)`
  have hmpos : (0 : ℝ) < (m : ℝ) := by exact_mod_cast hm
  obtain ⟨Cμ, δμ, hCμ, hδμ, hμbd⟩ := B.exists_abs_muVal_le hmpos
  obtain ⟨n, hndef⟩ : ∃ n : ℕ, n = 2 * m / d := ⟨_, rfl⟩
  obtain ⟨N, hNdef⟩ : ∃ N : ℝ, N = (n : ℝ) ^ 2 * (2 ^ d : ℝ) ^ (2 * n) := ⟨_, rfl⟩
  ...
  obtain ⟨s, hsdef⟩ : ∃ s : ℝ, s = min 1 (min ((τ / 3) / (CC * K₀ + 1))
      (min ((τ / 3) / (CK * K' + 1)) ((c / 2) / (Cμ * 40 ^ m * K₀ + 1)))) :=
    ⟨_, rfl⟩
  have hs0 : 0 < s := by
    rw [hsdef]
  ...
  intro V _ _ H _ hreg hcard h hh hpos hlt W P hclose hres
  have hcard1 : 1 ≤ H.edgeFinset.card := by omega
  have hv := graphonLagrangian_two_le_card H hd hreg hcard1
  have hVn : Fintype.card V = n := by
    rw [hndef, ← hcard, factorMain_two_mul_card_edgeFinset H hreg]
    exact (Nat.mul_div_cancel _ (by omega)).symm
  ...
  have hhs3 : h ≤ (c / 2) / (Cμ * 40 ^ m * K₀ + 1) := le_trans hhs (by
    rw [hsdef]
  ...
  rw [eA, eT, eq] at hlaw'
  rw [eA, eT, eq, eN, P.residL2_sq, hcard]
  -- the pieces
  ...
  have hlag := muVal_tDensity_le_etaVal hd H hreg hv hcard1 B hh P
  rw [hVn, ← hNdef, hcard] at hlag
  have hμ := hμbd h hhμ
...
This is `graphon_lagrangian_bound_of_close_uniform` at `m = |E(H)|`; there `C_{d,m}` and `h_ρ`
are chosen before the graph. -/
theorem graphon_lagrangian_bound_of_close (hd : 2 ≤ d) (B : KKTFamily d) :
  ...
  refine ⟨ρ₀, hρ₀, Cρ, hCρ1, ?_⟩
  intro V _ _ H _ hreg hcard
  obtain ⟨Cm, hCm1, hgenm⟩ := hgen H.edgeFinset.card hcard
  refine ⟨Cm, hCm1, fun ρ hρ0 hρlt K₀ hK₀ => ?_⟩
  obtain ⟨hρ, hρ0', hρle, hbound⟩ := hgenm ρ hρ0 hρlt K₀ hK₀
  exact ⟨hρ, hρ0', hρle, hbound H hreg rfl⟩
