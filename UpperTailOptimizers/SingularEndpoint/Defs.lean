...
/-!
# The singular regular endpoint: scalar objects (Section 7, `paper/singular_endpoint.tex`)

This file introduces the scalar objects of `sec:singular-endpoint`, the section of
`paper/bipodal_optimizer.tex` that is `\input` from `paper/singular_endpoint.tex`, together with the
elementary arithmetic facts about them.

Section 7 treats the density excluded from `thm:local-optimizer-structure`, namely `r_* = (d-1)/d`, at
...
* `Gam`, `Rd` — the singular endpoint supporting gap `Γ_d` `eq:endpoint-supporting-gap` and the
  convexity defect `R_d` (unlabelled in the paper);
* `gd` — the diagonal divided slope `g_d(z) = 1/((d-1)z^{d-1}(1-z))` of
  `lem:rank-one-kkt-family`.

Nothing here depends on the graph `H`; the objects that do live in `SingularEndpoint/RankOne.lean`
and above.
-/
...
graphons of Section 7 are `f ⊗ f` with `f` two-valued around `u_*`, so that the three edge
...
(unlabelled in the paper; its two consequences there are the edge/moment identity
and `R_d² ≤ C_dΓ_d`).  It is the Jensen gap that converts a `d`-th moment constraint into
an edge-density statement. -/
