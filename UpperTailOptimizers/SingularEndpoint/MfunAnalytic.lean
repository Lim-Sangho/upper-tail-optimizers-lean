...
/-!
# Analyticity of `𝓜` along the coalescing family (Section 7, `paper/singular_endpoint.tex`)
...
with `s_h = u_h - h`, `t_h = u_h + h` and `𝓜 = Mfun` of `SingularEndpoint/Family.lean`.  Both
...
(`SingularEndpoint/OrderFactor.lean`), which cancels the common factor `h⁴` and reads off the
...
(`LocalOptimizer/Analytic.lean`), and is reused verbatim rather than restated.

The hypotheses of the last three theorems are deliberately spelled with the names and the
shapes that `exists_scalar_family_density` (`SingularEndpoint/FamilySymm.lean`) binds — the analytic
...
`LocalOptimizer/Analytic.lean` — precomposed with `k ↦ (P k, Z k)` via `AnalyticAt.comp₂`. -/
