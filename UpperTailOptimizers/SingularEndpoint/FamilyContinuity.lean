...
`SingularEndpoint/PsiTilde.lean` and `SingularEndpoint/TailScalar.lean` each used to carry a private copy of
...
`SingularEndpoint/CostGap.lean` is the lowest file that has everything: `η_h` and `κ_h` come from
`SingularEndpoint/FirstVariation.lean`, `s_0 = t_0 = u_*` from `CostGap.lean` itself, and the
displacement identity from `SingularEndpoint/Identities.lean`.
...
* `graphon_rankOne`, `graphon_isBipodal`, `exists_graphon_unif` — structural properties
  and uniform convergence used in Theorem 7.1 and hence its introduction corollary.
-/
...
This is the join between the Section-7 layer and the boundary-curve layer of
