...
/-!
# The nonlinear Factor decomposition: the algebraic half (Section 7)

This file formalises the **algebraic consequences** of `lem:localization-rank-one`
of `paper/singular_endpoint.tex`.  That lemma produces, for a graphon `W` close to the constant
constant kernel at the exceptional density `r_*`, a nonnegative factor `f` with
...
genuinely infinite-dimensional argument (a variational problem in `L^{d/(d-1)}` plus a
Banach-space implicit function theorem in `L^{2d}`) and is *not* formalised here.  Instead
...
of the exact binomial expansion of `∫ W^d`, so that `eq:rank-one-reduction-bounds` starts at
order two in `E`.  That is `linear_term_zero`, and `Wmoment_eq` is the expansion itself.
...
* `FactorDecomp.Wmoment_eq` — `eq:rank-one-reduction-bounds`,
...
`exists_factorDecomp` (`SingularEndpoint/FactorMain.lean`), built by the contraction iteration of
`SingularEndpoint/FactorContraction.lean`, `SingularEndpoint/FactorFix.lean` and `SingularEndpoint/FactorSolution.lean`. -/
