...
/-!
# The continuation of the singular endpoint gap to `[0,4]` (Section 7, `paper/singular_endpoint.tex`)
...
`Γ_d(u) = J_{p_*}(u) - J_{p_*}(r_*) - β_d (u^d - r_*^d)` — has a graphon meaning only for
`0 ≤ u ≤ 1`.  The paper therefore replaces `Γ_d` by the real continuation

```
Γ̃_d(u) = Γ_d(u)              for 0 ≤ u ≤ 1,
Γ̃_d(u) = Γ_d(1) + (u-1)^2    for 1 < u ≤ 4,
```
...
* `exists_quartic_le_GamTilde` — the continuation of
  `eq:endpoint-gap-quartic-bound`: `|u - r_*|^4 ≤ C Γ̃_d(u)` on `[0,4]`.
-/
