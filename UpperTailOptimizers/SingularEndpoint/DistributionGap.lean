...
/-!
# The exact quadratic expansion of the law gap (Section 7)

`eq:first-variation`, the algebraic
core of the quantitative half of `lem:auxiliary-lagrangian-bound`.  With the law
kernel `K_h(x,y) = J̃_{p_h}(xy)`, the
two-atom candidate `ν_h` and `σ = ν - ν_h`, the paper's display is

`𝒥_h(ν) - 𝒥_h(ν_h) = ∫Ψ_h dν + η_hΔ_h(ν) + ∬K_h dσdσ`,

and `KKTFamily.distributionJ_sub_eq` is exactly that identity.  Nothing here is an estimate.
The whole content is a four-line computation:
...
(`SingularEndpoint/DistributionVariance.lean`) applies to any finite measure carried by `[0,2]`.

## No signed measures

As in `SingularEndpoint/DistributionMeasure.lean`, the paper's `σ = ν - ν_h` is never formed as a
`MeasureTheory.SignedMeasure`: every occurrence is "integral against `ν` minus integral
against `ν_h`".  Accordingly `KKTFamily.sigmaQuad` *defines* `∬K_h dσdσ` by its
expansion, and `KKTFamily.sigmaQuad_eq` records that this is the expansion the paper
writes, `∬K_h dνdν - 2∬K_h dν_h dν + ∬K_h dν_h dν_h`.  Both integrals converge separately —
...
The quantitative estimate that consumes `eq:first-variation` splits
...
* `KKTFamily.distributionJ_sub_eq` — **`eq:first-variation`**;
...
* `KKTFamily.integral_Icc_split` — additivity over `[0,2] = A ⊔ ([0,2] ∖ A)`, shared
  with `SingularEndpoint/DistributionQuant.lean`.
-/
...
/-- **`∫Ψ_hdν_h = 0`**, the step of `paper/singular_endpoint.tex` reading "`Ψ_h = 0` on the
support of `ν_h`".  Immediate from `PsiT_sVal` and `PsiT_tVal`, since `ν_h` is carried by the
two atoms. -/
...
/-- **The quadratic form `∬K_hdσdσ`** of `paper/singular_endpoint.tex`, defined by its
expansion rather than through a signed measure: with `σ = ν - ν_h` and `∫K_h(x,·)dν_h = 𝒦_h`
(`integral_distributionMeasure_kernel`),

`∬K_hdσdσ = 𝒥_h(ν) - 2∫𝒦_hdν + 𝒥_h(ν_h)`.

`sigmaQuad_eq` records that the right-hand side is the paper's
...
cancels `κ_h`, which is the paper's "`∫dσ = 0`", and leaves `η_hΔ_h(ν)` where the paper's
"`Ψ_h = 0` on the support of `ν_h`" has already been used inside
...
small `h`.  This is the step of `paper/singular_endpoint.tex` reading "`σ` coincides with `ν`
on `T_ρ`". -/
