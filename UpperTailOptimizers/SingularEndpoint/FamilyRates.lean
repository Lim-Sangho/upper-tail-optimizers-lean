...
/-!
# The node rates along the coalescing family (Section 7, `paper/singular_endpoint.tex`)

The proof of `lem:constant-graphon-comparison` evaluates `∫Γ_d(W_h)` — the unlabelled display
`∫Γ_d(W_h) = d³h⁴/3 + O_d(h⁶)` — by expanding `Γ_d` to fourth order at the
...
* `tendsto_self_punctured` — the identity map tends to `0` along `𝓝[≠] 0`.  The last two are
  shared with `SingularEndpoint/FamilyQuadratic.lean`, `SingularEndpoint/Expansions.lean` and
  `SingularEndpoint/IncrementSum.lean`.
-/
