...
/-!
# The family expansions at quadratic order (Section 7, `paper/singular_endpoint.tex`)

`eq:rank-one-parameter-expansions` (in `lem:rank-one-parameter-expansions`) records the Taylor
coefficients of the rank-one KKT family,

  `u_h = u_* + \frac{5-3d}{6u_*}h² + O_d(h⁴)`,
  `γ_h = γ_* + \frac{2d^{d+2}}{3(d-1)^d}h² + O_d(h⁴)`,
  `ℓ(p_h) = ℓ_* + \frac{2d³}{3(d-1)}h² + O_d(h⁴)`.

This file proves the three of them.  Nothing here needs the implicit function theorem a
...
Feeding in the Taylor expansion of `𝓛_*` at `r_*` — which by
`eq:endpoint-entropy-derivatives` starts at the cube,

  `𝓛_*(z) = \frac{k₃}{6}(z-r_*)³ + (z-r_*)⁴ S(z)`,   `k₃ = d⁵/(d-1)²`, `S(r_*) = k₄/24`,

(`Lstar_expand`, from `SingularEndpoint/TaylorTail.lean`) — and dividing by `h²`, `h³`, `h⁴`
respectively gives three linear equations for the three coefficients:

* the **cross** equation, at `h²`: `ℓ₂ = γ₂ r_*^{d-1}`;
* the **difference** of the outer equations, at `h³`: `γ₂(d-1)r_*^{d-2} = \frac{2k₃r_*}{3}`;
...
* `Gcoeff_eq`, `Lcoeff_eq`, `Ucoeff_eq` — `eq:rank-one-parameter-expansions` at quadratic order;
* `eventually_window`, `tendsto_pow_punctured` — the two filter conveniences, the second
  shared with `SingularEndpoint/IncrementSum.lean`.
-/
...
The two coefficients are the `k₃` and `k₄` of `FORMALISATION.md`. -/
...
`SingularEndpoint/TaylorTail.lean` starts at the cube, with the exact non-degenerate coefficient
...
with no remainder.  In the limit `γ₂(d-1)r_*^{d-2} = 2k₃r_*/3` with `k₃ = d⁵/(d-1)²`. -/
...
/-- **`eq:rank-one-parameter-expansions`, the `γ` coefficient**:
...
/-- **`eq:rank-one-parameter-expansions`, the `ℓ` coefficient** (the third line of the display,
which also carries the expansion of `Λ_h`):
...
/-- **`eq:rank-one-parameter-expansions`, the `u` coefficient**:
