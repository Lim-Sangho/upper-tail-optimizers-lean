...
gap by `Γ̃_d(u) = Γ_d(u)` on `[0,1]` and `Γ̃_d(u) = Γ_d(1) + (u-1)²` on `(1,4]`, and sets

```
J̃_{p_*}(u) := J_{p_*}(r_*) + β_d(u^d - r_*^d) + Γ̃_d(u),
J̃_{p_h}(u) := J̃_{p_*}(u) + Λ_h u + J_{p_h}(0) - J_{p_*}(0),      0 ≤ u ≤ 4,
...
`0 ≤ h < h₀`, `u, u' ∈ [0,4]` and `x, x', y ∈ [0,2]`,

```
|J̃_{p_h}(u) - J̃_{p_h}(u')| ≤ C_d |u - u'|^{1/2},   ‖J̃_{p_h} - J̃_{p_*}‖_∞ ≤ C_d h²,   ‖J̃_{p_h}‖_∞ ≤ C_d,
...
* **Hölder.**  The logarithmic modulus `exists_JpTilde_modulus` gives
  `|J̃_{p_*}(u) - J̃_{p_*}(u')| ≤ C τ(1 + log⁺(1/τ))` with `τ = |u - u'| ≤ 4`, and
...
* **Kernel.**  All three kernel bounds are the continuation bounds at `u = xy`, `u' = x'y`,
  using `|xy - x'y| ≤ 2|x - x'|`.
-/
...
window) such that for every `0 ≤ h < h₀`, `u, u' ∈ [0,4]` and `x, x', y ∈ [0,2]`:

* `|J̃_{p_h}(u) - J̃_{p_h}(u')| ≤ C|u - u'|^{1/2}`;
* `|J̃_{p_h}(u) - J̃_{p_*}(u)| ≤ Ch²`, i.e. `‖J̃_{p_h} - J̃_{p_*}‖_{L^∞([0,4])} ≤ Ch²`;
* `|J̃_{p_h}(u)| ≤ C`;
...
theorem continuation_kernel_bounds (hd : 2 ≤ d) (B : KKTFamily d) :
    ∃ C h₀ : ℝ, 1 ≤ C ∧ 0 < h₀ ∧ h₀ ≤ B.h₀ ∧ ∀ h : ℝ, 0 ≤ h → h < h₀ →
      (∀ u ∈ Set.Icc (0 : ℝ) 4, ∀ u' ∈ Set.Icc (0 : ℝ) 4,
          |B.JpTildeH h u - B.JpTildeH h u'| ≤ C * Real.sqrt |u - u'|) ∧
      (∀ u ∈ Set.Icc (0 : ℝ) 4, |B.JpTildeH h u - JpTilde d u| ≤ C * h ^ 2) ∧
      (∀ u ∈ Set.Icc (0 : ℝ) 4, |B.JpTildeH h u| ≤ C) ∧
