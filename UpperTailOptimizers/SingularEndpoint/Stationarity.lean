...
Section 7 of `paper/bipodal_optimizer.tex` states its first-order conditions variationally.  `W` is
...
computes the `I_p` half of the first variation.  `SingularEndpoint/StationaryVariation.lean`
computes the `t(H,·)` half; `SingularEndpoint/StationaryReduction.lean` defines
