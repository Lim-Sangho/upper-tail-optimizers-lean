...
/-!
# The analytic quotient by the node cubic (Section 7, `paper/singular_endpoint.tex`)

Section 7 divides the family function `L_h` by the **node cubic**

  `P(z) = (z - s_h²)(z - s_h t_h)(z - t_h²)`,

and calls the result `R`; the paper obtains `R` by an *analytic Weierstrass division*.  The
`h⁴` order of the increments `A_h`, `B_h` of `lem:constant-graphon-comparison` only needs a Lagrange
remainder (`SingularEndpoint/Order4.lean`), but the `h⁵` order — the coefficient that pins
...
`SingularEndpoint/LogSlope.lean` uses), and away from the centre `dslope f a` is literally a quotient
...
coalesce, which is the case `h = 0` of Section 7. -/
...
case `h ≠ 0` of Section 7, where `s_h², s_h t_h, t_h²` are three separate roots. -/
