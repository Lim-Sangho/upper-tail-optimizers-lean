...
/-!
# The `h⁴` order along the coalescing family (Section 7, `paper/singular_endpoint.tex`)

This is the paper's left-increment expansion:
...
* `Mfun_sandwich_left` (`SingularEndpoint/MfunSandwich.lean`) — for any two-sided bound
...
* `exists_scalar_family_density` (`SingularEndpoint/FamilySymm.lean`) — the family itself.
...
* `family_left_increment_asymptotics` — the `h⁴` limit, with constant `2d³/3`, for a family
  ...
  the right increment of `SingularEndpoint/Order4Right.lean` can be taken against one and the same
...
right increment of `SingularEndpoint/Order4Right.lean` can then be evaluated along *the same*
...
so `A_h = 𝓜_h(s_h²) - 𝓜_h(s_h t_h) = -2d³h⁴/3 + o(h⁴)`, which is the paper's expansion.
`SingularEndpoint/RowSign.lean` already gives `A_h < 0`; this gives its size. -/
