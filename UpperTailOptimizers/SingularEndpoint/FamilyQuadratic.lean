...
/-!
# Quadratic-order rates along the family (Section 7, `paper/singular_endpoint.tex`)

`SingularEndpoint/FamilyRates.lean` records the **first**-order rates of the three edge values,
`(s_h² - r_*)/h → -2u_*`, `(t_h² - r_*)/h → 2u_*`, `(s_h t_h - r_*)/h → 0`, which is all the
`h⁴` order of `SingularEndpoint/GamAverage.lean` needs.  The expansions
...
* the three family coefficients `Ucoeff`, `Lcoeff`, `Gcoeff` — the `U₂`, the `h²` coefficient
  of `ℓ_h`, and the `h²` coefficient of `γ_h` of `eq:rank-one-parameter-expansions` — with their
  defining limits;
...
Nothing here uses `𝓛_*`; the equations themselves are in `SingularEndpoint/Expansions.lean`.
...
For the family functions `u_h, ℓ_h, γ_h` — analytic and even in `h` — this is the coefficient
`eq:rank-one-parameter-expansions` displays, and `tendsto_coeff2` is the limit that pins it. -/
...
paper now introduces `Λ_h` without a label in Section 7.3; its quadratic rate is the
third line of `eq:rank-one-parameter-expansions`. -/
