...
/-!
# The calibration interface of `lem:graphon-lagrangian-bound` (Section 7, `paper/singular_endpoint.tex`)

`lem:graphon-lagrangian-bound` is where the two halves of Section 7
meet.  The **law layer** (`SingularEndpoint/Law*.lean`) speaks of a probability measure `ν` on
the law interval `[0,2]`; the **Factor layer** (`SingularEndpoint/Factor*.lean`) speaks of a
function `f` on `[0,1]` with `W = f ⊗ f + E`.  The lemma joins them through the sentence "let
`ν` be the law of `f`".  This file builds that join and carries it as far as the law part
of the master estimate.

## Paper results realised

* the phrase "`ν` is the law of `f`" of `lem:graphon-lagrangian-bound` — `comparisonDistribution` and the
  transport lemmas below;
* the first line of **`lem:graphon-lagrangian-bound`** (Step 1 of the proof): the identity
  `I_{p_h}(W_h) = 𝒥_h(ν_h)` (`comparison_distributionJ_distributionMeasure`) and the exact splitting of
  the cost difference into the law term `(P)` and one residual entropy integral
...
* the law half of **`eq:graphon-lagrangian-bound`** — `exists_comparison_gap`, which is
  ...
  `lem:auxiliary-lagrangian-bound` and the residual entropy left as an explicit integral.
...
on its left-hand side rather than the paper's `M_ρ⁻¹‖E‖₂²`, and retains the paper's
`η_hΔ_h(ν)` in place of `μ_h{t(H,W) - r_h^m}` — the graph-free Lagrangian used throughout the
`SingularEndpoint` layer (see the module docstring of `SingularEndpoint/DistributionFinal.lean`).  Estimating that
integral is Steps 3 and 4 of the paper's proof: the central convexity bound
of `lem:graphon-lagrangian-bound`, its KKT error (which
needs the Weierstrass division) and its Factor corner are `SingularEndpoint/GraphonComparisonMain.lean`
(`comparisonMain_exists_central_bound`), and Step 3 — the complement of the central square, where the
row is collapsed to its average, pinned by Factor orthogonality and compared by Jensen — is
`SingularEndpoint/RowJensen.lean` (`rowJensen_exists_mixed`, `rowJensen_exists_tailSq`).  Step 4 adds
the three at a common window radius in
`SingularEndpoint/GraphonComparisonMaster.lean`, giving `eq:graphon-lagrangian-bound` itself as
`exists_comparison_master`.

The two hypotheses of `exists_comparison_gap` are the paper's `eq:graphon-comparison-estimates`.
The first, `|{x : f(x) ∉ 𝓝_ρ}| ≤ Kh⁴`, is supplied along the family by `family_factor_tail_mass`
(`SingularEndpoint/FactorTail.lean`) in *exactly* this form.  The second, `|∫f^d - q_h| ≤ Kh⁴`, is
`factorTail_abs_qVal_sub_le`, whose family-level instantiation is `family_factor_clause`
(`SingularEndpoint/LdSharp.lean`), built on `family_ld_sharp` = the sharp `∫W^d` bound.
...
* `comparison_measurable_JpTildeH`, `comparison_measurable_PsiT` — measurability of the
  ...
  `SingularEndpoint/RowJensen.lean` and `SingularEndpoint/GraphonComparisonMain.lean`.
-/
...
`lem:graphon-lagrangian-bound` writes "let `ν` be the law of `f`".  Since `unitμ` is a probability
...
identifies the law layer's `ν(T_ρ)` with the paper's `|G_ρ^c|`. -/
...
`G_ρ^c = {x : f(x) ∉ 𝓝_ρ}` — the shape in which the rare-row cancellation step consumes
it. -/
...
/-! ## Step 1 of the proof: the splitting -/

/-- **`I_{p_h}(W_h) = 𝒥_h(ν_h)`**, the first sentence of Step 1 of the proof of
`lem:graphon-lagrangian-bound`: the family graphon's cost is the law functional at the two-atom
candidate law.
...
/-- **The exact splitting of the Lagrangian gap**, the first line of
...
factor `f` valued in `[0,2]`.  The paper splits the second summand further into the central
part `(C)` and the tail part `(T)`; that split, and its estimation, is what this file does not
do. -/
...
satisfying the a-priori bounds `eq:graphon-comparison-estimates`

* `|{x : f(x) ∉ 𝓝_ρ}| ≤ Kh⁴`  (`eq:factor-tail-mass`, i.e. `family_factor_tail_mass`), and
* `|∫f^d - q_h| ≤ Kh⁴`  (the moment clause of `lem:localization-rank-one`),
...
Comparing with `eq:graphon-lagrangian-bound`: the law terms `M_ρ⁻¹[A_ρ + ν(T_ρ)]` and
`-M_ρΔ²` are here, the Lagrangian term is the layer's graph-free `η_hΔ` in place of
`μ_h{t(H,W) - r_h^m}`, and the residual `M_ρ⁻¹‖E‖₂²` has **not** been extracted: it is still
carried as the entropy integral, whose estimation is Steps 3–4 of the paper's proof.  Nothing
