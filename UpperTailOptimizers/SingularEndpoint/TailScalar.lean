...
/-!
# The scalar half of the tail-uniform two-point interpolation (Section 7)
...
paper does not state that lemma — the frozen-tail pairing is a step
inside the proof of `lem:auxiliary-lagrangian-bound`,
argued by single-point subtraction and Markov with the abstract common modulus `ω_d`, so its
bound carries no logarithmic factor.  The quantitative two-node route formalised here goes
...
`KKTFamily.JpTildeH` (`SingularEndpoint/PsiTilde.lean`), never with `Jp`.  This is the same tilde
issue recorded in `FORMALISATION.md` for the dual potential.
...
* `logMod_le_mul` is the scaling/monotonicity fact `τ ≤ Kσ ⟹ ω(τ) ≤ K ω(σ)` for
  `ω(τ) = τ(1 + log⁺(1/τ))`.  It replaces the draft's implicit use of monotonicity of `ω`,
...
* `KKTFamily.exists_JpTildeH_modulus` transfers the logarithmic modulus
  (`exists_JpTilde_modulus`, `SingularEndpoint/JpTilde.lean`) from `J̃_{p_*}` to `J̃_{p_h}`: the
...
* `KKTFamily.exists_pow_gap_lower` is the node separation `t_h^d - s_h^d ≥ c_d h`.
  ...
  (`Nondegeneracy/PowBounds.lean`) gives `s_h^{d-1}(t_h - s_h) ≤ t_h^d - s_h^d` and
...
singular endpoint (`SingularEndpoint/FamilyContinuity.lean`). -/
...
reals has square at most their product.  Consequently the hypothesis `0 < h` — which the
draft's argument uses through `t_h - s_h = 2h > 0` — is not consumed, and is kept only to
