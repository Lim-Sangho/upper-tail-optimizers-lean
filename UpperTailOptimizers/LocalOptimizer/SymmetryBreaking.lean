...
/-!
# Section 6 of `paper/bipodal_optimizer.tex`, parts (b) and (c): the symmetry-breaking side `p < pc(r)`

The heart of the paper's `thm:local-optimizer-structure`.  Near a non-exceptional `r₀` of a
regular Lubetzky–Zhao boundary arc there are `ρ, η > 0` such that for every `r` with `|r - r₀| < ρ` and every
`p` with `pc(r) - η < p < pc(r)`:

* **(b)** the optimizer of the upper-tail problem is unique up to relabelling, non-constant
  and bipodal;
* **(c)** its edge density and the optimal value have the expansions
...
The assembly follows the paper.  The shift identity of `LocalOptimizer/Basic.lean` turns the
Section-4 reduction into the tilted scalar problem
`Φ_H(p,r) = J_p(r) + min_{0<δ<δ₀} (G_r(δ) - λ(p,r)δ)` (eq. `scalar-minimization-paper`); the
Section-5 chart `boundaryExcess_chart` provides `G_r'' = A_H(r) + O(|δ|) ≥ A_H(r)/2 > 0`; and
...
`r - δ_*`, hence is a relabelling of the single KRR-S maximizer `Ŵ_{r-δ_*, r}`.

Two deliberate deviations from the paper's proof, both simplifications: the critical point is
located by the intermediate value theorem against the strictly increasing `G_r'` rather than by
the analytic implicit function theorem, so no patching of local familyes over `K` is needed;
and the whole argument runs inside **one** chart, since `thm:local-optimizer-structure` is local at `r₀`.
-/
...
/-! ### `thm:local-optimizer-structure`(b),(c) -/

/-- **`thm:local-optimizer-structure`(b),(c), chart-relative core.**  The whole symmetry-breaking analysis, with
the Section-5 analytic chart supplied as *hypotheses* rather than obtained internally, so that
one caller can feed the **same** chart both here and to the implicit-function step of
`LocalOptimizer/Analytic.lean`.  (Two independent calls to `boundaryExcess_chart` produce
...
that the "Furthermore" clause of `thm:local-optimizer-structure` needs: the critical point `δ_*` itself, the fact
...
`Φ = 𝓕_{p,r}(r - δ_*) = J_p(r) + Gc(r, δ_*) - λ δ_*`, the raw first-order bound
...
theorem symmetry_breaking_core {d : ℕ} (hd : 2 ≤ d) (M : LZBoundaryArc d)
    ...
    ∃ ρ η δ₀ C : ℝ, 0 < ρ ∧ 0 < η ∧ 0 < δ₀ ∧ 0 ≤ C ∧ δ₀ ≤ ρc ∧ 2 * ρ ≤ w ∧
      ...
      ∀ r : ℝ, |r - r₀| < ρ → ∀ p : ℝ, M.pc r - η < p → p < M.pc r →
        ...
        ∃ ds : ℝ, ds ∈ Set.Ioo (0:ℝ) δ₀ ∧
          ...
          phiVar H p r = reducedObjective H p (r - ds) (r ^ H.edgeFinset.card) ∧
  ...
  -- ### Uniform Section-5 and Section-4 data on `K`
  ...
  have hedge : ∀ W : Graphon, Feasible H r W → W.Ip p = phiVar H p r →
    ...
    have hmin : ∀ δ ∈ Set.Ioo (0:ℝ) δ₀,
        Gc (r, r - W.edgeDensity) - lam * (r - W.edgeDensity)
          ≤ Gc (r, δ) - lam * δ := by
      intro δ hδ
      have h1 : phiVar H p r ≤ reducedObjective H p (r - δ) (r ^ H.edgeFinset.card) :=
  ...
  have hPhiF : phiVar H p r = reducedObjective H p (r - ds) (r ^ H.edgeFinset.card) := by
...
/-- **`thm:local-optimizer-structure`(b),(c): the symmetry-breaking side.**  Near a non-exceptional `r₀` of a
...
range.  This is `symmetry_breaking_core` with the Section-5 chart supplied by
