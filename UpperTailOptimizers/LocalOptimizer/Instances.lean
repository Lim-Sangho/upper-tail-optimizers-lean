...
theorem K3_has_nonconstant_optimizer :
  ...
  obtain ⟨B, δ₁, hresult⟩ :=
    singular_endpoint_optimizers (d := 2) (by norm_num) K3 K3_reg K3_card_ge K3_verts
  obtain ⟨δ₂, hδ₂, hphase⟩ :=
    singular_endpoint_symmetry_breaking (d := 2) (by norm_num) K3 K3_reg K3_card_ge B
  let δ := min δ₁ δ₂
  have hδ : 0 < δ := lt_min hresult.delta_pos hδ₂
  obtain ⟨h, hpos, hwin, hlt, hle1⟩ : ∃ h : ℝ, 0 < h ∧ |h| < B.h₀ ∧ h < δ ∧ h ≤ 1 :=
    ⟨min (min δ B.h₀) 1 / 2, half_pos (lt_min (lt_min hδ B.h₀_pos) one_pos), by
      rw [abs_of_pos (half_pos (lt_min (lt_min hδ B.h₀_pos) one_pos))]
      have : min (min δ B.h₀) 1 ≤ B.h₀ := le_trans (min_le_left _ _) (min_le_right _ _)
      have := B.h₀_pos; linarith,
      by have : min (min δ B.h₀) 1 ≤ δ := le_trans (min_le_left _ _) (min_le_left _ _)
         linarith,
      by have : min (min δ B.h₀) 1 ≤ 1 := min_le_right _ _
         linarith⟩
  obtain ⟨-, -, hnc, htd, hopt⟩ :=
    hresult.optimizers h hwin hpos (lt_of_lt_of_le hlt (min_le_left _ _)) hle1
  have hsym := hphase h hwin hpos (lt_of_lt_of_le hlt (min_le_right _ _))
...
theorem K4_has_nonconstant_optimizer :
  ...
  obtain ⟨B, δ₁, hresult⟩ :=
    singular_endpoint_optimizers (d := 3) (by norm_num) K4 K4_reg K4_card_ge K4_verts
  obtain ⟨δ₂, hδ₂, hphase⟩ :=
    singular_endpoint_symmetry_breaking (d := 3) (by norm_num) K4 K4_reg K4_card_ge B
  let δ := min δ₁ δ₂
  have hδ : 0 < δ := lt_min hresult.delta_pos hδ₂
  obtain ⟨h, hpos, hwin, hlt, hle1⟩ : ∃ h : ℝ, 0 < h ∧ |h| < B.h₀ ∧ h < δ ∧ h ≤ 1 :=
    ⟨min (min δ B.h₀) 1 / 2, half_pos (lt_min (lt_min hδ B.h₀_pos) one_pos), by
