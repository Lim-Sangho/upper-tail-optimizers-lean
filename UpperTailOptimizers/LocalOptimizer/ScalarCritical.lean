...
/-!
# The tilted scalar problem of Section 6

Section 6 of `paper/bipodal_optimizer.tex` (Section 6) reduces the upper-tail problem, on the symmetry-breaking side, to minimising

`δ ↦ G_r(δ) - λ(p,r)·δ`   over `δ ∈ (0, δ₀)`

(eq. `scalar-minimization-paper`), where `G_r` is the boundary excess of Section 5.  This
...
`tilted_critical_point` produces the critical point `δ_*` and proves everything Section 6
...
function theorem and then patches the local familyes over `K`.  Here it is located by the
intermediate value theorem against the strictly increasing `g'`, which is *quantitative*
(the admissible range `0 < λ < a δ₀` is explicit), needs no patching, and makes the sign and
size of `δ_*` immediate.  The implicit function theorem is still used, but only for the
analyticity clause of `thm:local-optimizer-structure`.
-/
...
* `δ_*` is the **unique root** of `g' = λ` on the whole two-sided interval `[-δ₀, δ₀]`
  ...
  analytic implicit-function family of Section 6 with `δ_*`: that family is only known to
