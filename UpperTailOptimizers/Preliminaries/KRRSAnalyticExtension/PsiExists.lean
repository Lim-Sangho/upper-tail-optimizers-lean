...
`Preliminaries/KRRSAnalyticExtension/PsiUnique.lean` shows that `ψ_d(ε,·)` has *at most* one critical point off the diagonal.
This file produces it, and reads off every property Theorem 3.3 asserts.

## Existence

`Preliminaries/KRRSAnalyticExtension/PsiCompare.lean` gives the sign of `W = ∂_zψ_d·𝒟²` on the two intervals where the ratio
...
The comparison `ψ_d(ε,z) < R_d(ε) < ψ_d(ε,r_*)` of `Preliminaries/KRRSAnalyticExtension/PsiCompare.lean` handles the values on
...
`Preliminaries/KRRSAnalyticExtension/PsiCompare.lean`, since `R_d` is monotone between `ε` and `r_*`. -/
