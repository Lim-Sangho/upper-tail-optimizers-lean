...
single positive excess `ϑ`, for abstract parameters and an abstract pode-size chart:
...
5. `isLocalMax_reduced_entropy` (`Preliminaries/KRRSAnalyticExtension/LocalMax.lean`) turns graphon maximality into local
   maximality of the reduced entropy `Σ(ε,·,·,ϑ)`, and
6. `F_eq_zero_of_isLocalMax` (`Preliminaries/KRRSAnalyticExtension/SigmaLink.lean`) turns that into `F₁ = F₂ = 0`.

The strict positivity `0 < c` needed in step 6 comes from the strict excess: a bipodal
graphon with a null first pode is constant (`tBip_zero`), so its `H`-density would be
`ε^m`, contradicting `ϑ > 0`.

It is used in `Preliminaries/KRRSBipodality/Optimal.lean`, where the chart is taken at the first-pode density of
the maximizer and a compactness argument makes the neighbourhoods uniform.

## Contents

* `F_eq_zero_of_maximizer` — stationarity of a bipodal maximizer at a fixed excess.
-/
...
The strict excess `0 < ϑ` is what forces `0 < c` (`tBip_zero`), which is in turn what
...
theorem F_eq_zero_of_maximizer {V : Type*} [Fintype V] [DecidableEq V] (H : SimpleGraph V)
  ...
  -- `0 < c`: a null first pode makes the graphon constant, so the excess would vanish.
  have hcpos : 0 < c := by
