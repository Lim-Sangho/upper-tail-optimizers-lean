...
`Preliminaries/KRRSAnalyticExtension/PsiIntegral.lean` by that endpoint value, which is exactly the hypothesis of the two
sign lemmas `Hfun_nonpos_right` and `Hfun_nonneg_left` proved in `Preliminaries/KRRSAnalyticExtension/PsiCritical.lean`.
Feeding them the identity

  `W(a,b) = -𝒟'_a(b)·∫_a^b H`

gives `W > 0` below the exceptional density and `W < 0` above it — the two facts that locate
the critical point in `Preliminaries/KRRSAnalyticExtension/PsiExists.lean`.
...
/-- `K_c(s) = 𝒩'_ε(s) - c·𝒟'_ε(s)`; the `H` of `Preliminaries/KRRSAnalyticExtension/PsiIntegral.lean` is the case
`c = A(ε,z)`. -/
...
test function of `Preliminaries/KRRSAnalyticExtension/PsiIntegral.lean`, scaled by `𝒟'`. -/
