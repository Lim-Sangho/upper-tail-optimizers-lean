...
Theorem 1.1) is proved for `d`-regular `H` in `UpperTailOptimizers/Preliminaries/KRRSBipodality/`.

## The argument

1. **The family** (`Preliminaries/KRRSAnalyticExtension/FamilyCore.lean`).  The base point is `(ε₀, a₀, b₀, 0)` with
   `b₀ = ζ_d(ε₀)` (`Preliminaries/KRRSAnalyticExtension/PsiExists.lean`) and `a₀ = a_d(ε₀)` the unique solution of
   `F₁(ε₀,·,b₀,0) = 0` (`Preliminaries/KRRSAnalyticExtension/BasePoint.lean`).  The pode-size chart `exists_C_chart` and
   ...
   (`krrs_family_exists`).
2. **Optimality** (`Preliminaries/KRRSBipodality/Optimal.lean`).  For `ε` near `ε₀` and small `ϑ > 0`, every
   ...
   `paper/sections/krrs_bipodality.tex`).
3. `krrs_rectangle_of_isOptimal` (`Preliminaries/KRRSAnalyticExtension/FamilyCore.lean`) turns optimality into the rectangle
   theorem, and the public forms are corollaries.

Each public form carries part of `thm:krrs-analytic-extension`.  The theorem with all of its
clauses on one `U` and `Δ` is `krrs_analytic_extension` (`Preliminaries/KRRSAnalyticExtension/Extension.lean`).
-/
...
positive-excess regime the entropy maximizer at `(ε, τ)` is the bipodal graphon with those
...
`ε` and fixed `H`-density `τ = ε^m + ϑ` (small excess `ϑ > 0`) exists, is bipodal, and is unique up
...
`c(ε,0) = 0`, `q₂₂(ε,0) = ε`, `q₁₂(ε,0) = ζ_d(ε) ≠ ε` and `q₁₁(ε,0) ∈ (0,1)`, all block
...
Analyticity is stated here only on the boundary slice `ϑ = 0`, which is what the consumers
use; `kenyonRadinRenSadunStrip` below carries the paper's full strip `U × (−Δ, Δ)`.
On `0 < ϑ < Δ` the concrete two-block graphon with these
parameters and block `A = [0,c]` is an entropy maximizer at `(e, t_H) = (ε, τ)`.  The
paper's clause `c(ε,ϑ) = O_{H,U}(ϑ)` is not restated here: analyticity and `c(ε,0) = 0` give it
*locally* uniformly in `ε`, but `thm:krrs-analytic-extension` asks for it uniformly on `U`.  That form is
`kenyonRadinRenSadunUniform` below, which carries every clause of this theorem together with a
single constant, on a possibly smaller rectangle.  All clauses of `thm:krrs-analytic-extension` on one
rectangle are in `krrs_analytic_extension`. -/
...
For `ϑ < 0` the values are only the analytic continuation — as `paper/paper.tex` says
immediately after the theorem, they describe no entropy maximizer, and nothing here claims
they do — this theorem states no maximizer clause at all.  The positive-excess maximizer is
...
`thm:krrs-analytic-extension` asserts `c(ε,ϑ) = O(ϑ)` as `ϑ ↓ 0` **uniformly for `ε ∈ U`**.  Analyticity
...
/-- **`thm:krrs-analytic-extension`, with the uniform block-size bound.**

`kenyonRadinRenSadunAnalytic` together with the paper's uniform smallness clause

`c(ε,ϑ) = O(ϑ)  (ϑ ↓ 0),  uniformly for ε ∈ U`,

on a possibly smaller rectangle.  `thm:krrs-analytic-extension` asks for uniformity over the whole of `U`,
...
form `∀ ε ∈ U, …` or `∀ ε ∈ U, ∀ τ, … < Δ → …`, so all of them survive the shrinking.  Strip
analyticity, `U ⊆ (0,1) ∖ {r_*}`, the smaller first block and `q₁₂(ε,0) = ζ_d(ε)` are added in
`krrs_analytic_extension`. -/
...
/-- **`ζ_d` is real-analytic on `(0,1) ∖ {(d-1)/d}`.**  This is `paper/paper.tex`'s remark
that "`thm:krrs-analytic-extension` below also implies that `ζ_d` is real-analytic on
...
The guard `w ≠ ε` is needed because `psiD d ε ε` is the junk value `0/0 = 0` rather than the
honest limit (see `Preliminaries/KRRSAnalyticExtension/Inputs.lean`).
