...
`Preliminaries/KRRSAnalyticExtension/BasePoint.lean` supplies explicitly.
...
`KRRSFamily.IsOptimal`; it is proved in `Preliminaries/KRRSBipodality/Optimal.lean` (`KRRSFamily.isOptimal`),
and `krrs_rectangle_of_isOptimal` turns it into `krrs_rectangle`.

## Contents

* `exists_ball_pode_pos` — positive excess forces a positive pode size near the base point;
...
/-- **Positive excess forces a positive pode size.**  Near `(ε₀,a₀,b₀,0)` with `b₀ ≠ ε₀` the
...
/-- Inside the positive-excess window the family's graphon is the bipodal graphon with its
