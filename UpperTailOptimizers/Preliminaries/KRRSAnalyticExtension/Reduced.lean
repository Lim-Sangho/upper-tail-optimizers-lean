...
live in `Preliminaries/KRRSAnalyticExtension/Inputs.lean`.

## Contents

* `S0`, `dS0` — the paper's `S₀(z) = -½[z log z + (1-z) log(1-z)]` and its derivative.
...
/-- The paper's `S₀(z) = -½[z log z + (1-z) log(1-z)]`, i.e. half the Shannon entropy.
...
/-- Strict convexity of `z ↦ z^d` in the form used in paragraph 2: `𝒟_ε(b) > 0` for
`b ≠ ε`.  This is `pow_convex_gap_pos` of `NonexceptionalEndpoint/QuadraticGrowth/PowBounds.lean` restated in the
appendix's notation. -/
...
and `hasDerivAt_Shat_c_zero` (`Preliminaries/KRRSAnalyticExtension/SCalc.lean`). -/
...
`hasDerivAt_Qmap_c` and `hasDerivAt_Shat_c_zero` (`Preliminaries/KRRSAnalyticExtension/SCalc.lean`). -/
...
/-- The paper's `𝒯̂(ε,a,b,c) = t(H, W(a,b,Q(ε,a,b,c),c))`. -/
...
`Preliminaries/KRRSAnalyticExtension/Chart.lean` and the appendix's `Ŝ` agree. -/
