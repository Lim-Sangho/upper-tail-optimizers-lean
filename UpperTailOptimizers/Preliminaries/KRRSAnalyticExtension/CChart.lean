...
* **uniqueness** — paragraphs 3 and 4 pin the pode size of the KRR–S maximizer by "the
  local uniqueness assertion in `eq:krrs-density-constraint`";
* the identity **`C(ε,a,b,0) = 0` for every nearby `(ε,a,b)`**, not merely at the base
  point (also part of `eq:krrs-density-constraint`).
...
`Preliminaries/KRRSAnalyticExtension/TCalc.lean`) — so the inverse is written down explicitly and the two round-trip
...
`Φ(ε,a,b,c)` is the excess `ϑ`, and the first three are carried along unchanged. -/
...
* **local uniqueness** — a solution `c ∈ S` of the constraint with excess `ϑ ∈ T` equals
  `C ε a b ϑ`.  This is `Set.InjOn (CPhi H) S` from `analytic_inverse`, and it is what
  paragraphs 3 and 4 use to pin the pode size of the KRR–S maximizer. -/
