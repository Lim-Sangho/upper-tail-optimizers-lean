...
Paragraph 4 repeats the argument for the maximizer `W⁺_{ε,ϑ}`.

This file formalises that passage up to "interior local maximizer" — the bridge from the
graphon layer to the finite-dimensional one.  It turns the statement "`W` maximises the
entropy among all graphons with edge density `ε` and `H`-density `ε^m + ϑ`, and `W` is (up to
relabelling) the bipodal graphon with parameters `(a, b, Q(ε,a,b,c), c)`, `c = C(ε,a,b,ϑ)`" into
...
The step from there to `F₁ = F₂ = 0` starts from the elementary calculus fact
`partials_eq_zero_of_isLocalMax` at the end of the file (the two partial derivatives vanish)
and is completed by `F_eq_zero_of_isLocalMax` of `Preliminaries/KRRSAnalyticExtension/SigmaLink.lean`.

## Contents

* `isLocalMax_reduced_entropy` — **paragraph 3** (and its repetition in paragraph 4):
  graphon maximality transports to `IsLocalMax` of the reduced entropy.
* `partials_eq_zero_of_isLocalMax` — the two partial derivatives of a function of two real
  variables vanish at a local maximum; the calculus half of "is an interior local maximizer
  of `Σ(ε₀,·,·,ϑ)` and therefore satisfies `𝓕₁ = 𝓕₂ = 0`".
