...
* **`Rda_zero_indep_a`** — `Rda(ε,·,b,0)` is *constant*.  This is the sentence of the
  justification of `eq:krrs-density-expansion` that reads: an assignment with exactly two
  vertices in the first pode contains at most one factor `a`, because `H` is simple and those
  two vertices support at most one edge; assignments with at least three vertices in the first
  pode are divisible by `c³`.  It is what makes the coefficient `B₁(ε,b)` of `a` in
...
* `bothCount_le_one_of_trueCount_eq_two` — simplicity of `H`: two marked vertices support
  at most one edge;
* `Rda_zero_indep_a` — the transfer of the two preceding facts to `Preliminaries/KRRSAnalyticExtension/TCalc.lean`'s
  `Rda`;
* `partialB`, `Rdb`, `analyticAt_Rdb`, `hasDerivAt_That_b` — the `b`-derivative, mirroring
  the `partialA`/`ThatRem` pattern of `Preliminaries/KRRSAnalyticExtension/TCalc.lean`.
...
that defines `edgeProd` in `NonexceptionalEndpoint/QuadraticGrowth/TDensityExpansion.lean`. -/
...
/-- The `a`-partial of `Preliminaries/KRRSAnalyticExtension/TCalc.lean`'s `ThatRem` at `c = 0` is the explicit sum
...
parameters `(ε,a,b,c)` — the `b`-analogue of `partialA` of `Preliminaries/KRRSAnalyticExtension/TCalc.lean`. -/
...
`hasDerivAt_That_a` of `Preliminaries/KRRSAnalyticExtension/TCalc.lean` in the `b`-direction: the `c`-normalised
`b`-stationarity function `F2` of `Preliminaries/KRRSAnalyticExtension/Stationarity.lean` divides by this one power of `c`. -/
