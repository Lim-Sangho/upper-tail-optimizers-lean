...
and then evaluates the system and its `(a,b)`-Jacobian at `ϑ = 0`.

This file normalises by powers of the pode size `c` instead of the excess `ϑ`.  Those
quotients are available *exactly*, because
`Preliminaries/KRRSAnalyticExtension/SCalc.lean` and `Preliminaries/KRRSAnalyticExtension/TCalcAB.lean` compute the four partial derivatives in already
factored form:

`∂_aŜ = c²(S₀'(a) - S₀'(Q))`,  `∂_bŜ = 2c(1-c)(S₀'(b) - S₀'(Q))`,
`∂_a𝒯̂ = c²·Rda`,               `∂_b𝒯̂ = c·Rdb`.

So `F1` and `F2` below are defined with no division at all; they are the naive
Wronskian-style stationarity expressions divided by `c²`, resp. `c`, and satisfy
`∂_aΣ = c²F₁/∂_c𝒯̂`, `∂_bΣ = cF₂/∂_c𝒯̂` (`Preliminaries/KRRSAnalyticExtension/SigmaLink.lean`).  Their evaluations at `c = 0`
below are the counterparts of the paper's evaluations of `𝓕₁`, `𝓕₂` at `ϑ = 0`, up to
positive normalising factors.

## Contents

* `partialC` — the `c`-partial in the style of `partialA` (`Preliminaries/KRRSAnalyticExtension/TCalc.lean`) and
  `partialB` (`Preliminaries/KRRSAnalyticExtension/TCalcAB.lean`), together with its two boundary values
...
* `F1`, `F2` — the desingularized system, the `c`-normalised counterparts of
...
parameters `(ε,a,b,c)` — the `c`-analogue of `partialA` (`Preliminaries/KRRSAnalyticExtension/TCalc.lean`) and `partialB`
(`Preliminaries/KRRSAnalyticExtension/TCalcAB.lean`). -/
...
This is the nondegeneracy that `Preliminaries/KRRSBipodality/Optimal.lean` feeds to `F_eq_zero_of_maximizer`
(`Preliminaries/KRRSAnalyticExtension/MaximizerStationary.lean`). -/
...
/-- **`eq:krrs-small-block-stationarity`**, in the `c`-normalised desingularized form used here
(the paper's `𝓕₁` is `∂_aL/ϑ`):
...
/-- **`eq:krrs-cross-block-stationarity`**, in the `c`-normalised desingularized form used here
(the paper's `𝓕₂` is `∂_bL`):
...
`Preliminaries/KRRSAnalyticExtension/PsiNondeg.lean`.  This is the quotient-free form of the boundary identity
...
/-- **`eq:krrs-jacobian-aa`**: `∂_aF₁|_{c=0} = S₀''(a)·𝒜(ε,b)` (the paper's entry, in its
normalisation, is `S₀''(a₀)/𝒜(ε₀,z₀)²`; both are nonzero for the same reason).
...
/-- **Paragraph 3's Jacobian conclusion.**  At the KRR–S base point — `b` an interior maximizer of
...
* `F₂(ε,a,b,0) = 0`, the second half of `eq:krrs-boundary-stationarity` (the paper's "by
  `eq:krrs-quotient-boundary` and the critical-point property of `z₀`"; the first half,
  `F₁ = 0`, is arranged here by the choice of `a₀` in `Preliminaries/KRRSAnalyticExtension/BasePoint.lean`);
