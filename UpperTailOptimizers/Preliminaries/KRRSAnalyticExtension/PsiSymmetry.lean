...
`thm:krrs-cross-density` of `paper/paper.tex` records Kenyon–Radin–Ren–Sadun
...
the numerator of `∂_zψ_d` (`Preliminaries/KRRSAnalyticExtension/PsiNondeg.lean`), the conclusion is the unconditional
...
Combined with the uniqueness of the critical point — which the symmetry does not give, and
which is proved separately as `crit_unique` (`Preliminaries/KRRSAnalyticExtension/PsiUnique.lean`) by an argument replacing
the continuity argument in a *real* parameter `k` that Kenyon–Radin–Ren–Sadun's Step 4
sketches — the symmetry yields the involution property:
if `ζ_d(ε)` is the unique critical point of `ψ_d(ε,·)`, then `ε` is a critical point of
`ψ_d(ζ_d(ε),·)`, hence equals `ζ_d(ζ_d(ε))`.  `Preliminaries/KRRSAnalyticExtension/Inputs.lean` performs that derivation,
