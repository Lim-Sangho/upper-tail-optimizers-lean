...
/-!
# Convexity, monotonicity and the zero set of `J_p` (analysis layer for Section 4.1)

This file packages the elementary analytic facts about the binomial relative entropy
`J_p` that Section 4.1 of `paper/paper.tex` (the local reduction) needs but that were not previously isolated:
...
`continuousOn_Jp_Icc` of `Preliminaries/Graphons/Basic.lean`.
-/
