...
This file records the cut-metric notions of Section 2 of `paper/paper.tex`.
...
* Attainment of the infimum is the **theorem**
  `feasible_attains` in `Preliminaries/Graphons/Attainment.lean`, derived from cut-compactness and the
  lower semicontinuity of `I_p`.
* The Chatterjee–Varadhan graphon LDP (`thm:graphon-large-deviations` of
  `paper/paper.tex`) is outside the formalised scope. The unconstrained
...
`thm:graphon-large-deviations` of `paper/paper.tex` (the Chatterjee–Varadhan graphon LDP) is **not**
