...
`Preliminaries/Graphons/Basic.lean` carries the *entropy integrand* `entIntegrand t = t log t + (1-t) log(1-t)`,
...
the two-block evaluation of the edge, entropy and `t`-densities-style computations (`Preliminaries/Graphons/BipodalBridge.lean`), the analytic block entropy
of `sec:nonexceptional-quadratic-growth`, and the reduced entropy `S_0 = H/2` of the KRR--S appendix.  Analyticity of
