...
`A`, `q₁₂` across, `q₂₂` outside.  `Graphon/Bipodal.lean` gives the pointwise value
...
* `bipodalGraphon_entropy` — `Ent(W) = ½(H(q₁₁)a² + 2H(q₁₂)a(1-a) + H(q₂₂)(1-a)²)`, with `H` the
  Shannon entropy of `Graphon/ScalarEntropy.lean` (the `Graphon.entropy` normalisation carries `-½∫`);
...
`sec:nondegeneracy` (`Nondegeneracy/BipodalUpper.lean`, `Nondegeneracy/QuadraticUpper.lean`), the
`t`-density expansion of `Nondegeneracy/TDensityExpansion.lean`, the chart of the KRR--S appendix
(`KRRS/Chart.lean`), and the rank-one family of Section 7 (`SingularEndpoint/RankOne.lean`,
`SingularEndpoint/BipodalTransport.lean`).
