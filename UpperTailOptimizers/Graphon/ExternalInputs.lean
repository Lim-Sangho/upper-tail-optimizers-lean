...
This file records the external results of Sections 1–2 of `paper/bipodal_optimizer.tex` that are
taken **as axioms**:
...
used by `boundary_uniqueness`.  (The Chatterjee–Varadhan graphon LDP, Theorem 1.3, is
neither assumed nor used anywhere in the formalisation; see the prose in
`Graphon/CutMetric.lean`.)
...
As for `lubetzkyZhao` and the two Kenyon–Radin–Ren–Sadun inputs, the paper's standing
