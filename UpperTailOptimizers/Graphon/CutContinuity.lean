...
Section 4's convergence-to-the-boundary lemma (`lem:boundary-convergence`) is driven by four standard
...
These are not re-proved here — they belong to Section 2 of `paper/bipodal_optimizer.tex` and the
cited literature — and are recorded **as axioms**; they are exactly the graph-limit facts
listed as assumed in the paper's "Lean formalization" paragraph.  (Attainment of the
feasible infimum is the **theorem** `feasible_attains` in
`Graphon/Attainment.lean`, derived from three of these four axioms.)  They are phrased
