...
Shared infrastructure for the whole formalisation of `paper/bipodal_optimizer.tex`; not itself a
statement of the paper.  Low-level counting lemmas for a `d`-regular graph `H` with `m = |E(H)|` edges, placed
below `Graphon/ExternalInputs.lean` so that every layer of the development can use them.

The one that matters for the axiom discipline is `two_le_card_edgeFinset`: the paper's
`thm:krrs-analytic-extension` (Kenyon–Radin–Ren–Sadun) is stated for graphs with `m ≥ 2`, so the Lean
axioms recording it must carry that hypothesis; `two_le_card_edgeFinset` discharges it at
every call site from the standing hypotheses `2 ≤ d`, `H` `d`-regular, `1 ≤ m`.  Without
it the axioms would assert the KRR–S conclusion in a regime (`m = 1`) that the cited
source does not cover.

`nonempty_of_edge`, `regular_handshake` and `degree_le_card_edges` were previously stated
in `Nondegeneracy/QuadraticUpper.lean`; they live here now and are re-used there.
-/
...
This is the hypothesis `m = |E(H)| ≥ 2` of `thm:krrs-analytic-extension` of `paper/bipodal_optimizer.tex`
(Kenyon–Radin–Ren–Sadun); it is *derivable* here, so recording it in the KRR–S axioms
costs nothing at the call sites while keeping those axioms no stronger than the cited
source. -/
