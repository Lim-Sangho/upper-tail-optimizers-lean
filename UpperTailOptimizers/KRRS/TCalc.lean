...
Paragraph 2 of Appendix A of `paper/bipodal_optimizer.tex` (Appendix A) expands the
...
* `tBipRem2`, `tBipRem_eq_sq_mul` — the small-block remainder of
  `Nondegeneracy/TDensityExpansion.lean` carries a factor `c²` *exactly*: every surviving
...
* `partialSlot`, `analyticAt_partialSlot`, `hasDerivAt_partialSlot` — the directional
  ...
  here, `partialB` (`KRRS/TCalcAB.lean`) and `partialC` (`KRRS/Stationarity.lean`).
...
jointly real-analytic (mirrors `analyticAt_bipTd` of `KRRS/Chart.lean`). -/
...
of `Nondegeneracy/TDensityExpansion.lean` with its remainder written as `c²` times a
polynomial. -/
...
`c`-slot of the Jacobian in paragraph 2 and the factor multiplying `S₀''(a)` in
paragraph 3.  Strict convexity of `u ↦ u^d` gives `𝒟_ε(b) > 0` (`Dfun_pos`), and a graph
with an edge has a vertex (`nonempty_of_edge`), so `n_d = |V| > 0`. -/
...
`partialA` (below), `partialB` (`KRRS/TCalcAB.lean`) and `partialC`
(`KRRS/Stationarity.lean`). -/
...
`KRRS/SigmaLink.lean` uses the graph of the chart `C`. -/
