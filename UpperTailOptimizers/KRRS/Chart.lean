...
Appendix A of `paper/bipodal_optimizer.tex` (Appendix A) works with the bipodal parameter
vector
...
layer, so that the reduced calculus of `KRRS/Reduced.lean` onwards can be run purely in
θ-coordinates and the derivation in `KRRS/Main.lean` never has to touch a measure-theoretic
argument.
...
`tBip H a s q c` of `Nondegeneracy/TDensityExpansion.lean`.  It deliberately does **not**
...
`Nondegeneracy/TDensityExpansion.lean` (§5.2 of `paper/bipodal_optimizer.tex`). -/
...
`c ≠ 0` step of `KRRS/Main.lean` (`c = 0` would force `τ = ε^m`). -/
...
lemmas are deliberately role-neutral: downstream of `KRRS/Reduced.lean` the same tuple
