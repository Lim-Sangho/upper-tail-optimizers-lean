...
/-!
# "Local maximality therefore gives `F₁ = F₂ = 0`" (Appendix A, paragraph 4)

Paragraph 4 of Appendix A of `paper/bipodal_optimizer.tex` (Appendix A) reads, in part:
...
`KRRS/LocalMax.lean` supplies the first sentence (`isLocalMax_reduced_entropy`) and the
...
in terms of the desingularized stationarity functions `F₁`, `F₂` of `KRRS/Stationarity.lean`
...
positivity of the pode size `c` (the strict positive surplus `ϑ > 0`) cancels the powers of
`c`.  The nonvanishing `∂_c𝒯̂ ≠ 0` holds near the base point because `∂_c𝒯̂|_{c=0} = 𝒜(ε,b)`
(`partialC_That_zero`) and `𝒜(ε₀,z₀) > 0` (`Afun_pos`).
...
* `F_eq_zero_of_isLocalMax` — **the payoff paragraph 4 consumes**: local maximality of `Σ`
  gives `F₁ = F₂ = 0`.

Two groups of helpers are file-private: the four `(a,b)`-partials of `KRRS/SCalc.lean` and
`KRRS/TCalc.lean`/`KRRS/TCalcAB.lean` restated in the Fréchet `partialA`/`partialB`
...
`KRRS/CChart.lean`), so nothing here depends on the implicit function theorem.
-/
...
/-- `∂_a𝒯̂ = c²·Rda` in `partialA` form: `hasDerivAt_That_a` of `KRRS/TCalc.lean` transported
...
/-- `∂_b𝒯̂ = c·Rdb` in `partialB` form: `hasDerivAt_That_b` of `KRRS/TCalcAB.lean`
...
This is the two-variable chain rule of paragraph 4, obtained by composing the Fréchet
derivative of `Φ` with the curve `x ↦ (ε,x,b,C(ε,x,b,ϑ))`, whose velocity is `(0,1,0,∂_aC)`;
...
`isLocalMax_reduced_entropy` of `KRRS/LocalMax.lean` delivers from graphon maximality — then
...
The two hypotheses that make the division legitimate are exactly the paper's:

* `0 < c`, the strict positive surplus: it cancels the desingularizing powers `c²` and `c`;
...
theorem F_eq_zero_of_isLocalMax {V : Type*} [Fintype V] [DecidableEq V] (H : SimpleGraph V)
  ...
  -- The two derivatives of `Σ` and the calculus step of `KRRS/LocalMax.lean`.
