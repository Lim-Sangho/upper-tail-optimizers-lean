...
Appendix A of `paper/bipodal_optimizer.tex` opens with the sentence

> We use only the statements of \[KRR–S, Theorems 1.1 and 3.3\].

Neither statement is assumed in this development.  Theorem 3.3 of Kenyon–Radin–Ren–Sadun,
...
the cross density `ζ_d` is constructed and all four of its properties are proved, in
`KRRS/PsiSymmetry.lean`, `KRRS/PsiRatio.lean`, `KRRS/PsiIntegral.lean`, `KRRS/PsiCritical.lean`,
`KRRS/PsiUnique.lean`, `KRRS/PsiCompare.lean` and `KRRS/PsiExists.lean`, following the source's
own Steps 1–5.  Their Theorem 1.1, the bipodality of the fixed-density entropy maximizer, is
proved for `d`-regular `H` in `UpperTailOptimizers/Bipodality/` (`KRRSFamily.isOptimal`) and
enters the appendix through `KRRS/FamilyCore.lean` and `KRRS/Main.lean`.
...
with `zetaFun` of `KRRS/PsiExists.lean` as the selector.  It is kept as a structure so that the
...
structure KRRSZeta (d : ℕ) where
  ...
  these four below, from Steps 1–4 of the source's own proof (`KRRS/PsiSymmetry.lean`,
  `KRRS/PsiCritical.lean`, `KRRS/PsiUnique.lean`). -/
...
follows the source's Steps 1–5, with Step 4 replaced — see `KRRS/PsiCritical.lean`. -/
