...
Paragraph 3 of Appendix A of `paper/bipodal_optimizer.tex` (Appendix A) substitutes the pode
size chart `c = C(ε,a,b,ϑ)` of `eq:krrs-density-constraint` into the desingularized stationarity
system `F₁, F₂` (`eq:krrs-small-block-stationarity`, `eq:krrs-cross-block-stationarity`) and solves the resulting
`2 × 2` system for `(a,b)` as real-analytic functions of `(ε,ϑ)` on a **two-sided** window
around `(ε₀, 0)`.

## Why the inverse function theorem again

As in `KRRS/CChart.lean`, `analytic_inverse` is used rather than a bare implicit function
...
which are exactly `eq:krrs-jacobian-aa`, `eq:krrs-jacobian-ba`, `eq:krrs-jacobian-bb` as evaluated by
...
The first-equation base condition `F₁(ε₀,a₀,b₀,0) = 0` is likewise a hypothesis — it is the
defining property of the appendix's degenerate first pode density `a₀ = q₁₁⁰(ε₀)`, and
paragraph 3 supplies only `F₂(ε₀,a₀,b₀,0) = 0` on its own.
-/
