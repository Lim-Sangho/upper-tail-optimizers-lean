...
This file formalises paragraph 1 of Appendix A of `paper/bipodal_optimizer.tex`
(Appendix A), whose conclusion is `eq:krrs-scalar-nondegeneracy`,
`∂_z²ψ_d(ε₀,z₀) < 0`.
...
Exactly as in the appendix, the argument is run on the *auxiliary* function
...
The last identity, combined with `h''(z) = 0`, forces `z = (d-1)/d = r_*`, which is
...
there, then `f₁(x) = 0`, `f₂(x) ≤ 0`, and — the "no first nonvanishing derivative of odd
order at an interior maximum" step — `f₂(x) = 0` forces `f₃(x) = 0`. -/
private theorem localmax_derivs {f f₁ f₂ f₃ : ℝ → ℝ} {A B x : ℝ}
  ...
  -- Step 4: the second derivative is nonpositive.
  have h2 : f₂ x ≤ 0 := by
    by_contra hcon
    push Not at hcon
    ...
    linarith
  refine ⟨h1, h2, ?_⟩
  -- Step 5: a vanishing second derivative forces a vanishing third derivative.
  intro h2z
  by_contra hcon
...
/-- **Appendix A, paragraph 1** `eq:krrs-scalar-nondegeneracy`, in the quotient-free form
...
theorem krrs_scalar_nondegenerate {d : ℕ} (hd : 2 ≤ d) {ε z : ℝ}
  ...
  -- Step 1: the auxiliary function `h(w) = 𝒩(w) - λ𝒟(w)` is `≤ 0` on `(0,1)`, with
  -- value `0` at `z`.
  ...
  refine ⟨?_, ?_⟩
  · -- Step 3: the critical-point identity.
    unfold Wr
