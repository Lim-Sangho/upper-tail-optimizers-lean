...
This file carries out the entropy half of paragraph 2 of Appendix A of `paper/bipodal_optimizer.tex`
(Appendix A): the *exact* partial derivatives of the edge-constrained entropy

`Ŝ(ε,a,b,c) = c²S₀(a) + 2c(1-c)S₀(b) + (1-c)²S₀(Q(ε,a,b,c))`,

where `Q` is the edge-density solve `eq:krrs-edge-constraint` of `KRRS/Reduced.lean`.
...
* `hasDerivAt_Shat_a`, `hasDerivAt_Shat_b` — the exact formulas
  ...
  desingularises the stationarity equation in paragraph 3.
...
* `analyticAt_Shat` — joint real-analyticity of `Ŝ` in `(ε,a,b,c)`, the input for the
  analytic implicit function theorem used to build `C` of `eq:krrs-density-constraint`.
-/
...
order `c²`, which is what desingularises the stationarity equation in paragraph 3. -/
