...
`boundaryExcess_chart` (`Nondegeneracy/AnalyticExcess.lean`) extends the boundary excess across
...
`thm:positive-second-variation` of `paper/bipodal_optimizer.tex` states instead a **single**
real-analytic `G` on an open neighbourhood `𝒩` of `K × {0}`, for a compact `K`.  This file
performs that patching, which is the step the paper carries out with the identity theorem:

> the local parameter maps agree on the open set where `ϑ > 0` … The real-analytic identity
> theorem then shows that the local extensions also agree at `ϑ = 0` and for small negative `ϑ`.
> They therefore patch to a single analytic map on a neighbourhood of `K × {0}`.
...
limit `lim_{δ↓0} 2G_r(δ)/δ²` of `Nondegeneracy/AnalyticExcess.lean`, and the extension is proved
...
subcover of `K`; it depends on that subcover, exactly as the paper's `G` depends on its own
finite subcover, and any two choices agree on the common domain by the same identity-theorem
argument used to build them. -/
...
* **(iii)** `∂²_δG(r,0) = A_H(r)` and `|∂²_δG(r,δ) - A_H(r)| ≤ C₂|δ|` for `|δ| < δ̄`,
  with `C₂` depending only on `K`  (`eq:boundary-excess-curvature`);
...
theorem positive_second_variation {d : ℕ} (hd : 2 ≤ d) (M : LZBoundaryArc d)
    ...
    ∃ (δbar a₀ C₂ Ccub : ℝ) (N : Set (ℝ × ℝ)) (G : ℝ × ℝ → ℝ),
      0 < δbar ∧ 0 < a₀ ∧ 0 ≤ C₂ ∧ 0 ≤ Ccub ∧ IsOpen N ∧
      (∀ r ∈ K, ∀ δ : ℝ, |δ| < δbar → (r, δ) ∈ N) ∧
      (∀ p ∈ N, AnalyticAt ℝ G p) ∧
      (∀ r ∈ K, ∀ δ : ℝ, 0 < δ → δ < δbar → G (r, δ) = boundaryExcess H M r δ) ∧
  ...
  refine ⟨min δ₁ δ₀, CA, C₂, Ccub, N, G, lt_min hδ₁ hδ₀, hCA, hC₂, hCcub, hNopen,
    ...
    hd2G0,
    fun r hr δ hδ => hcurv r hr δ (lt_of_lt_of_le hδ (min_le_left _ _)),
