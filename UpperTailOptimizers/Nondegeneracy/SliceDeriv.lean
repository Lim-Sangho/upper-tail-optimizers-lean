...
`Nondegeneracy/AnalyticTools.lean` so that the bipodality development
(`Bipodality/SliceBounds.lean`) can use them without importing the Section 5 chain, which sits
downstream of `KRRS/Main.lean`.
