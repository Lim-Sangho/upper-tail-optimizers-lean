...
* the `δ`-slice derivative `dDelta`, its analyticity and the uniform `O(δ)` bound
  `exists_lipschitz_bound_of_boundary_zero` live in `NonexceptionalEndpoint/QuadraticGrowth/SliceDeriv.lean`;
...
-- `Preliminaries/KRRSAnalyticExtension/Reduced.lean`: importing it from the KRR–S development would otherwise close the
-- cycle `Preliminaries/KRRSAnalyticExtension/Reduced → AnalyticTools → … → LocalReduction → Preliminaries/KRRSAnalyticExtension/Main → … → Preliminaries/KRRSAnalyticExtension/Reduced`.
