...
`NonexceptionalEndpoint/QuadraticGrowth/BoundaryExcess.lean` without any Kenyon–Radin–Ren–Sadun input.  The
missing analytic structure enters through `krrs_analytic_extension`
(the paper's `thm:krrs-analytic-extension`, proved in `Preliminaries/KRRSAnalyticExtension/Extension.lean` and `Preliminaries/KRRSAnalyticExtension/Main.lean`,
with the Kenyon–Radin–Ren–Sadun theorems it uses proved in `Preliminaries/KRRSAnalyticExtension/Psi*` and
`Preliminaries/KRRSBipodality/`): near each
...
The chart is exported in three forms: `boundaryExcess_chart_full_zeta`, which carries everything
Section 4 consumes (including the four Kenyon–Radin–Ren–Sadun parameter maps, the identification
`q₁₂(r,0) = ζ_d(r)` of their boundary cross value, and the fact that near every `(r,0)` these maps
are the analytically extended KRR–S parameter maps of `krrs_analytic_extension`), its projection
`boundaryExcess_chart_full` without the last two clauses, and the trimmed form
...
/-- **An off-diagonal maximizer of `ψ_d(ε,·)` is `ζ_d(ε)`.**  If `u ∈ (0,1) ∖ {ε}` maximizes
`ψ_d(ε,·)` over `(0,1) ∖ {ε}`, then `u` is an interior local maximum, hence a critical point:
`W(ε,u) = 0`, and `u = ζ_d(ε)` by uniqueness of the off-diagonal critical point
(`zetaFun_eq_of_crit`).  This converts the defining property of the cross value in
`kenyonRadinRenSadunAnalytic` and `krrs_rectangle` into the identity `q₁₂(ε,0) = ζ_d(ε)`. -/
theorem eq_zetaFun_of_psiD_isMax {d : ℕ} (hd : 2 ≤ d) {ε u : ℝ} (hε : ε ∈ Set.Ioo (0:ℝ) 1)
    (hu : u ∈ Set.Ioo (0:ℝ) 1) (hne : u ≠ ε)
    (hmax : ∀ w ∈ Set.Ioo (0:ℝ) 1, w ≠ ε → psiD d ε w ≤ psiD d ε u) :
    u = zetaFun d ε := by
  have hloc : IsLocalMax (fun w => psiD d ε w) u := by
    have hnb : ∀ᶠ w in 𝓝 u, w ∈ Set.Ioo (0:ℝ) 1 ∧ w ≠ ε :=
      Filter.Eventually.and (isOpen_Ioo.mem_nhds hu) (isOpen_ne.mem_nhds hne)
    filter_upwards [hnb] with w hw
    exact hmax w hw.1 hw.2
  have h0 := hloc.hasDerivAt_eq_zero (hasDerivAt_psiD hd hε.1 hu.1 hu.2 hne)
...
/-- **The local analytic chart of the boundary excess, full form with the KRR–S parameter maps.**
This is `boundaryExcess_chart_full` together with two more clauses about the chart's four
Kenyon–Radin–Ren–Sadun parameter maps `q₁₁, q₁₂, q₂₂, c`, which are functions of
`(ε, ϑ) = (e, t_H - e^m)`:

* the boundary cross value is the KRR–S cross density, `q₁₂(r,0) = ζ_d(r)` for `|r - r₀| < w`;
* they are the analytically extended KRR–S parameter maps of `thm:krrs-analytic-extension` on
  `{|ε - r₀| < w} × (-Δ, Δ)`: analytic at every `(ε, ϑ)` with `|ε - r₀| < w`, `|ϑ| < Δ`, and for
  `0 < ϑ = τ - ε^m < Δ` the entropy maximizer at `(e, t_H) = (ε, τ)` is unique up to relabelling,
  bipodal, and equal a.e. to the two-block graphon with first block `[0, c(ε,ϑ)]`,
  `0 < c(ε,ϑ) < 1/2`, and densities `q₁₁(ε,ϑ), q₁₂(ε,ϑ), q₂₂(ε,ϑ)`.

The maps are those of `krrs_analytic_extension` at `ε₀ = r₀`, which also gives `q₁₂(ε,0) = ζ_d(ε)`.
`rmk:bipodal-parameter-expansions` needs the first clause to state its expansions with `ζ_d(r)`
itself, and the second to identify `q₁₁(r,0)` with the value at `(r,0)` of the analytically
extended KRR–S parameter `q₁₁`. -/
theorem boundaryExcess_chart_full_zeta {d : ℕ} (hd : 2 ≤ d) (M : LZBoundaryArc d)
      ...
      (∀ r δ : ℝ, |r - r₀| < w → 0 < δ → δ < ρ →
        ...
        ∃ W : Graphon, W.edgeDensity = r - δ ∧
          ...
          (∀ᵐ z ∂gμ, W.toFun z.1 z.2 = bipodalValue
            ...
            (q11 (r - δ) (r ^ H.edgeFinset.card - (r - δ) ^ H.edgeFinset.card))
            (q12 (r - δ) (r ^ H.edgeFinset.card - (r - δ) ^ H.edgeFinset.card))
            (q22 (r - δ) (r ^ H.edgeFinset.card - (r - δ) ^ H.edgeFinset.card)) z)) ∧
      (∀ r : ℝ, |r - r₀| < w → q12 r 0 = zetaFun d r) ∧
      (∃ Δ : ℝ, 0 < Δ ∧ ∀ r : ℝ, |r - r₀| < w →
        (∀ ϑ : ℝ, |ϑ| < Δ →
          AnalyticAt ℝ (fun x : ℝ × ℝ => q11 x.1 x.2) (r, ϑ) ∧
          AnalyticAt ℝ (fun x : ℝ × ℝ => q12 x.1 x.2) (r, ϑ) ∧
          AnalyticAt ℝ (fun x : ℝ × ℝ => q22 x.1 x.2) (r, ϑ) ∧
          AnalyticAt ℝ (fun x : ℝ × ℝ => cc x.1 x.2) (r, ϑ)) ∧
        ∀ τ : ℝ, 0 < τ - r ^ H.edgeFinset.card → τ - r ^ H.edgeFinset.card < Δ →
          ∃ W : Graphon, W.edgeDensity = r ∧ W.tDensity H = τ ∧
            ...
            IsBipodal W ∧
            cc r (τ - r ^ H.edgeFinset.card) ∈ Set.Ioo (0:ℝ) (1 / 2) ∧
            (∀ᵐ z ∂gμ, W.toFun z.1 z.2
              = bipodalValue (Set.Icc 0 (cc r (τ - r ^ H.edgeFinset.card)))
                  (q11 r (τ - r ^ H.edgeFinset.card))
                  (q12 r (τ - r ^ H.edgeFinset.card))
                  (q22 r (τ - r ^ H.edgeFinset.card)) z)) := by
  classical
  obtain ⟨hpc0, hpcr, hr₀1, _, _, _⟩ := M.ordering r₀ hr₀U
  have hr₀0 : 0 < r₀ := lt_trans hpc0 hpcr
  obtain ⟨U, Δ, q11, q12, q22, cc, hUopen, hr₀mem, hUsub, hΔ0, hKR, hA11, hA12, hA22, hAcc,
    hbdry0, -, -⟩ :=
    krrs_analytic_extension H hd hreg (two_le_card_edgeFinset H hd hreg hm) hr₀0 hr₀1 hr₀ex
  -- ### The three consequences of `krrs_analytic_extension` used for the chart
  have hstrip : ∀ ε ∈ U, ∀ ϑ : ℝ, |ϑ| < Δ → ((ε, ϑ) : ℝ × ℝ) ∈ U ×ˢ Set.Ioo (-Δ) Δ :=
    fun ε hε ϑ hϑ => ⟨hε, abs_lt.mp hϑ⟩
  have hana : ∀ ε ∈ U,
      AnalyticAt ℝ (fun p : ℝ × ℝ => q11 p.1 p.2) (ε, 0) ∧
      AnalyticAt ℝ (fun p : ℝ × ℝ => q12 p.1 p.2) (ε, 0) ∧
      AnalyticAt ℝ (fun p : ℝ × ℝ => q22 p.1 p.2) (ε, 0) ∧
      AnalyticAt ℝ (fun p : ℝ × ℝ => cc p.1 p.2) (ε, 0) := by
    intro ε hε
    have h := hstrip ε hε 0 (by rw [abs_zero]; exact hΔ0)
    exact ⟨hA11 _ h, hA12 _ h, hA22 _ h, hAcc _ h⟩
  have hbdry : ∀ ε ∈ U, cc ε 0 = 0 ∧ q22 ε 0 = ε ∧ q11 ε 0 ∈ Set.Ioo (0:ℝ) 1 ∧
      q12 ε 0 ∈ Set.Ioo (0:ℝ) 1 ∧ q12 ε 0 ≠ ε ∧ q12 ε 0 = zetaFun d ε := by
    intro ε hε
    obtain ⟨h11, h12, hne, h22, hc⟩ := hbdry0 ε hε
    obtain ⟨hεI, -⟩ := hUsub hε
    exact ⟨hc, h22, h11, h12 ▸ zetaFun_mem hd hεI, h12 ▸ hne, h12⟩
  have hmaxi : ∀ ε ∈ U, ∀ τ : ℝ, 0 < τ - ε ^ H.edgeFinset.card →
      ...
      cc ε (τ - ε ^ H.edgeFinset.card) ∈ Set.Icc (0:ℝ) 1 ∧
      ∃ W : Graphon, W.edgeDensity = ε ∧ W.tDensity H = τ ∧
        (∀ W' : Graphon, W'.edgeDensity = ε → W'.tDensity H = τ →
          W'.entropy ≤ W.entropy) ∧
        (∀ᵐ z ∂gμ, W.toFun z.1 z.2
          = bipodalValue (Set.Icc 0 (cc ε (τ - ε ^ H.edgeFinset.card)))
              (q11 ε (τ - ε ^ H.edgeFinset.card))
              (q12 ε (τ - ε ^ H.edgeFinset.card))
              (q22 ε (τ - ε ^ H.edgeFinset.card)) z) := by
    intro ε hε τ h1 h2
    obtain ⟨W, hWe, hWt, hWmax, -, -, hc, h11, h12, h22, hWae⟩ := hKR ε hε τ h1 h2
    exact ⟨Set.Ioo_subset_Icc_self h11, Set.Ioo_subset_Icc_self h12,
      Set.Ioo_subset_Icc_self h22, ⟨hc.1.le, by linarith [hc.2]⟩, W, hWe, hWt, hWmax, hWae⟩
  ...
  have heq : ∀ r : ℝ, |r - r₀| < w → ∀ δ : ℝ, 0 < δ → δ < ρ →
    ...
    have hσ : entropyEnvelopeReal H (r - δ) (r ^ H.edgeFinset.card) = W.entropy :=
    ...
    -- assemble
    rw [boundaryExcess, reducedObjectiveReal, hσ, hWent, hGc_def]
  ...
  · intro r hr
    ...
    -- the maximizer property of `q₁₂(r,0)` is used in the last clause
    obtain ⟨b1, b2, b3, b4, b5, -⟩ := hbdry r hU2
    exact ⟨b1, b2, b3, b4, b5⟩
  · intro r δ hr hδ0 hδρ
    obtain ⟨-, -, -, hcIcc, hW⟩ := hmax r hr δ hδ0 hδρ
    exact ⟨hcIcc, hW⟩
  · -- `q₁₂(r,0) = ζ_d(r)`, a boundary value of `krrs_analytic_extension`
    intro r hr
    obtain ⟨-, -, ⟨-, hU2, -, -⟩⟩ :=
      hball _ (hmemball r 0 hr (by rw [abs_zero]; exact hρ0.le))
    exact (hbdry r hU2).2.2.2.2.2
  · -- near every `(r, 0)` the maps are the KRR–S parameter maps of `krrs_analytic_extension`
    refine ⟨Δ, hΔ0, fun r hr => ?_⟩
    obtain ⟨-, -, ⟨-, hU2, -, -⟩⟩ :=
      hball _ (hmemball r 0 hr (by rw [abs_zero]; exact hρ0.le))
    refine ⟨fun ϑ hϑ => ?_, fun τ h1 h2 => ?_⟩
    · have h := hstrip r hU2 ϑ hϑ
      exact ⟨hA11 _ h, hA12 _ h, hA22 _ h, hAcc _ h⟩
    · obtain ⟨W, hWe, hWt, hWmax, hWuniq, hWbip, hc, -, -, -, hWae⟩ := hKR r hU2 τ h1 h2
      exact ⟨W, hWe, hWt, hWmax, hWuniq, hWbip, hc, hWae⟩
...
`thm:nonexceptional-endpoint` and `rmk:bipodal-parameter-expansions` need.  The trimmed form used by the Section 4.2 consumers is `boundaryExcess_chart`. -/
theorem boundaryExcess_chart_full {d : ℕ} (hd : 2 ≤ d) (M : LZBoundaryArc d)
  ...
  obtain ⟨w, ρ, Mc, hw0, hρ0, hMc0, Gc, q11, q12, q22, cc,
    ...
    boundaryExcess_chart_full_zeta hd M H hreg hm hr₀U hr₀ex
  exact ⟨w, ρ, Mc, hw0, hρ0, hMc0, Gc, q11, q12, q22, cc,
    h1, h2, h3, h4, h5, h6, h7, h8, h9, h10, h11, h12⟩
...
which is what the Section 4.2 consumers (`boundaryExcess_taylor`) and the scalar half of
Section 4 need. -/
...
the step of `rmk:bipodal-parameter-expansions` where `thm:krrs-analytic-extension` and
analyticity at `(r,0)` give `c, q₂₂ - r, q₁₂ - z, q₁₁ - q₁₁⁰(r) = O(δ_*)`, made uniform on a
half-box, and is what that remark linearises around.  Purely a consequence of
...
`krrs_analytic_extension` (`thm:krrs-analytic-extension`):
`generalized_holder` and the four cut axioms, which enter through the existence of fixed-density
entropy maximizers in `Preliminaries/KRRSBipodality/Existence.lean`. -/
