...
/-!
# `thm:nonexceptional-endpoint`: optimizers near nonexceptional endpoints
...
/-- **Optimizer structure on one boundary arc** (`thm:nonexceptional-endpoint`, with the
...
theorem replica_symmetric_unique_global {d : ℕ} (hd : 2 ≤ d)
  ...
  obtain ⟨M, hrU, hrne⟩ := lz_boundary_arcs hd hr0 hr1 hrex
...
properties are included in `local_structure` and inherited by `thm:nonexceptional-optimizers`. -/
...
theorem analyticAt_AHGlobal_and_pos {d : ℕ} (hd : 2 ≤ d)
  ...
  obtain ⟨M, hrU, -⟩ := lz_boundary_arcs hd hr0 hr1 hrex
...
structure NonexceptionalOptimizers {V : Type*} [Fintype V] [DecidableEq V]
  ...
  smallBlock :
    ∃ ρ η : ℝ, 0 < ρ ∧ 0 < η ∧ ∃ c q11 q12 q22 : ℝ → ℝ → ℝ,
      ∀ r : ℝ, |r - r₀| < ρ →
        (∀ p : ℝ, pcGlobal d r - η < p → p < pcGlobal d r →
          c p r ∈ Set.Ioo (0:ℝ) (1 / 2) ∧
          ∃ W : Graphon, Feasible H r W ∧ W.Ip p = phiVar H p r ∧
            ∀ᵐ z ∂gμ, W.toFun z.1 z.2
              = bipodalValue (Set.Icc 0 (c p r)) (q11 p r) (q12 p r) (q22 p r) z) ∧
        Tendsto (fun p : ℝ => c p r) (𝓝[<] (pcGlobal d r)) (𝓝 0)
...
The arc is constructed internally. `thm:nonexceptional-optimizers` is the projection onto
`NonexceptionalOptimizers`, with no additional analytic or optimization argument.  The
`smallBlock` and `analyticFamily` fields are `bipodal_family_smallBlock` in global
coordinates. -/
theorem local_structure {d : ℕ} (hd : 2 ≤ d)
    {V : Type*} [Fintype V] [DecidableEq V] (H : SimpleGraph V) [DecidableRel H.Adj]
    (hreg : ∀ v, H.degree v = d) (hm : 1 ≤ H.edgeFinset.card)
    {r₀ : ℝ} (hr₀0 : 0 < r₀) (hr₀1 : r₀ < 1) (hr₀ex : r₀ ≠ rStar d) : LocalOptimizerStructure H d r₀ := by
  refine { coefficient := ?_, window := ?_, smallBlock := ?_, analyticFamily := ?_ }
  · intro r hr0 hr1 hrex
    exact analyticAt_AHGlobal_and_pos hd H hreg hm hr0 hr1 hrex
  · obtain ⟨M, hr₀U, hr₀ne⟩ := lz_boundary_arcs hd hr₀0 hr₀1 hr₀ex
  ...
  · obtain ⟨M, hrU, hrne⟩ := lz_boundary_arcs hd hr₀0 hr₀1 hr₀ex
    ...
    refine ⟨ρ, η, hρ, hη,
      fun p r => cc (r - Dl (p, r)) (r ^ H.edgeFinset.card - (r - Dl (p, r)) ^ H.edgeFinset.card),
      fun p r => q11 (r - Dl (p, r)) (r ^ H.edgeFinset.card - (r - Dl (p, r)) ^ H.edgeFinset.card),
      fun p r => q12 (r - Dl (p, r)) (r ^ H.edgeFinset.card - (r - Dl (p, r)) ^ H.edgeFinset.card),
      fun p r => q22 (r - Dl (p, r)) (r ^ H.edgeFinset.card - (r - Dl (p, r)) ^ H.edgeFinset.card),
      fun r hr => ⟨fun p hp_lo hp_hi => ?_, ?_⟩⟩
    · have hrU' : r ∈ M.U := (hbase r hr).1
      rw [pcGlobal_eq_pc hd M hrU'] at hp_lo hp_hi
      obtain ⟨-, -, -, -, -, -, -, -, -, -, -, -, -, -, -, -, -, ε, θ, hε, hθ, -, -, -, hcc, -, -,
        -, -, B, hB1, hB2, -, -, hBae⟩ := hfamily r hr p hp_lo hp_hi
      subst hε
      subst hθ
      exact ⟨hcc, B, hB1, hB2, hBae⟩
    · have hrU' : r ∈ M.U := (hbase r hr).1
