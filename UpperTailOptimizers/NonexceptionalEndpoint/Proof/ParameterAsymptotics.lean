...
* `c(p,r) = λ(p,r) / (2 D_d(r,z) A_H(r)) + O(λ²)`         (`eq:optimizer-block-size`),
* `q₂₂(p,r) = r - (z^d - r^d)/(d r^{d-1} D_d(r,z) A_H(r)) · λ(p,r) + O(λ²)`
  (`eq:optimizer-block-density`),
* `q₁₂(p,r) = z + O(λ)` and `q₁₁(p,r) = q₁₁⁰(r) + O(λ)`.

The derivation has four steps, mirrored by the five scalar lemmas of this file.

1. **`krrs_edge_linearization`** (`eq:bipodal-edge-linearization`).  The two-block edge-density
...
2. **`krrs_H_linearization`** (`eq:bipodal-density-linearization`).  The `H`-density constraint is
   linearised with the polynomial estimate `tBip_linearization` of Section 4.2; the only extra step
...
4. **`c_expansion_bound` / `q22_expansion_bound`**.  Substituting
   `δ_* = λ/A_H + O(λ²)` (`thm:nonexceptional-endpoint`) converts the `δ_*`-expansions into the `λ`-expansions.

`parameter_asymptotics_zeta` is the assembly, and `parameter_asymptotics` its projection onto the
original clauses.  Everything it needs about the Kenyon–Radin–Ren–Sadun
...
analytically extended KRR–S parameter maps of `thm:krrs-analytic-extension`, which makes `q11 r 0`
the paper's `q₁₁⁰(r)`, and it records `q12 r 0 = zetaFun d r`, so its expansions can
be read with `ζ_d(r)` itself (`bipodal_parameter_expansions` states them that way).  (ii) The
optimizer is described through the
...
/-- **Eq. `eq:bipodal-edge-linearization`.**  The two-block edge-density identity
...
/-- **Eq. `eq:bipodal-density-linearization`.**  With the constraint `t(H, ·) = r^m` imposed, the
polynomial estimate `tBip_linearization` of Section 4.2 says
...
The difference from the Section 4.2 model `tBip_linearization` is the last step: there the cross
...
/-- **Eq. `eq:optimizer-block-size`.**  Combining `δ = 2 D c + O(δ²)` (`krrs_eliminate`) with
`δ = λ/A + O(λ²)` (`thm:nonexceptional-endpoint`) and `δ ≤ Cd λ` gives `c = λ/(2DA) + O(λ²)`.  The uniform floor
...
/-- **Eq. `eq:optimizer-block-density`.**  Substituting the `c`-expansion into the companion relation
...
/-- **`rmk:bipodal-parameter-expansions` for one family, with `ζ_d(r)` and `q₁₁⁰(r)` identified.**
The clauses of `parameter_asymptotics`, obtained from `bipodal_family_smallBlock_zeta`, together
with: the boundary values `c(r,0) = 0`, `q₂₂(r,0) = r`, `q₁₁(r,0) ∈ (0,1)`, `q₁₂(r,0) = ζ_d(r)`; a
uniform floor `κ ≤ D_d(r, ζ_d(r))`; the identification of the maps, as functions of
`(ε, ϑ) = (e, t_H - e^m)`, with the analytically extended KRR–S parameter maps of
`thm:krrs-analytic-extension` near every `(r, 0)` (analytic at `(r, ϑ)` for `|ϑ| < Δ`, and
describing the unique entropy maximizer at `(e, t_H) = (r, r^m + ϑ)` for `0 < ϑ < Δ`), so that
`q₁₁(r,0)` is the paper's `q₁₁⁰(r)`; uniqueness of the optimizer up to relabelling; the smaller
first block `0 < c < 1/2`; and the relation `δ_* = 2 D_d(r, ζ_d(r)) c + O(δ_*²)` of
`krrs_eliminate`, with `δ_* = Δ(p,r)`.  One constant `C` serves all the bounds. -/
theorem parameter_asymptotics_zeta {d : ℕ} (hd : 2 ≤ d) (M : LZBoundaryArc d)
    ...
    ∃ ρ η C κ : ℝ, 0 < ρ ∧ 0 < η ∧ 0 ≤ C ∧ 0 < κ ∧
      ∃ (Dl : ℝ × ℝ → ℝ) (q11 q12 q22 cc : ℝ → ℝ → ℝ),
        (∀ r : ℝ, |r - r₀| < ρ →
          r ∈ M.U ∧ r ≠ rStar d ∧ 0 < r ∧ r < 1 ∧ cc r 0 = 0 ∧ q22 r 0 = r ∧
          q11 r 0 ∈ Set.Ioo (0:ℝ) 1 ∧ q12 r 0 = zetaFun d r ∧ q12 r 0 ≠ r ∧
          κ ≤ Dd d r (q12 r 0)) ∧
        (∃ Δ : ℝ, 0 < Δ ∧ ∀ r : ℝ, |r - r₀| < ρ →
          (∀ ϑ : ℝ, |ϑ| < Δ →
        ...
        ∀ r : ℝ, |r - r₀| < ρ → ∀ p : ℝ, M.pc r - η < p → p < M.pc r →
          ...
          ∃ ε θ : ℝ, ε = r - Dl (p, r) ∧
            ...
            cc ε θ ∈ Set.Ioo (0:ℝ) (1 / 2) ∧
            |Dl (p, r) - 2 * Dd d r (q12 r 0) * cc ε θ| ≤ C * Dl (p, r) ^ 2 ∧
  ...
  obtain ⟨ρ1, η, L, Cd, hρ10, hη0, hL0, hCd0, Dl, q11, q12, q22, cc, hzeta, ⟨Δ, hΔ0, hKRS⟩,
    hbdry, hmain⟩ := bipodal_family_smallBlock_zeta hd M H hreg hm hr₀U hr₀ex
  ...
  have hCE : C ≤ max C E3 := le_max_left _ _
  have hE3E : E3 ≤ max C E3 := le_max_right _ _
  refine ⟨ρ, η, max C E3, kappa, hρ0, hη0, le_trans hC0 hCE, hkappa0,
    Dl, q11, q12, q22, cc, ?_, ⟨Δ, hΔ0, fun r hrρ => hKRS r (hKmem r (hmemK r hrρ))⟩, ?_⟩
  · intro r hrρ
    obtain ⟨b1, b2, b3, b4, b5, b6, b7, -, b9, -, -⟩ := hbdry r (hKmem r (hmemK r hrρ))
    exact ⟨b1, b2, b3, b4, b5, b6, b7, hzeta r (hKmem r (hmemK r hrρ)), b9,
      hkappa r (hmemK r hrρ)⟩
  ...
  obtain ⟨hp0, hpr, hAH0, hlam0, hDl0, -, -, -, -, -, -, -,
    hedgeAll, hWstar, hDlexp, hDlC, -,
    ε, θ, hε, hθ, hI11, hI12, hI22, hccIoo, hLc, hL22, hL12, hL11,
    B, hBfeas, hBopt, hBe, hBt, hBae⟩ := hmain r hrρ1 p hp_lo hp_hi
  have hccIcc : cc ε θ ∈ Set.Icc (0:ℝ) 1 := ⟨hccIoo.1.le, by linarith [hccIoo.2]⟩
  ...
  -- ### The output
  have hsq : 0 ≤ lambdaDisp M p r ^ 2 := sq_nonneg _
  obtain ⟨Wstar, hWf, hWo, -, -, hWrel⟩ := hWstar
  refine ⟨hAH0, hlam0, hDl0,
    le_trans hDlexp (mul_le_mul_of_nonneg_right (le_trans hCdC hCE) hsq),
    ε, θ, hε, hθ,
    fun W hW hI => by rw [hedgeAll W hW hI]; exact hε.symm,
    ⟨Wstar, hWf, hWo, hWrel⟩, ⟨B, hBfeas, hBopt, hBae⟩, hccIoo,
    le_trans helim2 (mul_le_mul_of_nonneg_right hE3E (sq_nonneg _)),
    le_trans hcexp (mul_le_mul_of_nonneg_right (le_trans hCcC hCE) hsq), ?_, ?_, ?_⟩
  · have hrw : q22 ε θ - (r - (q12 r 0 ^ d - r ^ d)
      ...
      ring
    rw [hrw]
    exact le_trans hq22exp (mul_le_mul_of_nonneg_right (le_trans hCqC hCE) hsq)
...
* `Δ(p,r) = λ(p,r)/A_H(r) + O(λ²)`   (`thm:nonexceptional-endpoint`, restated here for reference);
* `c(p,r) = λ(p,r)/(2 D_d(r,z) A_H(r)) + O(λ²)`      — eq. `eq:optimizer-block-size`;
* `q₂₂(p,r) = r - (z^d - r^d)/(d r^{d-1} D_d(r,z) A_H(r))·λ(p,r) + O(λ²)`
  — eq. `eq:optimizer-block-density`;
* `q₁₂(p,r) = z + O(λ)` and `q₁₁(p,r) = q₁₁⁰(r) + O(λ)`.
      ...
      ∃ (Dl : ℝ × ℝ → ℝ) (q11 q12 q22 cc : ℝ → ℝ → ℝ),
        ...
        ∀ r : ℝ, |r - r₀| < ρ → ∀ p : ℝ, M.pc r - η < p → p < M.pc r →
          ...
          ∃ ε θ : ℝ, ε = r - Dl (p, r) ∧
            ...
            |q12 ε θ - q12 r 0| ≤ C * lambdaDisp M p r ∧
            |q11 ε θ - q11 r 0| ≤ C * lambdaDisp M p r := by
  obtain ⟨ρ, η, C, κ, hρ0, hη0, hC0, hκ0, Dl, q11, q12, q22, cc, hbase, -, hmain⟩ :=
    parameter_asymptotics_zeta hd M H hreg hm hr₀U hr₀ex
  refine ⟨ρ, η, C, hρ0, hη0, hC0, Dl, q11, q12, q22, cc, fun r hr => ?_, fun r hr p h1 h2 => ?_⟩
  · obtain ⟨-, -, -, -, -, -, -, -, hne, hκ⟩ := hbase r hr
    exact ⟨hne, lt_of_lt_of_le hκ0 hκ⟩
  · obtain ⟨hA, hl, hD, hDexp, ε, θ, hε, hθ, hedge, -, hB, -, -, hc, hq22, hq12, hq11⟩ :=
      hmain r hr p h1 h2
    exact ⟨hA, hl, hD, hDexp, ε, θ, hε, hθ, hedge, hB, hc, hq22, hq12, hq11⟩
