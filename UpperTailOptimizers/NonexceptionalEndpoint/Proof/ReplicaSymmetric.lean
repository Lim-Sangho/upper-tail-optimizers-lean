...
for every graphon `W`.  Both halves of the replica-symmetric side — the value `Φ_H = J_p(r)` and
the uniqueness — are read off from this. -/
...
symmetry-breaking side of `thm:nonexceptional-endpoint`. -/
...
theorem replica_symmetric_unique_of_pStar_le {d : ℕ} (hd : 2 ≤ d) {r : ℝ} (hr0 : 0 < r)
  ...
  have hp1 : p < 1 := lt_trans hpr hr1
  obtain ⟨a, ha⟩ := orientation_of_pStar_le hd hps hp1 hr0 hr1
  have hattain : ∀ hr' : r ∈ Set.Icc (0:ℝ) 1, (constGraphon r hr').Ip p = Jp p r :=
    fun hr' => Ip_constGraphon hr' p
  have hphi : phiVar H p r = Jp p r := by
    refine le_antisymm (phiVar_le_Jp H hp0 hp1 hr_mem) ?_
  ...
  have hphi_int : ∫ x, phi p d x ∂(powDistribution W d) = Jp p r := by
    rw [powLaw_integral_phi hd W, hWopt]
  have hdirac : powDistribution W d = Measure.dirac (r ^ d) :=
    (noFlatTie_of_pStar_le hd hps hp1 hr0 hr1 (powDistribution W d) inferInstance
      (powLaw_compl_Icc W d) hmom_eq).2 hphi_int
  filter_upwards [ae_pow_eq_of_powLaw_dirac (W := W) (d := d) (c := r ^ d) hdirac] with z hz
  rcases lt_trichotomy (W.toFun z.1 z.2) r with h | h | h
  · exact absurd hz (ne_of_lt (pow_lt_pow_left₀ h (W.nonneg' z.1 z.2) hdne))
  · exact h
  · exact absurd hz.symm (ne_of_lt (pow_lt_pow_left₀ h hr0.le hdne))
