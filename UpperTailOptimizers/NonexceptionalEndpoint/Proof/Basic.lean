...
/-!
# Preliminaries for Section 4.3: the log-odds displacement `λ` and the deficit rate `D_d`

This file collects the log-odds displacement recalled at the head of Section 4 of
`paper/paper.tex` and the deficit coefficient `D_d` of `rmk:bipodal-parameter-expansions`,
...
/-- **The first-order edge-density deficit rate** (eq. `eq:edge-deficit-coefficient`)
...
theorem reducedObjective_sub_Jp {V : Type*} [Fintype V] [DecidableEq V] (H : SimpleGraph V)
    ...
    (hr0 : 0 ≤ r) (hr1 : r ≤ 1) :
    reducedObjectiveReal H p ε τ - Jp p r
      = -2 * entropyEnvelopeReal H ε τ - entIntegrand r + (ε - r) * logOdds p := by
  have hlog : Real.log ((1 - p) / p) = Real.log (1 - p) - Real.log p :=
    Real.log_div (by linarith : (1:ℝ) - p ≠ 0) (ne_of_gt hp0)
  rw [reducedObjectiveReal, logOdds, hlog, Jp_eq_entIntegrand hp0 hp1 hr0 hr1]
  ring

/-- **The shift identity of Section 4.3.**  For every `p ∈ (0,1)` and every edge-density deficit
`δ`,
