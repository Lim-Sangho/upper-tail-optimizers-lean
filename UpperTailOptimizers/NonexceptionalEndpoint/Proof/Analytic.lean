...
/-!
# The analytic family of `thm:nonexceptional-endpoint` (Theorem 4.1 of `paper/paper.tex`)

`thm:nonexceptional-endpoint` asserts that, on the symmetry-breaking side, `Φ_H(p,r)`,
`e(W_{p,r})` and the four block parameters are **analytic functions of `(p,r)`**.  Since `e(W_{p,r}) = r - δ_*(p,r)` and
...
`bipodal_family_zeta` is the assembly, and `bipodal_family` its projection without the identity
`q₁₂(r,0) = ζ_d(r)` and without the clause identifying the parameter maps near `(r,0)` with the
analytically extended KRR–S parameter maps.  It takes **one** chart
(`boundaryExcess_chart_full_zeta`) and
...
`W_{p,r}` through its block parameters `c, q₁₁, q₁₂, q₂₂` is rendered as: that concrete
graphon `B` is an optimizer, and every optimizer — `B` included — is `W_*` up to a
measure-preserving relabelling.  Since relabellings (`IsRelabelling`) carry an a.e. inverse
and compose (`IsRelabelling.exists_symm`, `IsRelabelling.comp`), the choice of reference
loses nothing.  This is the convention already used by `symmetry_breaking_side`.

No patching of local families over a compact set is needed: as in the paper's proof, the
whole of Section 4.3 runs inside one chart at `r₀`, so a single implicit-function family
suffices.
-/
...
is real-analytic there too.  This is the implicit-function step behind the analyticity
assertion of `thm:nonexceptional-endpoint`, stated on its own; the finished assertion, in which
`Δ` is *identified* with the critical point of the tilted problem and the value function with
`Φ_H`, is `symmetry_breaking_analytic`.
...
/-- **`bipodal_family` with the KRR–S parameter maps identified.**  The same family, window and
clauses as `bipodal_family`, together with two clauses on the window `|r - r₀| < ρ`, both carried by
the chart `boundaryExcess_chart_full_zeta`:

* `q₁₂(r,0) = ζ_d(r)`;
* the parameter maps, as functions of `(ε, ϑ) = (e, t_H - e^m)`, are the analytically extended
  KRR–S parameter maps of `thm:krrs-analytic-extension` near every `(r, 0)`: for some `Δ > 0` they
  are analytic at every `(r, ϑ)` with `|ϑ| < Δ`, and for `0 < ϑ = τ - r^m < Δ` the entropy
  maximizer at `(e, t_H) = (r, τ)` is unique up to relabelling, bipodal, and equal a.e. to the
  two-block graphon with parameters `(c, q₁₁, q₁₂, q₂₂)(r, ϑ)`, `0 < c(r,ϑ) < 1/2`. -/
theorem bipodal_family_zeta {d : ℕ} (hd : 2 ≤ d) (M : LZBoundaryArc d)
    {V : Type*} [Fintype V] [DecidableEq V] (H : SimpleGraph V) [DecidableRel H.Adj]
    (hreg : ∀ v, H.degree v = d) (hm : 1 ≤ H.edgeFinset.card)
    {r₀ : ℝ} (hr₀U : r₀ ∈ M.U) (hr₀ex : r₀ ≠ rStar d) :
    ∃ ρ η L Cd : ℝ, 0 < ρ ∧ 0 < η ∧ 0 ≤ L ∧ 0 ≤ Cd ∧
      ∃ (Dl : ℝ × ℝ → ℝ) (q11 q12 q22 cc : ℝ → ℝ → ℝ),
        (∀ r : ℝ, |r - r₀| < ρ → q12 r 0 = zetaFun d r) ∧
        (∃ Δ : ℝ, 0 < Δ ∧ ∀ r : ℝ, |r - r₀| < ρ →
          (∀ ϑ : ℝ, |ϑ| < Δ →
  ...
  obtain ⟨w, ρch, Mch, hw0, hρch0, hMch0, Gc, q11, q12, q22, cc,
    hGcA, hG1A, heqG, -, hG10, hG2AH, hlipG, htayG, hparA, hparIoo, hbdry, hmaxKR, hzeta,
    ⟨Δ, hΔ0, hKRS⟩⟩ :=
  ...
  have key : ∀ r : ℝ, |r - r₀| < ρ → ∀ p : ℝ, M.pc r - η < p → p < M.pc r →
      ...
      phiVar H p r = reducedObjectiveReal H p (r - Dl (p, r)) (r ^ H.edgeFinset.card) ∧
      (∀ W : Graphon, Feasible H r W → W.Ip p = phiVar H p r →
        W.edgeDensity = r - Dl (p, r)) ∧
    ...
    exact h
  -- ### The output
  refine ⟨ρ, η, L, C₁, hρ0, hη0, hL0, hC₁0, Dl, q11, q12, q22, cc,
    fun r hrρ => hzeta r (by linarith [hρρ₁, hρ₁w, hrρ]),
    ⟨Δ, hΔ0, fun r hrρ => hKRS r (by linarith [hρρ₁, hρ₁w, hrρ])⟩, ?_, ?_⟩
        ...
        ∀ r : ℝ, |r - r₀| < ρ → ∀ p : ℝ, M.pc r - η < p → p < M.pc r →
          ...
          ∃ ε θ : ℝ, ε = r - Dl (p, r) ∧
            ...
            (∃ B : Graphon, Feasible H r B ∧ B.Ip p = phiVar H p r ∧
              ...
              (∀ᵐ z ∂gμ, B.toFun z.1 z.2
                = bipodalValue (Set.Icc 0 (cc ε θ)) (q11 ε θ) (q12 ε θ) (q22 ε θ) z)) := by
  obtain ⟨ρ, η, L, Cd, hρ, hη, hL, hCd, Dl, q11, q12, q22, cc, -, -, hbase, hwin⟩ :=
    bipodal_family_zeta hd M H hreg hm hr₀U hr₀ex
  exact ⟨ρ, η, L, Cd, hρ, hη, hL, hCd, Dl, q11, q12, q22, cc, hbase, hwin⟩

/-- **The analyticity assertion of `thm:nonexceptional-endpoint`.**  On the symmetry-breaking
side of the window,
...
* the edge density of the optimizer is `r - Δ(p,r)` for an analytic `Δ` with `Δ > 0` — the
  identity holds for *every* optimizer, so the assertion carries its full content even though the
...
/-! ## `thm:nonexceptional-endpoint`: the first block is the smaller one on one window
...
/-- **`bipodal_family_smallBlock` with the KRR–S parameter maps identified.**  The same family,
window and clauses as `bipodal_family_smallBlock`, together with the two clauses of
`bipodal_family_zeta` on the window: `q₁₂(r,0) = ζ_d(r)`, and near every `(r, 0)` the parameter
maps are the analytically extended KRR–S parameter maps of `thm:krrs-analytic-extension`. -/
theorem bipodal_family_smallBlock_zeta {d : ℕ} (hd : 2 ≤ d) (M : LZBoundaryArc d)
    {V : Type*} [Fintype V] [DecidableEq V] (H : SimpleGraph V) [DecidableRel H.Adj]
    (hreg : ∀ v, H.degree v = d) (hm : 1 ≤ H.edgeFinset.card)
    {r₀ : ℝ} (hr₀U : r₀ ∈ M.U) (hr₀ex : r₀ ≠ rStar d) :
    ∃ ρ η L Cd : ℝ, 0 < ρ ∧ 0 < η ∧ 0 ≤ L ∧ 0 ≤ Cd ∧
  ...
  refine ⟨ρ, η, L, max Cd C, hρ0, hη0, hL0, le_trans hCd0 (le_max_left _ _),
    Dl, q11, q12, q22, cc, fun r hr => hzeta r (lt_of_lt_of_le hr hρ₁),
    ⟨Δ, hΔ0, fun r hr => hKRS r (lt_of_lt_of_le hr hρ₁)⟩, ?_, ?_⟩
  · intro r hr
...
/-- **`thm:nonexceptional-endpoint` for one analytic bipodal family on one window.**
          ...
          ∃ ε θ : ℝ, ε = r - Dl (p, r) ∧
            ...
            (∃ B : Graphon, Feasible H r B ∧ B.Ip p = phiVar H p r ∧
              ...
              (∀ᵐ z ∂gμ, B.toFun z.1 z.2
                = bipodalValue (Set.Icc 0 (cc ε θ)) (q11 ε θ) (q12 ε θ) (q22 ε θ) z)) := by
  obtain ⟨ρ, η, L, Cd, hρ, hη, hL, hCd, Dl, q11, q12, q22, cc, -, -, hbase, hwin⟩ :=
    bipodal_family_smallBlock_zeta hd M H hreg hm hr₀U hr₀ex
  exact ⟨ρ, η, L, Cd, hρ, hη, hL, hCd, Dl, q11, q12, q22, cc, hbase, hwin⟩
