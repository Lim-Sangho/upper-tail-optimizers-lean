...
/-!
# Section 4.1: the local reduction (`sec:local-reduction` of `paper/paper.tex`)
...
formalises the results of Section 4.1:
...
Kenyon–Radin–Ren–Sadun (`thm:krrs-analytic-extension`, `kenyonRadinRenSadun` of `Preliminaries/KRRSAnalyticExtension/Main.lean`), the
...
`lem:boundary-convergence` (`e(W_n) → r₀`).  It is the step that
...
theorem entropyEnvelope_eq_of_isMax {V : Type*} [Fintype V] [DecidableEq V] (H : SimpleGraph V)
    ...
    entropyEnvelopeReal H ε τ = Wmax.entropy := by
  refine IsGreatest.csSup_eq ⟨⟨Wmax, he, ht, rfl⟩, ?_⟩
  rintro s ⟨W', hW'e, hW't, rfl⟩
  exact hmax W' hW'e hW't

/-- At a fixed-`(ε,τ)` entropy maximizer `Wmax`, the reduced objective equals `I_p(Wmax)`; for
`τ = r^m` this is `I_{p,r}(ε) = I_p(Wmax)` (`eq:reduced-objective-attainment`). -/
theorem reducedObjective_eq_Ip {V : Type*} [Fintype V] [DecidableEq V] (H : SimpleGraph V)
    ...
    (hmax : ∀ W' : Graphon, W'.edgeDensity = ε → W'.tDensity H = τ → W'.entropy ≤ Wmax.entropy) :
    reducedObjectiveReal H p ε τ = Wmax.Ip p := by
  unfold reducedObjectiveReal
...
/-- In `EReal`, the supremum of a nonempty set of reals that is bounded above is the real
supremum. -/
private theorem ereal_sSup_image_coe {S : Set ℝ} (hne : S.Nonempty) (hbdd : BddAbove S) :
    sSup ((fun x : ℝ => (x : EReal)) '' S) = ((sSup S : ℝ) : EReal) := by
  refine IsLUB.sSup_eq ⟨?_, fun b hb => ?_⟩
  · rintro _ ⟨x, hx, rfl⟩
    exact EReal.coe_le_coe_iff.mpr (le_csSup hbdd hx)
  · induction b using EReal.rec with
    | bot =>
      obtain ⟨x, hx⟩ := hne
      exact absurd (hb ⟨x, hx, rfl⟩) (by simp)
    | coe y =>
...
noncomputable def reducedObjective {V : Type*} [Fintype V] [DecidableEq V] (H : SimpleGraph V)
  ...
  (-2 : EReal) * entropyEnvelope H ε (r ^ H.edgeFinset.card)
    + ((-Real.log (1 - p) + ε * Real.log ((1 - p) / p) : ℝ) : EReal)

/-- When some graphon has edge density `ε` and `H`-density `τ`, the envelope is the real
supremum `entropyEnvelopeReal`. -/
theorem entropyEnvelope_eq_coe {V : Type*} [Fintype V] [DecidableEq V] (H : SimpleGraph V)
    [DecidableRel H.Adj] {ε τ : ℝ} (hne : ∃ W : Graphon, W.edgeDensity = ε ∧ W.tDensity H = τ) :
    entropyEnvelope H ε τ = (entropyEnvelopeReal H ε τ : EReal) := by
  have himage : (fun W : Graphon => (W.entropy : EReal)) '' {W | W.edgeDensity = ε ∧ W.tDensity H = τ}
      = (fun x : ℝ => (x : EReal)) ''
          {s | ∃ W : Graphon, W.edgeDensity = ε ∧ W.tDensity H = τ ∧ W.entropy = s} := by
    ext v
    constructor
    · rintro ⟨W, ⟨he, ht⟩, rfl⟩
      exact ⟨W.entropy, ⟨W, he, ht, rfl⟩, rfl⟩
    · rintro ⟨_, ⟨W, he, ht, rfl⟩, rfl⟩
      exact ⟨W, ⟨he, ht⟩, rfl⟩
  obtain ⟨W, he, ht⟩ := hne
  rw [entropyEnvelope, himage, entropyEnvelopeReal]
  exact ereal_sSup_image_coe ⟨W.entropy, W, he, ht, rfl⟩ (entropyEnvelope_bddAbove H ε τ)

/-- When no graphon has edge density `ε` and `H`-density `τ`, the envelope is `-∞`. -/
theorem entropyEnvelope_eq_bot {V : Type*} [Fintype V] [DecidableEq V] (H : SimpleGraph V)
    [DecidableRel H.Adj] {ε τ : ℝ}
    (hempty : ¬ ∃ W : Graphon, W.edgeDensity = ε ∧ W.tDensity H = τ) :
    entropyEnvelope H ε τ = ⊥ := by
  have h : {W : Graphon | W.edgeDensity = ε ∧ W.tDensity H = τ} = ∅ :=
    Set.eq_empty_iff_forall_notMem.mpr fun W hW => hempty ⟨W, hW⟩
  rw [entropyEnvelope, h, Set.image_empty, sSup_empty]

/-- When some graphon has edge density `ε` and `H`-density `r^m`, the reduced objective is the
real value `reducedObjectiveReal`. -/
theorem reducedObjective_eq_coe {V : Type*} [Fintype V] [DecidableEq V] (H : SimpleGraph V)
    [DecidableRel H.Adj] {p r ε : ℝ}
    (hne : ∃ W : Graphon, W.edgeDensity = ε ∧ W.tDensity H = r ^ H.edgeFinset.card) :
    reducedObjective H p r ε = (reducedObjectiveReal H p ε (r ^ H.edgeFinset.card) : EReal) := by
  rw [reducedObjective, entropyEnvelope_eq_coe H hne, reducedObjectiveReal,
    show (-2 : EReal) = ((-2 : ℝ) : EReal) from rfl, ← EReal.coe_mul, ← EReal.coe_add]
  congr 1
  ring

/-- When no graphon has edge density `ε` and `H`-density `r^m`, the reduced objective is `+∞`. -/
theorem reducedObjective_eq_top {V : Type*} [Fintype V] [DecidableEq V] (H : SimpleGraph V)
    [DecidableRel H.Adj] {p r ε : ℝ}
    (hempty : ¬ ∃ W : Graphon, W.edgeDensity = ε ∧ W.tDensity H = r ^ H.edgeFinset.card) :
    reducedObjective H p r ε = ⊤ := by
  rw [reducedObjective, entropyEnvelope_eq_bot H hempty,
    show (-2 : EReal) = ((-2 : ℝ) : EReal) from rfl,
    EReal.coe_mul_bot_of_neg (by norm_num : (-2 : ℝ) < 0), EReal.top_add_coe]
...
neighbourhoods, take `Δ_*` to be the smallest of their excess thresholds, and take `ρ₀`
...
`r^m - ε^m ≤ m(r-ε) < m ρ₀ ≤ Δ_*/2` keeps the `H`-density excess below every `Δ_i`
(the strictness comes from `r - ε < ρ₀`, not from the bound on `ρ₀`).

The maximizer `B_{ε,r}` is returned as an honest function `What : ℝ → ℝ → Graphon` (junk value
off the strip).  That matters downstream: two optimizers with the same edge density are
relabellings of the *same* graphon, which is what makes "unique up to relabelling" usable. -/
...
`B_{e(W),r}` — which is itself an optimizer. -/
theorem reduction_core_uniform {d : ℕ} (hd : 2 ≤ d) (M : LZBoundaryArc d)
    ...
    ∃ ρ₀ : ℝ, 0 < ρ₀ ∧ ∃ What : ℝ → ℝ → Graphon,
      (∀ r ∈ K, ∀ ε ∈ Set.Ioo (r - ρ₀) r,
        ...
        ∀ p : ℝ, 0 < p → p < 1 →
          reducedObjectiveReal H p ε (r ^ H.edgeFinset.card) = (What ε r).Ip p) ∧
      (∀ ρ : ℝ, 0 < ρ → ρ ≤ ρ₀ → ∃ η : ℝ, 0 < η ∧ (∀ r ∈ K, η ≤ M.pc r) ∧
        ∀ r ∈ K, ∀ p, M.pc r - η < p → p < M.pc r →
          ...
          (∀ Wopt : Graphon, Feasible H r Wopt → Wopt.Ip p = phiVar H p r →
            ...
            phiVar H p r = reducedObjectiveReal H p Wopt.edgeDensity (r ^ H.edgeFinset.card) ∧
            IsBipodal Wopt ∧
  ...
  · -- the maximizer family and the identity `I_{p,r}(ε) = I_p(B_{ε,r})`
    intro r hr ε hε
  ...
  · -- the localization window, uniform over `K`
    ...
    -- `Φ ≤ I_{p,r}(ε)` on the whole strip (the maximizer is feasible)
    ...
    have hs_eq : Wopt.entropy = (What Wopt.edgeDensity r).entropy := by
      ...
      rw [hIpeq] at h1
      linarith
    -- uniqueness up to relabelling ⇒ `Wopt = B_{e(Wopt),r} ∘ (σ × σ)` a.e., hence bipodal
    ...
    have hphiF : phiVar H p r
        = reducedObjectiveReal H p Wopt.edgeDensity (r ^ H.edgeFinset.card) := by
...
`reduction_core_uniform`: for each broken-side `p` it returns the lower bound `Φ ≤ I_{p,r}(ε)`
on the strip; an attained optimizer; and, for *every* optimizer `Wopt`, that its edge density
realizes `Φ = I_{p,r}(e(Wopt))`, that `Wopt` is **bipodal**, and that `Wopt` equals the KRRS bipodal
...
private theorem reduction_core {d : ℕ} (hd : 2 ≤ d) (M : LZBoundaryArc d) {r : ℝ} (hr : r ∈ M.U)
    ...
    ∃ ρ : ℝ, 0 < ρ ∧ ∃ η : ℝ, 0 < η ∧ ∀ p, M.pc r - η < p → p < M.pc r →
      (∀ ε ∈ Set.Ioo (r - ρ) r, phiVar H p r ≤ reducedObjectiveReal H p ε (r ^ H.edgeFinset.card)) ∧
      (∃ W0 : Graphon, Feasible H r W0 ∧ W0.Ip p = phiVar H p r) ∧
      (∀ Wopt : Graphon, Feasible H r Wopt → Wopt.Ip p = phiVar H p r →
        Wopt.edgeDensity ∈ Set.Ioo (r - ρ) r ∧
        phiVar H p r = reducedObjectiveReal H p Wopt.edgeDensity (r ^ H.edgeFinset.card) ∧
...
theorem scalar_reduction {d : ℕ} (hd : 2 ≤ d) (M : LZBoundaryArc d) {r : ℝ} (hr : r ∈ M.U)
    ...
    ∃ ρ : ℝ, 0 < ρ ∧ ∃ η : ℝ, 0 < η ∧ ∀ p, M.pc r - η < p → p < M.pc r →
      ∃ ε ∈ Set.Ioo (r - ρ) r,
        phiVar H p r = reducedObjectiveReal H p ε (r ^ H.edgeFinset.card) ∧
        ∀ ε' ∈ Set.Ioo (r - ρ) r,
          reducedObjectiveReal H p ε (r ^ H.edgeFinset.card)
            ≤ reducedObjectiveReal H p ε' (r ^ H.edgeFinset.card) := by
...
upper-tail problem admits a bipodal minimizer.  The bipodality half of
`thm:nonexceptional-optimizers` and `thm:nonexceptional-endpoint`. -/
